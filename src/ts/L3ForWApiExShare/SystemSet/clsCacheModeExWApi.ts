 ...
 * Created by pyf on 2024年11月26日.
 * 注意:该类必须与调用界面处于同一个包,否则调用不成功!
 **/
import { ObjectAssign } from '@/ts/PubFun/clsCommFunc4Web';
import {
  CacheMode_GetObjLstByPagerAsync,
  CacheMode_SortFunByKey,
 ...
 * @returns 对象列表
 */
export async function CacheModeEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsCacheModeENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrCacheModeObjLst = await CacheMode_GetObjLstByPagerAsync(objPagerPara);
