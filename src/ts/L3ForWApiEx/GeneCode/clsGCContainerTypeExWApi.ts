 ...
 * Created by pyf on 2023年12月25日.
 * 注意:该类必须与调用界面处于同一个包,否则调用不成功!
 **/
import { ObjectAssign, GetSortExpressInfo } from '@/ts/PubFun/clsCommFunc4Web';
import {
  GCContainerType_SortFunByKey,
  GCContainerType_GetObjLstByPagerAsync,
...
export const gCContainerTypeExController = 'GCContainerTypeExApi';
export const gCContainerTypeEx_ConstructorName = 'gCContainerTypeEx';
 ...
 **/
export function GCContainerTypeEx_GetWebApiUrl(strController: string, strAction: string): string {
 ...
 **/
export function GCContainerTypeEx_CopyToEx(
  objGCContainerTypeENS: clsGCContainerTypeEN,
): clsGCContainerTypeENEx {
  const strThisFuncName = GCContainerTypeEx_CopyToEx.name;
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      gCContainerTypeEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function GCContainerTypeEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsGCContainerTypeENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrGCContainerTypeObjLst = await GCContainerType_GetObjLstByPagerAsync(objPagerPara);
  const arrGCContainerTypeExObjLst = arrGCContainerTypeObjLst.map(GCContainerTypeEx_CopyToEx);
  ...
  if (
    ...
    for (const objInFor of arrGCContainerTypeExObjLst) {
      await GCContainerTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrGCContainerTypeSel = arrGCContainerTypeSel.sort(
        GCContainerTypeEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 返回两个对象比较的结果
 */
export function GCContainerTypeEx_SortFunByKey(strKey: string, AscOrDesc: string) {
 ...
 */
export function GCContainerTypeEx_FuncMapByFldName(
  strFldName: string,
  objGCContainerTypeEx: clsGCContainerTypeENEx,
) {
  const strThisFuncName = GCContainerTypeEx_FuncMapByFldName.name;
