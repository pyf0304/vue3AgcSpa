...
import {
  ...
  PrjDataBase_SortFunByKey,
  PrjDataBase_GetObjLstByPagerAsync,
...
export const prjDataBaseExController = 'PrjDataBaseExApi';
export const prjDataBaseEx_ConstructorName = 'prjDataBaseEx';
 ...
 **/
export function PrjDataBaseEx_GetWebApiUrl(strController: string, strAction: string): string {
 ...
 **/
export function PrjDataBaseEx_CopyToEx(objPrjDataBaseENS: clsPrjDataBaseEN): clsPrjDataBaseENEx {
  const strThisFuncName = PrjDataBaseEx_CopyToEx.name;
 ...
 * @returns 对象列表
 */
export async function PrjDataBaseEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsPrjDataBaseENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrPrjDataBaseObjLst = await PrjDataBase_GetObjLstByPagerAsync(objPagerPara);
  const arrPrjDataBaseExObjLst = arrPrjDataBaseObjLst.map(PrjDataBaseEx_CopyToEx);
  ...
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrPrjDataBaseSel = arrPrjDataBaseSel.sort(
        PrjDataBaseEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 返回两个对象比较的结果
 */
export function PrjDataBaseEx_SortFunByKey(strKey: string, AscOrDesc: string) {
 ...
 */
export function PrjDataBaseEx_FuncMapByFldName(
  strFldName: string,
  objPrjDataBaseEx: clsPrjDataBaseENEx,
) {
  const strThisFuncName = PrjDataBaseEx_FuncMapByFldName.name;
...
export async function PrjDataBaseEx_GetObjExLstByPrjId(
  strPrjId: string,
): Promise<Array<clsPrjDataBaseENEx>> {
  // const strThisFuncName = 'GetObjExLstByPrjId';

  const strCondition = `UseStateId = '0001' and PrjDataBaseId in (Select PrjDataBaseId from ProjectDatabaseRel where PrjId = '${strPrjId}')  Order By PrjDataBaseName`;

  const arrPrjDataBaseObjLst = await PrjDataBase_GetObjLstAsync(strCondition);
  const arrPrjDataBaseExObjLst = arrPrjDataBaseObjLst.map(PrjDataBaseEx_CopyToEx);
