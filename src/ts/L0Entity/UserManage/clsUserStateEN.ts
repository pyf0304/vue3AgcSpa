...
export class clsUserStateEN extends clsGeneralTab {
   ...
   * (AutoGCLib.EntityLayer4TypeScript:Gen_EN_ClsProperty)
   */
  public SetUserStateName(value: string) {
    if (value != undefined) {
  ...
  public GetFldValue(strFldName: string): any {
    ...
    switch (strFldName) {
      ...
      default:
        strMsg = `字段名:[${strFldName}]在表对象:[UserState]中不存在!`;
  ...
  public SetFldValue(strFldName: string, strValue: string) {
    ...
    switch (strFldName) {
      ...
      case 'sfFldComparisonOp':
        ...
        break;
      default:
        strMsg = `字段名:[${strFldName}]在表对象:[UserState]中不存在!(in ${this.constructor.name}.${strThisFuncName})`;
        console.error(strMsg);
        break;
    }
  }
