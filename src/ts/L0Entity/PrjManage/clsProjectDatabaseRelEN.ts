...
export class clsProjectDatabaseRelEN extends clsGeneralTab {
  public static CacheAddiCondition = ''; //缓存附加条件,作为向后台调取数据的附加条件
  public static CacheModeId = '03'; //localStorage
  public static IsUseDelSign = false; //使用删除标志,记录不能删除,仅设置删除标志
  public static WhereFormat = ''; //条件格式串
  public static _CurrTabName = 'ProjectDatabaseRel'; //当前表名,与该类相关的表名
  public static _KeyFldName = 'mId'; //当前表中的关键字名称,与该类相关的表中关键字名
  public static mintAttributeCount = 4;
  public static AttributeName = ['mId', 'prjId', 'prjDataBaseId', 'memo'];
  //以下是属性变量
  ...
  public GetFldValue(strFldName: string): any {
    ...
    switch (strFldName) {
      ...
      default:
        strMsg = `字段名:[${strFldName}]在表对象:[ProjectDatabaseRel]中不存在!`;
  ...
  public SetFldValue(strFldName: string, strValue: string) {
    ...
    switch (strFldName) {
      ...
      case 'sfFldComparisonOp':
        ...
        break;
      default:
        strMsg = `字段名:[${strFldName}]在表对象:[ProjectDatabaseRel]中不存在!(in ${this.constructor.name}.${strThisFuncName})`;
        console.error(strMsg);
        break;
    }
  }
