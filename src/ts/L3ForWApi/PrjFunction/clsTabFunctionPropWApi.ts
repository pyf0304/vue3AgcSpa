...
export async function TabFunctionProp_func(
  ...
  if (clsTabFunctionPropEN._AttributeName.indexOf(strOutFldName) == -1) {
    const strMsg = Format(
      '输出字段名:[{0}]不正确,不在输出字段范围之内!({1})',
      strOutFldName,
      clsTabFunctionPropEN._AttributeName.join(','),
    );
    console.error(strMsg);
    throw new Error(strMsg);
  }
...
export async function TabFunctionProp_GetObjLstClientCache(strTabId: string) {
  ...
  if (IsNullOrEmpty(clsTabFunctionPropEN._WhereFormat) == false) {
    strWhereCond = Format(clsTabFunctionPropEN._WhereFormat, strTabId);
  } else {
    strWhereCond = Format("TabId='{0}'", strTabId);
  }
  const strKey = Format('{0}_{1}', clsTabFunctionPropEN._CurrTabName, strTabId);
  if (IsNullOrEmpty(clsTabFunctionPropEN._CacheAddiCondition) == false) {
    strWhereCond += Format(' and {0}', clsTabFunctionPropEN._CacheAddiCondition);
  }
  if (strKey == '') {
    console.error('关键字为空!不正确');
    throw new Error('关键字为空!不正确');
...
export async function TabFunctionProp_GetObjLstCache(
  ...
  switch (clsTabFunctionPropEN._CacheModeId) {
...
export async function TabFunctionProp_GetObjExLstByPagerCache(
  ...
  if (
    ...
    clsTabFunctionPropEN._AttributeName.indexOf(objSortInfo.SortFld) == -1 &&
