/**
 * 类名:FunctionTemplateCRUD(界面:FunctionTemplateCRUD,00050347)
 * 表名:FunctionTemplate(00050312)
 * 版本:2026.04.19(服务器:WIN-SRV103-116)
 * 日期:2026/04/28 23:46:06
 * 生成者:
 工程名称:AGC(0005)
...
import {
  ...
  FunctionTemplate_DelRecordAsync,
  ...
  FunctionTemplate_GetObjLstByFunctionTemplateIdLstAsync,
  FunctionTemplate_GetMaxStrIdAsync,
  FunctionTemplate_AddNewRecordAsync,
...
/** FunctionTemplateCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class FunctionTemplateCRUD implements clsOperateList {
  ...
  public CombineData(
    ...
    const intRowNum = arrFunctionTemplateObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsFunctionTemplateEN = arrFunctionTemplateObjLst[i];
  ...
  public async ExportExcel_FunctionTemplateCache(): Promise<ExportExcelData> {
    ...
    if (arrFunctionTemplateObjLst.length == 0) {
      const strKey = Format('{0}', clsFunctionTemplateEN._CurrTabName);
    ...
    try {
      const arrDataColumn: Array<clsDataColumn> = [
        ...
        {
          fldName: 'functionTemplateENName',
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '函数模板英文名',
      ...
      //console.log("完成BindGv_FunctionTemplate!");
    } catch (e) {
   ...
   **/
  public async btnSelectRecordInTab_Click(strFunctionTemplateId: string) {
    ...
    try {
      ...
      this.SelectRecord(strFunctionTemplateId);
    } catch (e) {
   ...
   **/
  public async DelRecord(strFunctionTemplateId: string) {
    const strThisFuncName = this.DelRecord.name;
    try {
      const returnInt = await FunctionTemplate_DelRecordAsync(strFunctionTemplateId);
      if (returnInt > 0) {
        FunctionTemplate_ReFreshCache();
   ...
   * @param sender:参数列表
   **/
  public async SelectRecord(strFunctionTemplateId: string) {
    const strThisFuncName = this.SelectRecord.name;
    try {
      const objFunctionTemplateEN = await FunctionTemplate_GetObjByFunctionTemplateIdAsync(
        strFunctionTemplateId,
      );
      console.log('完成SelectRecord!', objFunctionTemplateEN);
      Redirect('/Index/Main_FunctionTemplate');
    } catch (e) {
  ...
  /** 显示FunctionTemplate对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrFunctionTemplateObjLst:需要绑定的对象列表
   **/
  public async BindTab_FunctionTemplate(
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      {
        fldName: clsFunctionTemplateEN.con_UpdDate,
        sortBy: 'updDate',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '修改日期',
    ...
    if (refFunctionTemplate_List.value != null) {
      ...
      if (divDataLst == null) {
        alert('在BindTab_FunctionTemplate函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
   ...
   * @param arrFunctionTemplateExObjLst:需要绑定的对象列表
   **/
   ...
   * @param arrFunctionTemplateExObjLst:需要映射的对象列表
  ...
  public async BindGv_FunctionTemplate4Func(divList: HTMLDivElement) {
    ...
    try {
      ...
      if (FunctionTemplateCRUD.sortFunStatic != undefined) {
        strSortFun = FunctionTemplateCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
      }
    ...
    try {
      ...
      //console.log("完成BindGv_FunctionTemplate4Func!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName})`;
      console.error(strMsg);
      alert(strMsg);
   ...
   **/
  public CopyToEx(objFunctionTemplateENS: clsFunctionTemplateEN): clsFunctionTemplateENEx {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortFunctionTemplateBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsFunctionTemplateENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
    ...
    FunctionTemplateCRUD.sortFunStatic = sortFun;
    await this.BindGv_FunctionTemplate4Func(this.listPara.listDiv);
  }
  ...
  public async ExtendTdFldFuncMap(arrFunctionTemplateExObjLst: Array<clsFunctionTemplateENEx>) {
    const arrFldName = clsFunctionTemplateEN._AttributeName;
    const tdFieldNames = refFunctionTemplate_List.value?.tdFieldNames;
    for (const strFldName of tdFieldNames) {
      if (IsNullOrEmpty(strFldName) == true) continue;
      if (arrFldName.indexOf(strFldName) > -1) continue;
      for (const objInFor of arrFunctionTemplateExObjLst) {
        await FunctionTemplate_FuncMapByFldName(strFldName, objInFor);
      }
    }
  }
   ...
   **/
  public async CopyRecord(arrFunctionTemplateId: Array<string>) {
    const strThisFuncName = this.CopyRecord.name;
    try {
      const arrFunctionTemplateObjLst =
        await FunctionTemplate_GetObjLstByFunctionTemplateIdLstAsync(arrFunctionTemplateId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrFunctionTemplateObjLst) {
        const strMaxStrId = await FunctionTemplate_GetMaxStrIdAsync();
        //console.log('strMaxStrId=' + strMaxStrId);
        objInFor.functionTemplateId = strMaxStrId;
        const returnBool = await FunctionTemplate_AddNewRecordAsync(objInFor);
   ...
   * @param divContainer:显示容器
   * @param objFunctionTemplate:需要显示的对象
   **/
  public ShowFunctionTemplateObj(
    divContainer: HTMLDivElement,
    ...
    const sstrKeys = GetObjKeys(objFunctionTemplate);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objFunctionTemplate.GetFldValue(strKey);
   ...
   * @param pobjFunctionTemplateEN:表实体类对象
   * @returns 列表的第一个关键字值
   **/
