...
import {
  ...
  getListColumnsAi,
} from '@/viewsBase/PrjFunction/FunctionTemplateRelaCRUDAiColumns';

/**
 * Ai版基类：
 * 在Ai3基础上增加命令schema驱动，不改变原有基类行为。
 */
export abstract class FunctionTemplateRelaCRUDAi
  ...
  implements clsOperateList
{
  ...
  public async PageLoadCache() {
    try {
      ...
      if (viewVarSet.sortFunctionTemplateRelaBy == '') {
        viewVarSet.sortFunctionTemplateRelaBy = `${clsFunctionTemplateRelaEN.con_mId} Asc`;
      }
  ...
  public async btnSetFunctionTemplateId_Click() {
    try {
      ...
      await this.ExecuteSelectionActionTemplate({
        ...
        emptySelectionMessage: `请选择需要设置函数模板Id的${this.thisTabName}记录!`,
        beforeExecute: () => {
      ...
      const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${this.btnSetFunctionTemplateId_Click.name}`;
      console.error(strMsg);
      alert(strMsg);
    }
  }
  ...
  public async DelMultiRecord(arrFunctionTemplateRelaId: Array<string>) {
    const strThisFuncName = this.DelMultiRecord.name;
    try {
      const returnInt = await FunctionTemplateRela_DelFunctionTemplateRelasAsync(
        arrFunctionTemplateRelaId,
      );
      if (returnInt > 0) {
  ...
  /** 批量设置函数模板Id。 */
  public async SetFunctionTemplateId(
    arrFunctionTemplateRelaId: Array<string>,
    ...
    if (strFunctionTemplateId == null || strFunctionTemplateId == '') {
      alert('请输入函数模板Id(FunctionTemplateId)!');
      return '';
    }
    if (arrFunctionTemplateRelaId.length == 0) {
  ...
  /** 按导出列规格整理导出结果。 */
  public async ExportExcel_FunctionTemplateRelaAi(): Promise<ExportExcelData> {
    const raw = await this.ExportExcel_FunctionTemplateRela();
  ...
  public async BindTab_FunctionTemplateRela4Func(
    ...
    if (refFunctionTemplateRela_List.value != null) {
      ...
      try {
        await this.ExtendTdFldFuncMap(arrFunctionTemplateRelaExObjLst);
      } catch (e) {
  ...
  public async BindGv_FunctionTemplateRela4Func(divList: HTMLDivElement) {
    ...
    try {
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortFunctionTemplateRelaBy,
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
  ...
  public async ExtendTdFldFuncMap(
    ...
    const tdFieldNames = this.ResolveTdFieldNames(
      refFunctionTemplateRela_List.value?.tdFieldNames,
      dataColumn.value,
      IsNullOrEmpty,
    );
    ...
    for (const normalizedFldName of tdFieldNames) {
      ...
      for (const objInFor of arrFunctionTemplateRelaExObjLst) {
        await FunctionTemplateRelaEx_FuncMapByFldName(normalizedFldName, objInFor);
      }
    }
  }
  ...
  public async SortBy(objAnchorElement: any) {
    await this.SortByTemplate({
      ...
      currSortBy: viewVarSet.sortFunctionTemplateRelaBy,
      onEntitySort: async (sortColumnKey: string, sortDirection: string) => {
        if (
          Object.prototype.hasOwnProperty.call(clsFunctionTemplateRelaENEx, sortColumnKey) == false
        ) {
          return false;
        }
      ...
      onApplySortState: (sortBy, ascOrDesc4SortFun, sortFun) => {
        ...
        const currentCtor = this.constructor as typeof FunctionTemplateRelaCRUDAi;
