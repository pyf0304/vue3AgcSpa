...
/** FunctionTemplateRela_EditAi 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScript_EditAi_TS4TypeScript:GeneCode)
 **/
export abstract class FunctionTemplateRela_EditAi {
  ...
  public SetKeyReadOnly(bolReadonly: boolean) {
    const txtmId = document.getElementById('txtmId') as HTMLInputElement | null;
    if (txtmId != null) {
      txtmId.readOnly = bolReadonly;
    }
  }
  /** 为插入记录做准备工作 */
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    try {
      ...
      if (returnBool == true) {
        // FunctionTemplateRela_ReFreshCache(); // 🔥 当前表未使用 localStorage/sessionStorage 缓存，无需刷新
