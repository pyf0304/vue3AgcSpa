...
export abstract class FunctionTemplateRela_Detail {
  ...
  public HideDialog_FunctionTemplateRela() {
    if (FunctionTemplateRela_Detail.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      refFunctionTemplateRela_Detail.value.hideDialog();
    }
  }
  ...
  public async ShowDialog_FunctionTemplateRela(strOp: string): Promise<boolean> {
    ...
    if (FunctionTemplateRela_Detail.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (refFunctionTemplateRela_Detail.value == null) {
        const strMsg = Format(
      ...
      await refFunctionTemplateRela_Detail.value.showDialog();
    }
    divVarSet.refDivDetail = refFunctionTemplateRela_Detail.value.$refs.refDivDetail;
    if (divVarSet.refDivDetail == null) {
      if (FunctionTemplateRela_Detail.times4TestShowDialog < 2) {
        FunctionTemplateRela_Detail.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_FunctionTemplateRela(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      FunctionTemplateRela_Detail.times4TestShowDialog = 0;
    }
    if (strOp === 'Detail') {
...
*/
  public async btnDetailRecordInTab_Click(strKeyId: string) {
    ...
    this.opType = 'Detail';
    const bolIsSuccess = await this.ShowDialog_FunctionTemplateRela('Detail');
...
*/
  public async DetailRecord(lngmId: number): Promise<boolean> {
    ...
    try {
      ...
      if (refFunctionTemplateRela_Detail.value == null) {
        const strMsg = Format(
          '当前详细信息区的DetailObj为空，请检查！(in {0}.{1}).\nrefFunctionTemplateRela_Detail imported from: @/views/PrjFunction/FunctionTemplateRelaVueShare',
          this.constructor.name,
          strThisFuncName,
        );
        console.error(strMsg);
      ...
      await refFunctionTemplateRela_Detail.value.ShowDataFromFunctionTemplateRelaObj(
...
*/
  ...
  public set btnCancelFunctionTemplateRela(value: string) {
    refFunctionTemplateRela_Detail.value.strCancelButtonText = value;
  }
}
