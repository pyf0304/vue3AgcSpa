/**
 * 类名:Function4GeneCode_DetailAi(界面:Function4GeneCodeCRUD,00050342)
 * 表名:Function4GeneCode(00050311)
 * 版本:2026.04.19(服务器:PYF-AI)
 * 日期:2026/05/29 10:40:37
 * 生成者:
 工程名称:AGC(0005)
...
/* Function4GeneCode_DetailAi 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_DetailAi_TS4TypeScript:GeneCode)
*/
export abstract class Function4GeneCode_DetailAi {
  ...
  public HideDialog_Function4GeneCode() {
    if (Function4GeneCode_DetailAi.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      refFunction4GeneCode_DetailAi.value.hideDialog();
    }
  }
  ...
  public async ShowDialog_Function4GeneCode(strOp: string): Promise<boolean> {
    ...
    if (Function4GeneCode_DetailAi.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (refFunction4GeneCode_DetailAi.value == null) {
        const strMsg = Format(
      ...
      await refFunction4GeneCode_DetailAi.value.showDialog();
    }
    divVarSet.refDivDetail = refFunction4GeneCode_DetailAi.value.$refs.refDivDetail;
    if (divVarSet.refDivDetail == null) {
      if (Function4GeneCode_DetailAi.times4TestShowDialog < 2) {
        Function4GeneCode_DetailAi.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_Function4GeneCode(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      Function4GeneCode_DetailAi.times4TestShowDialog = 0;
    }
    if (strOp === 'Detail') {
...
*/
  public async btnDetailRecordInTab_Click(key: Function4GeneCodeKey) {
    const strThisFuncName = this.btnDetailRecordInTab_Click.name;
    this.opType = 'Detail';
    const bolIsSuccess = await this.ShowDialog_Function4GeneCode('Detail');
...
*/
  public async DetailRecord(key: Function4GeneCodeKey): Promise<boolean> {
    ...
    try {
      ...
      if (refFunction4GeneCode_DetailAi.value == null) {
        const strMsg = Format(
          '当前详细信息区的DetailObj为空，请检查！(in {0}.{1}).\nrefFunction4GeneCode_DetailAi imported from: @/views/PrjFunction/Function4GeneCodeVueShare',
          this.constructor.name,
          strThisFuncName,
        );
        console.error(strMsg);
      ...
      await refFunction4GeneCode_DetailAi.value.ShowDataFromFunction4GeneCodeObj(
...
*/
  public async btnDetailRecord_Click(key: Function4GeneCodeKey) {
    this.opType = 'Detail';
    const bolIsSuccess = await this.ShowDialog_Function4GeneCode('Detail');
    if (bolIsSuccess == false) return;
  ...
  public set btnCancelFunction4GeneCode(value: string) {
    refFunction4GeneCode_DetailAi.value.strCancelButtonText = value;
  }
}
