...
/** Ai 场景下可用的命令标识。 */
export type PrjFileTypeCommandIdAi =
  | 'query'
  | 'create'
  | 'update'
  | 'delete'
...
export type PrjFileTypeCommandSpecAi = TableCommandSpec<PrjFileTypeCommandIdAi> & {
  /** 辅助控件ID（如下拉框ID） */
  auxControlId?: string;
  /** 辅助控件类型 */
...
export const PRJ_FILE_TYPE_COMMAND_SPECS_AI: Array<PrjFileTypeCommandSpecAi> = [
  {
    id: 'query',
    region: 'query',
    text: '查询',
...
/** 获取查询区命令。 */
export function getQueryCommandsAi(): Array<PrjFileTypeCommandSpecAi> {
  return PRJ_FILE_TYPE_COMMAND_SPECS_AI.filter((x) => x.region === 'query' && x.visible !== false);
}

/** 获取功能区命令。 */
export function getFeatureCommandsAi(): Array<PrjFileTypeCommandSpecAi> {
  return PRJ_FILE_TYPE_COMMAND_SPECS_AI.filter(
