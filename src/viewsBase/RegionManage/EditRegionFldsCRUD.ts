/**
 * 类名:EditRegionFldsCRUD(界面:EditRegionFldsCRUD,00050248)
 * 表名:EditRegionFlds(00050116)
 * 版本:2025.05.12.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/15 01:36:18
 * 生成者:
 工程名称:AGC(0005)
...
import {
  RegionId_Static,
  CombineEditRegionFldsConditionObj4ExportExcel,
...
import {
  ...
  EditRegionFlds_GetSubObjLstCache,
  EditRegionFlds_ReOrderAsync,
  EditRegionFlds_ReFreshCache,
  EditRegionFlds_GoBottomAsync,
  EditRegionFlds_DownMoveAsync,
  EditRegionFlds_UpMoveAsync,
  EditRegionFlds_GoTopAsync,
  EditRegionFlds_DelRecordAsync,
  EditRegionFlds_GetObjBymIdAsync,
  EditRegionFlds_GetObjExLstByPagerCache,
  EditRegionFlds_GetObjLstBymIdLstAsync,
  EditRegionFlds_AddNewRecordAsync,
  EditRegionFlds_UpdateRecordAsync,
  EditRegionFlds_DelEditRegionFldssAsync,
...
import { EditRegionFldsEx_FuncMapByFldName } from '@/ts/L3ForWApiEx/RegionManage/clsEditRegionFldsExWApi';
...
/** EditRegionFldsCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class EditRegionFldsCRUD implements clsOperateList {
  ...
  public static objPageCRUD: EditRegionFldsCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    EditRegionFldsCRUD.objPageCRUD = this;
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortEditRegionFldsBy == '')
        viewVarSet.sortEditRegionFldsBy = `${clsEditRegionFldsEN.con_SeqNum} Asc`;
   ...
   **/
  public CombineData(
    arrEditRegionFldsObjLst: Array<clsEditRegionFldsEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrEditRegionFldsObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsEditRegionFldsEN = arrEditRegionFldsObjLst[i];
    ...
    const strFileName = Format('编辑区域字段({0})导出.xlsx', clsEditRegionFldsEN._CurrTabName);
    const strSheetName = '编辑区域字段列表';
   ...
   **/
  public async ExportExcel_EditRegionFldsCache(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_EditRegionFldsCache.name;
    if (viewVarSet.sortEditRegionFldsBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortEditRegionFldsBy)为空,请检查!(In BindGv_EditRegionFldsCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return { arrObjLst: [], sheetName: '', fileName: '' };
    }
    const objEditRegionFldsCond = await CombineEditRegionFldsConditionObj4ExportExcel();
    ...
    try {
      this.recCount = await EditRegionFlds_GetRecCountByCondCache(
        objEditRegionFldsCond,
        RegionId_Static.value,
      );
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objEditRegionFldsCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrEditRegionFldsObjLst = await EditRegionFlds_GetSubObjLstCache(
        objEditRegionFldsCond,
        RegionId_Static.value,
      );
    } catch (e) {
    ...
    if (arrEditRegionFldsObjLst.length == 0) {
      const strKey = Format('{0}_{1}', clsEditRegionFldsEN._CurrTabName, RegionId_Static.value);
    ...
    try {
      ...
      arrEditRegionFldsObjLst = arrEditRegionFldsObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrEditRegionFldsObjLst, arrDataColumn);
      //console.log("完成BindGv_EditRegionFlds!");
    } catch (e) {
   ...
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_btnSetFldValue_Click)
   **/
  public async btnSetIsNotMultiRow_Click() {
    const strThisFuncName = this.btnSetIsNotMultiRow_Click.name;
    try {
      const arrKeyIds = GetCheckedKeyIdsInDivObj(divVarSet.refDivList);
      if (arrKeyIds.length == 0) {
        alert(`请选择需要设置是否多行的${this.thisTabName}记录!`);
        return '';
      }
      //console.log('arrKeyIds=');
      //console.log(arrKeyIds);
      await this.SetIsNotMultiRow(arrKeyIds);
      await this.BindGv_EditRegionFlds4Func(divVarSet.refDivList);
    } catch (e) {
      const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
   ...
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_btnSetFldValue_Click)
   **/
  public async btnSetIsMultiRow_Click() {
    const strThisFuncName = this.btnSetIsMultiRow_Click.name;
    try {
      const arrKeyIds = GetCheckedKeyIdsInDivObj(divVarSet.refDivList);
      if (arrKeyIds.length == 0) {
        alert(`请选择需要设置是否多行的${this.thisTabName}记录!`);
      ...
      //console.log('arrKeyIds=');
      //console.log(arrKeyIds);
      await this.SetIsMultiRow(arrKeyIds);
      await this.BindGv_EditRegionFlds4Func(divVarSet.refDivList);
    } catch (e) {
      const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
  ...
  public async btnReOrder_Click() {
    ...
    try {
      ...
      await EditRegionFlds_ReOrderAsync(objOrderByData);
  ...
  public async btnGoBottum_Click() {
    ...
    try {
      ...
      const jsonStr = JSON.stringify(jsonObject);
      objOrderByData.ClassificationFieldValueLst = jsonStr;
      await EditRegionFlds_GoBottomAsync(objOrderByData);
      EditRegionFlds_ReFreshCache(RegionId_Static.value);
    } catch (e) {
  ...
  public async btnDownMove_Click() {
    ...
    try {
      ...
      const jsonStr = JSON.stringify(jsonObject);
      objOrderByData.ClassificationFieldValueLst = jsonStr;
      await EditRegionFlds_DownMoveAsync(objOrderByData);
      EditRegionFlds_ReFreshCache(RegionId_Static.value);
    } catch (e) {
  ...
  public async btnUpMove_Click() {
    ...
    try {
      ...
      const jsonStr = JSON.stringify(jsonObject);
      objOrderByData.ClassificationFieldValueLst = jsonStr;
      await EditRegionFlds_UpMoveAsync(objOrderByData);
      EditRegionFlds_ReFreshCache(RegionId_Static.value);
    } catch (e) {
  ...
  public async btnGoTop_Click() {
    ...
    try {
      ...
      const jsonStr = JSON.stringify(jsonObject);
      objOrderByData.ClassificationFieldValueLst = jsonStr;
      await EditRegionFlds_GoTopAsync(objOrderByData);
      EditRegionFlds_ReFreshCache(RegionId_Static.value);
    } catch (e) {
  ...
  public async DelRecord(lngmId: number) {
    ...
    try {
      const returnInt = await EditRegionFlds_DelRecordAsync(lngmId);
      if (returnInt > 0) {
        EditRegionFlds_ReFreshCache(RegionId_Static.value);
  ...
  public async SelectRecord(lngmId: number) {
    ...
    try {
      ...
      console.log('完成SelectRecord!', objEditRegionFldsEN);
      Redirect('/Index/Main_EditRegionFlds');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_EditRegionFldsCache();
  }

  /** 显示EditRegionFlds对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrEditRegionFldsObjLst:需要绑定的对象列表
   **/
  public async BindTab_EditRegionFlds(
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      {
        fldName: clsEditRegionFldsEN.con_SeqNum,
        sortBy: clsEditRegionFldsEN.con_SeqNum,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '序号',
        text: '',
      ...
      {
        fldName: clsEditRegionFldsEN.con_RegionId,
        sortBy: 'tabName',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '表名',
      ...
      {
        fldName: clsEditRegionFldsEN.con_FldId,
        sortBy: 'fldName',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '字段名',
      ...
      {
        fldName: clsEditRegionFldsEN.con_LabelCaption,
        sortBy: clsEditRegionFldsEN.con_LabelCaption,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '标题',
        text: '',
      ...
      {
        fldName: clsEditRegionFldsENEx.con_CtlTypeName,
        sortBy: 'ctlTypeName',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '控件类型名',
      ...
      {
        fldName: clsEditRegionFldsEN.con_ColSpan,
        sortBy: clsEditRegionFldsEN.con_ColSpan,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '跨列数',
        text: '',
      ...
      {
        fldName: clsEditRegionFldsEN.con_Width,
        sortBy: clsEditRegionFldsEN.con_Width,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '宽',
        text: '',
      ...
      {
        fldName: clsEditRegionFldsEN.con_ClickEvent,
        sortBy: clsEditRegionFldsEN.con_ClickEvent,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: 'Click事件',
        text: '',
      ...
      {
        fldName: clsEditRegionFldsEN.con_ChangeEvent,
        sortBy: clsEditRegionFldsEN.con_ChangeEvent,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: 'Change事件',
        text: '',
    ...
    if (refEditRegionFlds_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrEditRegionFldsExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_EditRegionFlds函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
        arrEditRegionFldsExObjLst,
        arrDataColumn,
   ...
   * @param arrEditRegionFldsExObjLst:需要绑定的对象列表
   **/
  public async BindTab_EditRegionFlds4Func(
    ...
    if (refEditRegionFlds_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_EditRegionFlds4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
   ...
   * @param arrEditRegionFldsExObjLst:需要映射的对象列表
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_EditRegionFlds4Func(this.listPara.listDiv);
  }
  ...
  public async BindGv_EditRegionFlds4Func(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_EditRegionFlds4Func.name;
    if (viewVarSet.sortEditRegionFldsBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortEditRegionFldsBy)为空,请检查!(In BindGv_EditRegionFldsCache)',
      );
    ...
    const objEditRegionFldsCond = await CombineEditRegionFldsConditionObj();
    objEditRegionFldsCond.SetCondFldValue(
      clsEditRegionFldsEN.con_RegionId,
      RegionId_Static.value,
      '=',
    );
    const strWhereCond = JSON.stringify(objEditRegionFldsCond);
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrEditRegionFldsExObjLst, true);
        return;
      }
      ...
      if (EditRegionFldsCRUD.sortFunStatic != undefined) {
        strSortFun = EditRegionFldsCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
      }
      const objPagerPara: stuPagerPara = {
        pageIndex: intCurrPageIndex,
        pageSize: this.pageSize,
        whereCond: strWhereCond,
        conditionCollection: objEditRegionFldsCond,
    ...
    try {
      ...
      //console.log("完成BindGv_EditRegionFlds4Func!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName})`;
      console.error(strMsg);
      alert(strMsg);
   ...
   **/
  public CopyToEx(objEditRegionFldsENS: clsEditRegionFldsEN): clsEditRegionFldsENEx {
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsEditRegionFldsEN, b: clsEditRegionFldsEN): number {
    if (a.prjId == b.prjId) {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortEditRegionFldsBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsEditRegionFldsENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortEditRegionFldsBy = sortBy;
    viewVarSet.ascOrDesc4SortFun = ascOrDesc4SortFun;
    EditRegionFldsCRUD.sortFunStatic = sortFun;
    await this.BindGv_EditRegionFlds4Func(this.listPara.listDiv);
  }
  ...
  public async CopyRecord(arrmId: Array<string>) {
    ...
    try {
      const arrEditRegionFldsObjLst = await EditRegionFlds_GetObjLstBymIdLstAsync(arrmId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrEditRegionFldsObjLst) {
        const returnBool = await EditRegionFlds_AddNewRecordAsync(objInFor);
  ...
  public async SetInUse(arrmId: Array<string>) {
    ...
    try {
      ...
      let intCount = 0;
      for (const objInFor of arrEditRegionFldsObjLst) {
        const objEditRegionFldsEN = new clsEditRegionFldsEN();
        ObjectAssign(objEditRegionFldsEN, objInFor);
        objEditRegionFldsEN.SetmId(objInFor.mId);
        objEditRegionFldsEN.SetInUse(TransToBool('true'));
        let returnBool = false;
        try {
          objEditRegionFldsEN.sfUpdFldSetStr = objEditRegionFldsEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await EditRegionFlds_UpdateRecordAsync(objEditRegionFldsEN);
        } catch (e) {
          const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
          console.error(strMsg);
          throw strMsg;
        ...
        if (returnBool == true) {
          EditRegionFlds_DeleteKeyIdCache(RegionId_Static.value, objInFor.mId);
          intCount++;
        } else {
  ...
  public async SetNotInUse(arrmId: Array<string>) {
    ...
    try {
      ...
      for (const objInFor of arrEditRegionFldsObjLst) {
        ...
        ObjectAssign(objEditRegionFldsEN, objInFor);
        objEditRegionFldsEN.SetmId(objInFor.mId);
        objEditRegionFldsEN.SetInUse(TransToBool('false'));
        let returnBool = false;
        try {
          objEditRegionFldsEN.sfUpdFldSetStr = objEditRegionFldsEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await EditRegionFlds_UpdateRecordAsync(objEditRegionFldsEN);
   ...
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_SetFieldValue)
   **/
  public async SetIsNotMultiRow(arrmId: Array<string>) {
    const strThisFuncName = this.SetIsNotMultiRow.name;
    if (arrmId.length == 0) {
      const strMsg = '没有选择记录,不能设置字段值!';
      console.error('Error: ', strMsg);
      //console.trace();
    ...
    try {
      ...
      for (const objInFor of arrEditRegionFldsObjLst) {
        ...
        ObjectAssign(objEditRegionFldsEN, objInFor);
        objEditRegionFldsEN.SetmId(objInFor.mId);
        objEditRegionFldsEN.SetIsMultiRow(TransToBool('false'));
        let returnBool = false;
        try {
          objEditRegionFldsEN.sfUpdFldSetStr = objEditRegionFldsEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await EditRegionFlds_UpdateRecordAsync(objEditRegionFldsEN);
   ...
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_SetFieldValue)
   **/
  public async SetIsMultiRow(arrmId: Array<string>) {
    const strThisFuncName = this.SetIsMultiRow.name;
    if (arrmId.length == 0) {
      const strMsg = '没有选择记录,不能设置字段值!';
      console.error('Error: ', strMsg);
      //console.trace();
    ...
    try {
      ...
      for (const objInFor of arrEditRegionFldsObjLst) {
        ...
        ObjectAssign(objEditRegionFldsEN, objInFor);
        objEditRegionFldsEN.SetmId(objInFor.mId);
        objEditRegionFldsEN.SetIsMultiRow(TransToBool('true'));
        let returnBool = false;
        try {
          objEditRegionFldsEN.sfUpdFldSetStr = objEditRegionFldsEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await EditRegionFlds_UpdateRecordAsync(objEditRegionFldsEN);
  ...
  public async SetColSpan(arrmId: Array<string>, intColSpan: number) {
    ...
    try {
      ...
      for (const objInFor of arrEditRegionFldsObjLst) {
        ...
        ObjectAssign(objEditRegionFldsEN, objInFor);
        objEditRegionFldsEN.SetmId(objInFor.mId);
        objEditRegionFldsEN.SetColSpan(intColSpan);
        let returnBool = false;
        try {
          objEditRegionFldsEN.sfUpdFldSetStr = objEditRegionFldsEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await EditRegionFlds_UpdateRecordAsync(objEditRegionFldsEN);
  ...
  public async SetWidth(arrmId: Array<string>, intWidth: number) {
    ...
    try {
      ...
      for (const objInFor of arrEditRegionFldsObjLst) {
        ...
        ObjectAssign(objEditRegionFldsEN, objInFor);
        objEditRegionFldsEN.SetmId(objInFor.mId);
        objEditRegionFldsEN.SetWidth(intWidth);
        let returnBool = false;
        try {
          objEditRegionFldsEN.sfUpdFldSetStr = objEditRegionFldsEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await EditRegionFlds_UpdateRecordAsync(objEditRegionFldsEN);
  ...
  public async DelMultiRecord(arrmId: Array<string>) {
    ...
    try {
      const returnInt = await EditRegionFlds_DelEditRegionFldssAsync(arrmId);
      if (returnInt > 0) {
        EditRegionFlds_ReFreshCache(RegionId_Static.value);
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param divContainer:显示容器
   * @param objEditRegionFlds:需要显示的对象
   **/
  public ShowEditRegionFldsObj(
    divContainer: HTMLDivElement,
    ...
    const sstrKeys = GetObjKeys(objEditRegionFlds);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objEditRegionFlds.GetFldValue(strKey);
   ...
   * @param pobjEditRegionFldsEN:表实体类对象
   * @returns 列表的第一个关键字值
   **/
