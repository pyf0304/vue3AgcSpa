/**
 * 类名:ViewRegionCRUD(界面:ViewRegionCRUD,00050242)
 * 表名:ViewRegion(00050099)
 * 版本:2025.05.12.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/15 01:36:11
 * 生成者:
 工程名称:AGC(0005)
...
import {
  PrjId_Session,
  CombineViewRegionConditionObj4ExportExcel,
  CombineViewRegionConditionObj,
  ViewRegion_DeleteKeyIdCache,
  divVarSet,
  viewVarSet,
  dataColumn,
  BindTabByList,
...
import {
  ViewRegion_GetRecCountByCondCache,
  ViewRegion_GetSubObjLstCache,
  ViewRegion_DelRecordAsync,
  ViewRegion_ReFreshCache,
  ViewRegion_GetObjByRegionIdAsync,
  ViewRegion_GetObjExLstByPagerCache,
  ViewRegion_GetObjLstByRegionIdLstAsync,
  ViewRegion_GetMaxStrIdAsync,
  ViewRegion_AddNewRecordAsync,
  ViewRegion_UpdateRecordAsync,
  ViewRegion_DelViewRegionsAsync,
...
/** ViewRegionCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class ViewRegionCRUD implements clsOperateList {
  ...
  public static objPageCRUD: ViewRegionCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    ViewRegionCRUD.objPageCRUD = this;
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortViewRegionBy == '')
        viewVarSet.sortViewRegionBy = `${clsViewRegionEN.con_RegionId} Asc`;
   ...
   **/
  public CombineData(
    arrViewRegionObjLst: Array<clsViewRegionEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrViewRegionObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsViewRegionEN = arrViewRegionObjLst[i];
    ...
    const strFileName = Format('界面区域({0})导出.xlsx', clsViewRegionEN._CurrTabName);
    const strSheetName = '界面区域列表';
  ...
  public CombineData4Func(
    ...
    const intRowNum = arrViewRegionExObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsViewRegionENEx = arrViewRegionExObjLst[i];
      const objRow: Record<string, any> = {};
      for (let j = 0; j < intColNum; j++) {
        const fldName = arrDataColumn[j].fldName;
        const colHeader = arrDataColumn[j].colHeader;
   ...
   **/
  public async ExportExcel_ViewRegion4Func(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_ViewRegion4Func.name;
    if (viewVarSet.sortViewRegionBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortViewRegionBy)为空,请检查!(In BindGv_ViewRegionCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return { arrObjLst: [], sheetName: '', fileName: '' };
    }
    const objViewRegionCond = await CombineViewRegionConditionObj4ExportExcel();
    let arrViewRegionExObjLst: Array<clsViewRegionENEx> = [];
    try {
      this.recCount = await ViewRegion_GetRecCountByCondCache(
        objViewRegionCond,
        PrjId_Session.value,
      );
      if (this.recCount == 0) {
      ...
      const arrViewRegionObjLst = await ViewRegion_GetSubObjLstCache(
        objViewRegionCond,
        PrjId_Session.value,
      );
      arrViewRegionExObjLst = arrViewRegionObjLst.map(ViewRegionEx_CopyToEx);
    } catch (e) {
    ...
    if (arrViewRegionExObjLst.length == 0) {
      const strKey = Format('{0}_{1}', clsViewRegionEN._CurrTabName, PrjId_Session.value);
    ...
    try {
      const arrDataColumn: Array<clsDataColumn> = [
        ...
        {
          fldName: clsViewRegionEN.con_Width,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '宽',
        ...
        {
          fldName: clsViewRegionENEx.con_FldCountInUse,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '可用字段',
        ...
        {
          fldName: clsViewRegionENEx.con_CmPrjName,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: 'CM工程名',
        ...
        {
          fldName: clsViewRegionEN.con_UpdUser,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '修改者',
      ...
      arrViewRegionExObjLst = arrViewRegionExObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrViewRegionExObjLst, arrDataColumn);
      //console.log("完成BindGv_ViewRegion!");
    } catch (e) {
  ...
  public async btnSetTabId_Click() {
    ...
    try {
      ...
      //console.log(arrKeyIds);
      await this.SetTabId(arrKeyIds, strTabId);
      await this.BindGv_ViewRegion4Func(divVarSet.refDivList);
    } catch (e) {
   ...
   **/
  public async btnSelectRecordInTab_Click(strRegionId: string) {
   ...
   **/
  public async DelRecord(strRegionId: string) {
    const strThisFuncName = this.DelRecord.name;
    try {
      const returnInt = await ViewRegion_DelRecordAsync(strRegionId);
      if (returnInt > 0) {
        ViewRegion_ReFreshCache(PrjId_Session.value);
   ...
   * @param sender:参数列表
   **/
  public async SelectRecord(strRegionId: string) {
    const strThisFuncName = this.SelectRecord.name;
    try {
      const objViewRegionEN = await ViewRegion_GetObjByRegionIdAsync(strRegionId);
      console.log('完成SelectRecord!', objViewRegionEN);
      Redirect('/Index/Main_ViewRegion');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_ViewRegion4Func();
  }

  /** 显示ViewRegion对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrViewRegionObjLst:需要绑定的对象列表
   **/
  public async BindTab_ViewRegion(
    ...
    if (refViewRegion_List.value != null) {
      ...
      if (divDataLst == null) {
        alert('在BindTab_ViewRegion函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
   ...
   **/
  public async BindGv_ViewRegion4Func(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_ViewRegion4Func.name;
    if (viewVarSet.sortViewRegionBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortViewRegionBy)为空,请检查!(In BindGv_ViewRegionCache)',
      );
    ...
    const objViewRegionCond = await CombineViewRegionConditionObj();
    objViewRegionCond.SetCondFldValue(clsViewRegionEN.con_PrjId, PrjId_Session.value, '=');
    const strWhereCond = JSON.stringify(objViewRegionCond);
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrViewRegionExObjLst, true);
        return;
      }
      ...
      if (ViewRegionCRUD.sortFunStatic != undefined) {
        strSortFun = ViewRegionCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
      }
      const objPagerPara: stuPagerPara = {
        pageIndex: intCurrPageIndex,
        pageSize: this.pageSize,
        whereCond: strWhereCond,
        conditionCollection: objViewRegionCond,
        orderBy: viewVarSet.sortViewRegionBy,
        sortFun: strSortFun,
      };
      arrViewRegionExObjLst = await ViewRegion_GetObjExLstByPagerCache(
        objPagerPara,
        PrjId_Session.value,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_ViewRegion4Func(divList, arrViewRegionExObjLst);
      //console.log("完成BindGv_ViewRegion4Func!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName})`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsViewRegionEN, b: clsViewRegionEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortViewRegionBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsViewRegionENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortViewRegionBy = sortBy;
    viewVarSet.ascOrDesc4SortFun = ascOrDesc4SortFun;
    ViewRegionCRUD.sortFunStatic = sortFun;
    await this.BindGv_ViewRegion4Func(this.listPara.listDiv);
  }
   ...
   **/
  public async CopyRecord(arrRegionId: Array<string>) {
    ...
    try {
      ...
      for (const objInFor of arrViewRegionObjLst) {
        const strMaxStrId = await ViewRegion_GetMaxStrIdAsync();
        //console.log('strMaxStrId=' + strMaxStrId);
        objInFor.regionId = strMaxStrId;
        const returnBool = await ViewRegion_AddNewRecordAsync(objInFor);
  ...
  public async SetTabId(arrRegionId: Array<string>, strTabId: string) {
    ...
    try {
      ...
      for (const objInFor of arrViewRegionObjLst) {
        ...
        objViewRegionEN.SetRegionId(objInFor.regionId);
        ...
        if (returnBool == true) {
          ViewRegion_DeleteKeyIdCache(PrjId_Session.value, objInFor.regionId);
          intCount++;
        } else {
  ...
  public async DelMultiRecord(arrRegionId: Array<string>) {
    ...
    try {
      const returnInt = await ViewRegion_DelViewRegionsAsync(arrRegionId);
      if (returnInt > 0) {
        ViewRegion_ReFreshCache(PrjId_Session.value);
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param divContainer:显示容器
   * @param objViewRegion:需要显示的对象
   **/
  public ShowViewRegionObj(divContainer: HTMLDivElement, objViewRegion: clsViewRegionEN) {
    if (divContainer == null) {
      alert(Format('所给div为空，divContainer为null!', divContainer));
      return;
    }
    const sstrKeys = GetObjKeys(objViewRegion);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objViewRegion.GetFldValue(strKey);
