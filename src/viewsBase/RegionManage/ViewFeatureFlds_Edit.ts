/**
 * 类名:ViewFeatureFlds_Edit(界面:ViewFeatureFldsCRUD,00050314)
...
import {
  ...
  ViewFeatureFlds_DeleteKeyIdCache,
  divVarSet,
...
/** ViewFeatureFlds_Edit 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScript_EditCS_TS4TypeScript:GeneCode)
 **/
export abstract class ViewFeatureFlds_Edit {
  ...
  public static objPageEdit: ViewFeatureFlds_Edit;
  public static objPageEdit2: ViewFeatureFlds_Edit;
  public static objPageEdit3: ViewFeatureFlds_Edit;
  ...
  constructor(strClassName: string, objShowList: IShowList | null) {
    ...
    if (ViewFeatureFlds_Edit.SetPageEdit(this, 1) == true) return;
    if (ViewFeatureFlds_Edit.SetPageEdit(this, 2) == true) return;
    if (ViewFeatureFlds_Edit.SetPageEdit(this, 3) == true) return;
  }
  public static SetPageEdit(objDataLst: any, intIndex: number): boolean {
    const strNewClassName = objDataLst.className;
    switch (intIndex) {
      case 1:
        if (ViewFeatureFlds_Edit.objPageEdit == null) {
          ViewFeatureFlds_Edit.objPageEdit = objDataLst;
          return true;
        } else {
          const strClassNameOld = ViewFeatureFlds_Edit.objPageEdit.className;
        ...
        break;
      case 2:
        if (ViewFeatureFlds_Edit.objPageEdit2 == null) {
          ViewFeatureFlds_Edit.objPageEdit2 = objDataLst;
          return true;
        } else {
          const strClassNameOld = ViewFeatureFlds_Edit.objPageEdit2.className;
          if (strClassNameOld == strNewClassName) {
            ViewFeatureFlds_Edit.objPageEdit2 = objDataLst;
            return true;
          } else return false;
        ...
        break;
      case 3:
        if (ViewFeatureFlds_Edit.objPageEdit3 == null) {
          ViewFeatureFlds_Edit.objPageEdit3 = objDataLst;
          return true;
        } else {
          const strClassNameOld = ViewFeatureFlds_Edit.objPageEdit3.className;
          if (strClassNameOld == strNewClassName) {
            ViewFeatureFlds_Edit.objPageEdit3 = objDataLst;
            return true;
          } else return false;
  ...
  public static GetPageEditObj(strClassName: string): any {
    if (ViewFeatureFlds_Edit.objPageEdit != null) {
      const strClassNameOld = ViewFeatureFlds_Edit.objPageEdit.className;
      if (strClassNameOld == strClassName) return ViewFeatureFlds_Edit.objPageEdit;
    }
    if (ViewFeatureFlds_Edit.objPageEdit2 != null) {
      const strClassNameOld = ViewFeatureFlds_Edit.objPageEdit2.className;
      if (strClassNameOld == strClassName) return ViewFeatureFlds_Edit.objPageEdit2;
    }
    if (ViewFeatureFlds_Edit.objPageEdit3 != null) {
      const strClassNameOld = ViewFeatureFlds_Edit.objPageEdit3.className;
      if (strClassNameOld == strClassName) return ViewFeatureFlds_Edit.objPageEdit3;
    }
    return null;
  }
   ...
   **/
  public HideDialog_ViewFeatureFlds() {
    if (ViewFeatureFlds_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
   ...
   **/
  public async ShowDialog_ViewFeatureFlds(strOp: string): Promise<boolean> {
    const strThisFuncName = this.ShowDialog_ViewFeatureFlds.name;
    if (ViewFeatureFlds_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (refViewFeatureFlds_Edit.value == null) {
        const strMsg = Format(
      ...
      await refViewFeatureFlds_Edit.value.showDialog(this);
    }
    if (strOp === 'Add' || strOp === 'AddWithMaxId') {
      this.btnSubmitViewFeatureFlds = '确认添加';
      this.btnCancelViewFeatureFlds = '取消添加';
    } else if (strOp === 'Update') {
      this.btnSubmitViewFeatureFlds = '确认修改';
      this.btnCancelViewFeatureFlds = '取消修改';
    }
    return true;
  }
  ...
  public async btnSubmit_Click() {
    ...
    const strCommandText: string = this.btnSubmitViewFeatureFlds;
    try {
      ...
      switch (strCommandText) {
          ...
          if (['02', '03', '06'].indexOf(clsViewFeatureFldsEN.PrimaryTypeId) > -1) {
            ...
            if (returnKeyId != 0) {
              ...
              if (this.iShowList != null)
                this.iShowList.BindGvCache(clsViewFeatureFldsEN._CurrTabName, '');
            }
          } else {
        ...
        case '确认修改':
          ...
          strInfo += '(In ViewFeatureFlds_Edit.btnSubmit_Click)';
          //显示信息框
          //console.log(strInfo);
          alert(strInfo);
          if (returnBool == true) {
            ...
            if (this.iShowList != null)
              this.iShowList.BindGvCache(clsViewFeatureFldsEN._CurrTabName, this.keyId.toString());
          }
          break;
        default:
          strMsg = Format(
  ...
  public async AddNewRecord() {
    ...
    refViewFeatureFlds_Edit.value.Clear();
    //wucViewFeatureFldsB1.mId = ViewFeatureFldsGetMaxStrId_S();
  }
  ...
  public async AddNewRecordWithMaxId() {
    ...
    //this.mId = await ViewFeatureFlds_GetMaxStrIdAsync();
  }
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    let objViewFeatureFldsEN;
    try {
      objViewFeatureFldsEN = await refViewFeatureFlds_Edit.value.GetEditDataViewFeatureFldsObj();
    } catch (e) {
      const strMsg = Format(
        '从界面获取数据不成功,{0}.(in {1}.{2})',
        e,
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Add(objViewFeatureFldsEN);
      ...
      if (returnBool == true) {
        ...
        const strInfo = `添加[界面功能字段(ViewFeatureFlds)]记录成功!`;
        //显示信息框
   ...
   **/
  public async CheckUniCond4Add(objViewFeatureFldsEN: clsViewFeatureFldsEN): Promise<boolean> {
   ...
   **/
  public async CheckUniCond4Update(objViewFeatureFldsEN: clsViewFeatureFldsEN): Promise<boolean> {
  ...
  public async AddNewRecordWithReturnKeySave(): Promise<number> {
    ...
    try {
      ...
      const responseKeyId = await ViewFeatureFlds_AddNewRecordWithReturnKeyAsync(
        objViewFeatureFldsEN,
      );
      const returnKeyId: string = responseKeyId;
      if (IsNullOrEmpty(returnKeyId) == false) {
        objViewFeatureFldsEN.mId = Number(returnKeyId);
        ViewFeatureFlds_ReFreshCache(ViewFeatureId_Static.value);
        const strInfo = `添加[界面功能字段(ViewFeatureFlds)]记录成功!`;
        //显示信息框
        if (this.isShowMsg == true) alert(strInfo);
  ...
  public async ShowData(lngmId: number) {
    ...
    try {
      ...
      objViewFeatureFldsEN = objViewFeatureFldsENConst;
    } catch (e) {
      const strMsg = Format(
    ...
    //3、用提供的关键字初始化一个类对象；
    refViewFeatureFlds_Edit.value.ShowDataFromViewFeatureFldsObj(objViewFeatureFldsEN);
  }
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    const objViewFeatureFldsEN =
      await refViewFeatureFlds_Edit.value.GetEditDataViewFeatureFldsObj();
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Update(objViewFeatureFldsEN);
      if (bolIsExistCond == false) {
        return false;
      }
      ...
      if (returnBool == true) {
        ViewFeatureFlds_ReFreshCache(ViewFeatureId_Static.value);
        ViewFeatureFlds_DeleteKeyIdCache(ViewFeatureId_Static.value, this.keyId);
      }
      return returnBool;
    } catch (e) {
      const strMsg = Format(
  ...
  public async EditRecordExSave(): Promise<boolean> {
    ...
    try {
      ...
      const returnBool = await ViewFeatureFlds_EditRecordExAsync(objViewFeatureFldsEN);
      if (returnBool == true) {
        ViewFeatureFlds_ReFreshCache(ViewFeatureId_Static.value);
      }
      return returnBool;
   ...
   **/
  public set btnCancelViewFeatureFlds(value: string) {
    refViewFeatureFlds_Edit.value.strCancelButtonText = value;
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitViewFeatureFlds(): string {
    const strValue = refViewFeatureFlds_Edit.value.strSubmitButtonText;
    return strValue;
  }
  /**
   * 设置确定按钮的标题(Used In AddNewRecord())
   **/
  public set btnSubmitViewFeatureFlds(value: string) {
    refViewFeatureFlds_Edit.value.strSubmitButtonText = value;
  }
}
