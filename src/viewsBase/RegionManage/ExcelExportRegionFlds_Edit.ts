/**
 * 类名:ExcelExportRegionFlds_Edit(界面:ExcelExportRegionFldsCRUD,00050279)
 * 表名:ExcelExportRegionFlds(00050149)
 * 版本:2025.05.03.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/08 11:50:56
 * 生成者:
 工程名称:AGC(0005)
...
import {
  ...
  ExcelExportRegionFlds_GetUniCondStr,
  ExcelExportRegionFlds_IsExistRecordAsync,
  ExcelExportRegionFlds_GetUniCondStr4Update,
  ExcelExportRegionFlds_AddNewRecordWithReturnKeyAsync,
  ExcelExportRegionFlds_IsExistAsync,
  ExcelExportRegionFlds_GetObjBymIdAsync,
  ExcelExportRegionFlds_CheckProperty4Update,
  ExcelExportRegionFlds_UpdateRecordAsync,
  ExcelExportRegionFlds_EditRecordExAsync,
...
/** ExcelExportRegionFlds_Edit 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScript_EditCS_TS4TypeScript:GeneCode)
 **/
export abstract class ExcelExportRegionFlds_Edit {
  ...
  public static objPageEdit: ExcelExportRegionFlds_Edit;
  public static objPageEdit2: ExcelExportRegionFlds_Edit;
  public static objPageEdit3: ExcelExportRegionFlds_Edit;
  ...
  constructor(strClassName: string, objShowList: IShowList | null) {
    ...
    if (ExcelExportRegionFlds_Edit.SetPageEdit(this, 1) == true) return;
    if (ExcelExportRegionFlds_Edit.SetPageEdit(this, 2) == true) return;
    if (ExcelExportRegionFlds_Edit.SetPageEdit(this, 3) == true) return;
  }
  public static SetPageEdit(objDataLst: any, intIndex: number): boolean {
    const strNewClassName = objDataLst.className;
    switch (intIndex) {
      case 1:
        if (ExcelExportRegionFlds_Edit.objPageEdit == null) {
          ExcelExportRegionFlds_Edit.objPageEdit = objDataLst;
          return true;
        } else {
          const strClassNameOld = ExcelExportRegionFlds_Edit.objPageEdit.className;
        ...
        break;
      case 2:
        if (ExcelExportRegionFlds_Edit.objPageEdit2 == null) {
          ExcelExportRegionFlds_Edit.objPageEdit2 = objDataLst;
          return true;
        } else {
          const strClassNameOld = ExcelExportRegionFlds_Edit.objPageEdit2.className;
          if (strClassNameOld == strNewClassName) {
            ExcelExportRegionFlds_Edit.objPageEdit2 = objDataLst;
            return true;
          } else return false;
        ...
        break;
      case 3:
        if (ExcelExportRegionFlds_Edit.objPageEdit3 == null) {
          ExcelExportRegionFlds_Edit.objPageEdit3 = objDataLst;
          return true;
        } else {
          const strClassNameOld = ExcelExportRegionFlds_Edit.objPageEdit3.className;
          if (strClassNameOld == strNewClassName) {
            ExcelExportRegionFlds_Edit.objPageEdit3 = objDataLst;
            return true;
          } else return false;
  ...
  public static GetPageEditObj(strClassName: string): any {
    if (ExcelExportRegionFlds_Edit.objPageEdit != null) {
      const strClassNameOld = ExcelExportRegionFlds_Edit.objPageEdit.className;
      if (strClassNameOld == strClassName) return ExcelExportRegionFlds_Edit.objPageEdit;
    }
    if (ExcelExportRegionFlds_Edit.objPageEdit2 != null) {
      const strClassNameOld = ExcelExportRegionFlds_Edit.objPageEdit2.className;
      if (strClassNameOld == strClassName) return ExcelExportRegionFlds_Edit.objPageEdit2;
    }
    if (ExcelExportRegionFlds_Edit.objPageEdit3 != null) {
      const strClassNameOld = ExcelExportRegionFlds_Edit.objPageEdit3.className;
      if (strClassNameOld == strClassName) return ExcelExportRegionFlds_Edit.objPageEdit3;
    }
    return null;
  }
  ...
  public HideDialog_ExcelExportRegionFlds() {
    if (ExcelExportRegionFlds_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      refExcelExportRegionFlds_Edit.value.hideDialog();
    }
  }
  ...
  public async ShowDialog_ExcelExportRegionFlds(strOp: string): Promise<boolean> {
    ...
    if (ExcelExportRegionFlds_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (refExcelExportRegionFlds_Edit.value == null) {
        const strMsg = Format(
      ...
      await refExcelExportRegionFlds_Edit.value.showDialog(this);
    }
    if (strOp === 'Add' || strOp === 'AddWithMaxId') {
      this.btnSubmitExcelExportRegionFlds = '确认添加';
      this.btnCancelExcelExportRegionFlds = '取消添加';
    } else if (strOp === 'Update') {
      this.btnSubmitExcelExportRegionFlds = '确认修改';
      this.btnCancelExcelExportRegionFlds = '取消修改';
    }
    return true;
  }
  ...
  public async btnAddNewRecord_Click() {
    ...
    try {
      this.opType = 'Add';
      const bolIsSuccess = await this.ShowDialog_ExcelExportRegionFlds(this.opType);
  ...
  public async btnSubmit_Click() {
    ...
    const strCommandText: string = this.btnSubmitExcelExportRegionFlds;
    try {
          ...
          if (['02', '03', '06'].indexOf(clsExcelExportRegionFldsEN.PrimaryTypeId) > -1) {
            ...
            if (returnKeyId != 0) {
              ...
              if (this.iShowList != null)
                this.iShowList.BindGvCache(clsExcelExportRegionFldsEN._CurrTabName, '');
            }
          } else {
  ...
  public async AddNewRecord() {
    ...
    refExcelExportRegionFlds_Edit.value.Clear();
    //wucExcelExportRegionFldsB1.mId = ExcelExportRegionFldsGetMaxStrId_S();
  }
  ...
  public async AddNewRecordWithMaxId() {
    ...
    //this.mId = await ExcelExportRegionFlds_GetMaxStrIdAsync();
  }
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Add(objExcelExportRegionFldsEN);
      ...
      if (returnBool == true) {
        ...
        const strInfo = `添加[Excel导出区域字段(ExcelExportRegionFlds)]记录成功!`;
        //显示信息框
  ...
  public async AddNewRecordWithReturnKeySave(): Promise<number> {
    ...
    try {
      ...
      const responseKeyId = await ExcelExportRegionFlds_AddNewRecordWithReturnKeyAsync(
        objExcelExportRegionFldsEN,
      );
      const returnKeyId: string = responseKeyId;
      if (IsNullOrEmpty(returnKeyId) == false) {
        objExcelExportRegionFldsEN.mId = Number(returnKeyId);
        ExcelExportRegionFlds_ReFreshCache(RegionId_Static.value);
        const strInfo = `添加[Excel导出区域字段(ExcelExportRegionFlds)]记录成功!`;
        //显示信息框
        if (this.isShowMsg == true) alert(strInfo);
  ...
  public async ShowData(lngmId: number) {
    ...
    //2、检查该关键字的记录是否存在,如果不存在就返回不显示；
    let objExcelExportRegionFldsEN = new clsExcelExportRegionFldsEN();
    try {
      const returnBool = await ExcelExportRegionFlds_IsExistAsync(lngmId);
      if (returnBool == false) {
    ...
    try {
      const objExcelExportRegionFldsENConst = await ExcelExportRegionFlds_GetObjBymIdAsync(lngmId);
      if (objExcelExportRegionFldsENConst == null) {
        const strMsg = Format(
      ...
      objExcelExportRegionFldsEN = objExcelExportRegionFldsENConst;
    } catch (e) {
      const strMsg = Format(
    ...
    //3、用提供的关键字初始化一个类对象；
    refExcelExportRegionFlds_Edit.value.ShowDataFromExcelExportRegionFldsObj(
      objExcelExportRegionFldsEN,
    );
  }
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Update(objExcelExportRegionFldsEN);
      if (bolIsExistCond == false) {
        return false;
      }
      ...
      if (returnBool == true) {
        ExcelExportRegionFlds_ReFreshCache(RegionId_Static.value);
        ExcelExportRegionFlds_DeleteKeyIdCache(RegionId_Static.value, this.keyId);
      }
      return returnBool;
    } catch (e) {
      const strMsg = Format(
  ...
  public async EditRecordExSave(): Promise<boolean> {
    ...
    try {
      ...
      const returnBool = await ExcelExportRegionFlds_EditRecordExAsync(objExcelExportRegionFldsEN);
      if (returnBool == true) {
        ExcelExportRegionFlds_ReFreshCache(RegionId_Static.value);
      }
      return returnBool;
  ...
  public set btnCancelExcelExportRegionFlds(value: string) {
    refExcelExportRegionFlds_Edit.value.strCancelButtonText = value;
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitExcelExportRegionFlds(): string {
    const strValue = refExcelExportRegionFlds_Edit.value.strSubmitButtonText;
    return strValue;
  }
  /**
   * 设置确定按钮的标题(Used In AddNewRecord())
   **/
  public set btnSubmitExcelExportRegionFlds(value: string) {
    refExcelExportRegionFlds_Edit.value.strSubmitButtonText = value;
  }
}
