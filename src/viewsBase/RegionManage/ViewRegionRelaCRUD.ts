/**
 * 类名:ViewRegionRelaCRUD(界面:ViewRegionRelaCRUD,00050322)
 * 表名:ViewRegionRela(00050573)
 * 版本:2025.05.12.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/15 01:36:12
 * 生成者:
 工程名称:AGC(0005)
...
import {
  CmPrjId_Local,
  CombineViewRegionRelaConditionObj4ExportExcel,
  CombineViewRegionRelaConditionObj,
  divVarSet,
  viewVarSet,
  dataColumn,
  BindTabByList,
...
import {
  ...
  ViewRegionRela_GetSubObjLstCache,
  ViewRegionRela_DelRecordAsync,
  ViewRegionRela_ReFreshCache,
  ViewRegionRela_GetObjBymIdAsync,
  ViewRegionRela_GetObjExLstByPagerCache,
  ViewRegionRela_GetObjLstBymIdLstAsync,
  ViewRegionRela_AddNewRecordAsync,
  ViewRegionRela_DelViewRegionRelasAsync,
...
/** ViewRegionRelaCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class ViewRegionRelaCRUD implements clsOperateList {
  ...
  public static objPageCRUD: ViewRegionRelaCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    ViewRegionRelaCRUD.objPageCRUD = this;
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortViewRegionRelaBy == '')
        viewVarSet.sortViewRegionRelaBy = `${clsViewRegionRelaEN.con_RegionId} Asc`;
   ...
   **/
  public CombineData(
    arrViewRegionRelaObjLst: Array<clsViewRegionRelaEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrViewRegionRelaObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsViewRegionRelaEN = arrViewRegionRelaObjLst[i];
    ...
    const strFileName = Format('界面区域关系({0})导出.xlsx', clsViewRegionRelaEN._CurrTabName);
    const strSheetName = '界面区域关系列表';
  ...
  public CombineData4Func(
    ...
    const intRowNum = arrViewRegionRelaExObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsViewRegionRelaENEx = arrViewRegionRelaExObjLst[i];
      const objRow: Record<string, any> = {};
      for (let j = 0; j < intColNum; j++) {
        const fldName = arrDataColumn[j].fldName;
        const colHeader = arrDataColumn[j].colHeader;
   ...
   **/
  public async ExportExcel_ViewRegionRela4Func(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_ViewRegionRela4Func.name;
    if (viewVarSet.sortViewRegionRelaBy == null) {
      const strMsg = Format(
    ...
    const objViewRegionRelaCond = await CombineViewRegionRelaConditionObj4ExportExcel();
    ...
    try {
      ...
      const arrViewRegionRelaObjLst = await ViewRegionRela_GetSubObjLstCache(
        objViewRegionRelaCond,
        CmPrjId_Local.value,
      );
      arrViewRegionRelaExObjLst = arrViewRegionRelaObjLst.map(ViewRegionRelaEx_CopyToEx);
    } catch (e) {
    ...
    try {
      const arrDataColumn: Array<clsDataColumn> = [
        ...
        {
          fldName: clsViewRegionRelaENEx.con_RegionTypeSimName,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '区域类型简名',
        ...
        {
          fldName: clsViewRegionRelaEN.con_InUse,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '是否在用',
        ...
        {
          fldName: clsViewRegionRelaEN.con_IsDisp,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '是否显示',
      ...
      arrViewRegionRelaExObjLst = arrViewRegionRelaExObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrViewRegionRelaExObjLst, arrDataColumn);
      //console.log("完成BindGv_ViewRegionRela!");
    } catch (e) {
  ...
  public async DelRecord(lngmId: number) {
    ...
    try {
      const returnInt = await ViewRegionRela_DelRecordAsync(lngmId);
      if (returnInt > 0) {
  ...
  public async SelectRecord(lngmId: number) {
    ...
    try {
      ...
      console.log('完成SelectRecord!', objViewRegionRelaEN);
      Redirect('/Index/Main_ViewRegionRela');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_ViewRegionRela4Func();
  }

  /** 显示ViewRegionRela对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrViewRegionRelaObjLst:需要绑定的对象列表
   **/
  public async BindTab_ViewRegionRela(
    ...
    if (refViewRegionRela_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrViewRegionRelaExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_ViewRegionRela函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async BindTab_ViewRegionRela4Func(
    ...
    if (refViewRegionRela_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_ViewRegionRela4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async BindGv_ViewRegionRela4Func(divList: HTMLDivElement) {
    ...
    const objViewRegionRelaCond = await CombineViewRegionRelaConditionObj();
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrViewRegionRelaExObjLst, true);
        return;
      }
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortViewRegionRelaBy,
        sortFun: strSortFun,
      };
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsViewRegionRelaEN, b: clsViewRegionRelaEN): number {
    if (a.mId == b.mId) {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortViewRegionRelaBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsViewRegionRelaENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortViewRegionRelaBy = sortBy;
    viewVarSet.ascOrDesc4SortFun = ascOrDesc4SortFun;
    ViewRegionRelaCRUD.sortFunStatic = sortFun;
    await this.BindGv_ViewRegionRela4Func(this.listPara.listDiv);
  }
  ...
  public async CopyRecord(arrmId: Array<string>) {
    ...
    try {
      const arrViewRegionRelaObjLst = await ViewRegionRela_GetObjLstBymIdLstAsync(arrmId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrViewRegionRelaObjLst) {
        const returnBool = await ViewRegionRela_AddNewRecordAsync(objInFor);
  ...
  public async DelMultiRecord(arrmId: Array<string>) {
    ...
    try {
      const returnInt = await ViewRegionRela_DelViewRegionRelasAsync(arrmId);
      if (returnInt > 0) {
        ViewRegionRela_ReFreshCache(CmPrjId_Local.value);
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param divContainer:显示容器
   * @param objViewRegionRela:需要显示的对象
   **/
  public ShowViewRegionRelaObj(
    divContainer: HTMLDivElement,
    ...
    const sstrKeys = GetObjKeys(objViewRegionRela);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objViewRegionRela.GetFldValue(strKey);
   ...
   * @param pobjViewRegionRelaEN:表实体类对象
   * @returns 列表的第一个关键字值
   **/
