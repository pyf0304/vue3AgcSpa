/**
 * 类名:DGRegionFldsCRUD(界面:DGRegionFldsCRUD,00050249)
 * 表名:DGRegionFlds(00050113)
 * 版本:2025.05.12.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/15 01:36:17
 * 生成者:
 工程名称:AGC(0005)
...
import {
  RegionId_Static,
  CombineDGRegionFldsConditionObj4ExportExcel,
  CombineDGRegionFldsConditionObj,
...
import {
  ...
  DGRegionFlds_GetSubObjLstCache,
  DGRegionFlds_ReOrderAsync,
  DGRegionFlds_ReFreshCache,
  DGRegionFlds_GoBottomAsync,
  DGRegionFlds_DownMoveAsync,
  DGRegionFlds_UpMoveAsync,
  DGRegionFlds_GoTopAsync,
  DGRegionFlds_DelRecordAsync,
  DGRegionFlds_GetObjBymIdAsync,
  DGRegionFlds_GetObjExLstByPagerCache,
  DGRegionFlds_GetObjLstBymIdLstAsync,
  DGRegionFlds_AddNewRecordAsync,
  DGRegionFlds_UpdateRecordAsync,
  DGRegionFlds_DelDGRegionFldssAsync,
...
import { DGRegionFldsEx_FuncMapByFldName } from '@/ts/L3ForWApiEx/RegionManage/clsDGRegionFldsExWApi';
...
/** DGRegionFldsCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class DGRegionFldsCRUD implements clsOperateList {
  ...
  public static objPageCRUD: DGRegionFldsCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    DGRegionFldsCRUD.objPageCRUD = this;
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortDGRegionFldsBy == '')
        viewVarSet.sortDGRegionFldsBy = `${clsDGRegionFldsEN.con_IsNeedSort} Asc`;
   ...
   **/
  public CombineData(
    arrDGRegionFldsObjLst: Array<clsDGRegionFldsEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrDGRegionFldsObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsDGRegionFldsEN = arrDGRegionFldsObjLst[i];
    ...
    const strFileName = Format('DG区域字段({0})导出.xlsx', clsDGRegionFldsEN._CurrTabName);
    const strSheetName = 'DG区域字段列表';
   ...
   **/
  public async ExportExcel_DGRegionFldsCache(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_DGRegionFldsCache.name;
    if (viewVarSet.sortDGRegionFldsBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortDGRegionFldsBy)为空,请检查!(In BindGv_DGRegionFldsCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return { arrObjLst: [], sheetName: '', fileName: '' };
    }
    const objDGRegionFldsCond = await CombineDGRegionFldsConditionObj4ExportExcel();
    objDGRegionFldsCond.SetCondFldValue(clsDGRegionFldsEN.con_RegionId, RegionId_Static.value, '=');
    let arrDGRegionFldsObjLst: Array<clsDGRegionFldsEN> = [];
    try {
      ...
      arrDGRegionFldsObjLst = await DGRegionFlds_GetSubObjLstCache(
        objDGRegionFldsCond,
        RegionId_Static.value,
      );
    } catch (e) {
    ...
    if (arrDGRegionFldsObjLst.length == 0) {
      const strKey = Format('{0}_{1}', clsDGRegionFldsEN._CurrTabName, RegionId_Static.value);
    ...
    try {
      ...
      arrDGRegionFldsObjLst = arrDGRegionFldsObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrDGRegionFldsObjLst, arrDataColumn);
      //console.log("完成BindGv_DGRegionFlds!");
    } catch (e) {
  ...
  public async btnReOrder_Click() {
    ...
    try {
      ...
      await DGRegionFlds_ReOrderAsync(objOrderByData);
  ...
  public async btnGoBottum_Click() {
    ...
    try {
      ...
      const jsonStr = JSON.stringify(jsonObject);
      objOrderByData.ClassificationFieldValueLst = jsonStr;
      await DGRegionFlds_GoBottomAsync(objOrderByData);
      DGRegionFlds_ReFreshCache(RegionId_Static.value);
    } catch (e) {
  ...
  public async btnDownMove_Click() {
    ...
    try {
      ...
      const jsonStr = JSON.stringify(jsonObject);
      objOrderByData.ClassificationFieldValueLst = jsonStr;
      await DGRegionFlds_DownMoveAsync(objOrderByData);
      DGRegionFlds_ReFreshCache(RegionId_Static.value);
    } catch (e) {
  ...
  public async btnUpMove_Click() {
    ...
    try {
      ...
      const jsonStr = JSON.stringify(jsonObject);
      objOrderByData.ClassificationFieldValueLst = jsonStr;
      await DGRegionFlds_UpMoveAsync(objOrderByData);
      DGRegionFlds_ReFreshCache(RegionId_Static.value);
    } catch (e) {
  ...
  public async btnGoTop_Click() {
    ...
    try {
      ...
      const jsonStr = JSON.stringify(jsonObject);
      objOrderByData.ClassificationFieldValueLst = jsonStr;
      await DGRegionFlds_GoTopAsync(objOrderByData);
      DGRegionFlds_ReFreshCache(RegionId_Static.value);
    } catch (e) {
  ...
  public async DelRecord(lngmId: number) {
    ...
    try {
      const returnInt = await DGRegionFlds_DelRecordAsync(lngmId);
      if (returnInt > 0) {
        DGRegionFlds_ReFreshCache(RegionId_Static.value);
  ...
  public async SelectRecord(lngmId: number) {
    ...
    try {
      ...
      console.log('完成SelectRecord!', objDGRegionFldsEN);
      Redirect('/Index/Main_DGRegionFlds');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_DGRegionFldsCache();
  }

  /** 显示DGRegionFlds对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrDGRegionFldsObjLst:需要绑定的对象列表
   **/
  public async BindTab_DGRegionFlds(
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      {
        fldName: clsDGRegionFldsEN.con_SeqNum,
        sortBy: clsDGRegionFldsEN.con_SeqNum,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '字段序号',
        text: '',
      ...
      {
        fldName: clsDGRegionFldsENEx.con_FldName,
        sortBy: 'fldName',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '字段名',
      ...
      {
        fldName: clsDGRegionFldsEN.con_RegionId,
        sortBy: 'tabName',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '源表',
      ...
      {
        fldName: clsDGRegionFldsENEx.con_CtlTypeName,
        sortBy: 'ctlTypeName',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '控件类型名',
      ...
      {
        fldName: clsDGRegionFldsEN.con_IsNeedSort,
        sortBy: clsDGRegionFldsEN.con_IsNeedSort,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '排序?',
        text: '',
      ...
      {
        fldName: clsDGRegionFldsEN.con_IsTransToChkBox,
        sortBy: clsDGRegionFldsEN.con_IsTransToChkBox,
        sortFun: SortFun,
        getDataSource: '',
      ...
      {
        fldName: clsDGRegionFldsEN.con_SortExpression,
        sortBy: clsDGRegionFldsEN.con_SortExpression,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '排序表达式',
        text: '',
    ...
    if (refDGRegionFlds_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrDGRegionFldsExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_DGRegionFlds函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
        arrDGRegionFldsExObjLst,
        arrDataColumn,
  ...
  public async BindTab_DGRegionFlds4Func(
    ...
    if (refDGRegionFlds_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_DGRegionFlds4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
   ...
   * @param arrDGRegionFldsExObjLst:需要映射的对象列表
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_DGRegionFlds4Func(this.listPara.listDiv);
  }
  ...
  public async BindGv_DGRegionFlds4Func(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_DGRegionFlds4Func.name;
    if (viewVarSet.sortDGRegionFldsBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortDGRegionFldsBy)为空,请检查!(In BindGv_DGRegionFldsCache)',
      );
    ...
    const objDGRegionFldsCond = await CombineDGRegionFldsConditionObj();
    objDGRegionFldsCond.SetCondFldValue(clsDGRegionFldsEN.con_RegionId, RegionId_Static.value, '=');
    const strWhereCond = JSON.stringify(objDGRegionFldsCond);
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrDGRegionFldsExObjLst, true);
        return;
      }
      ...
      if (DGRegionFldsCRUD.sortFunStatic != undefined) {
        strSortFun = DGRegionFldsCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
      }
      const objPagerPara: stuPagerPara = {
        pageIndex: intCurrPageIndex,
        pageSize: this.pageSize,
        whereCond: strWhereCond,
        conditionCollection: objDGRegionFldsCond,
    ...
    try {
      ...
      //console.log("完成BindGv_DGRegionFlds4Func!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName})`;
      console.error(strMsg);
      alert(strMsg);
   ...
   **/
  public CopyToEx(objDGRegionFldsENS: clsDGRegionFldsEN): clsDGRegionFldsENEx {
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsDGRegionFldsEN, b: clsDGRegionFldsEN): number {
    if (a.prjId == b.prjId) {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortDGRegionFldsBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsDGRegionFldsENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortDGRegionFldsBy = sortBy;
    viewVarSet.ascOrDesc4SortFun = ascOrDesc4SortFun;
    DGRegionFldsCRUD.sortFunStatic = sortFun;
    await this.BindGv_DGRegionFlds4Func(this.listPara.listDiv);
  }
  ...
  public async CopyRecord(arrmId: Array<string>) {
    ...
    try {
      const arrDGRegionFldsObjLst = await DGRegionFlds_GetObjLstBymIdLstAsync(arrmId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrDGRegionFldsObjLst) {
        const returnBool = await DGRegionFlds_AddNewRecordAsync(objInFor);
  ...
  public async SetInUse(arrmId: Array<string>) {
    ...
    try {
      ...
      let intCount = 0;
      for (const objInFor of arrDGRegionFldsObjLst) {
        const objDGRegionFldsEN = new clsDGRegionFldsEN();
        ObjectAssign(objDGRegionFldsEN, objInFor);
        objDGRegionFldsEN.SetmId(objInFor.mId);
        objDGRegionFldsEN.SetInUse(TransToBool('true'));
        let returnBool = false;
        try {
          objDGRegionFldsEN.sfUpdFldSetStr = objDGRegionFldsEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await DGRegionFlds_UpdateRecordAsync(objDGRegionFldsEN);
        } catch (e) {
          const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
          console.error(strMsg);
          throw strMsg;
        ...
        if (returnBool == true) {
          DGRegionFlds_DeleteKeyIdCache(RegionId_Static.value, objInFor.mId);
          intCount++;
        } else {
  ...
  public async SetNotInUse(arrmId: Array<string>) {
    ...
    try {
      ...
      for (const objInFor of arrDGRegionFldsObjLst) {
        ...
        ObjectAssign(objDGRegionFldsEN, objInFor);
        objDGRegionFldsEN.SetmId(objInFor.mId);
        objDGRegionFldsEN.SetInUse(TransToBool('false'));
        let returnBool = false;
        try {
          objDGRegionFldsEN.sfUpdFldSetStr = objDGRegionFldsEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await DGRegionFlds_UpdateRecordAsync(objDGRegionFldsEN);
  ...
  public async DelMultiRecord(arrmId: Array<string>) {
    ...
    try {
      const returnInt = await DGRegionFlds_DelDGRegionFldssAsync(arrmId);
      if (returnInt > 0) {
        DGRegionFlds_ReFreshCache(RegionId_Static.value);
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param divContainer:显示容器
   * @param objDGRegionFlds:需要显示的对象
   **/
  public ShowDGRegionFldsObj(divContainer: HTMLDivElement, objDGRegionFlds: clsDGRegionFldsEN) {
    if (divContainer == null) {
      alert(Format('所给div为空，divContainer为null!', divContainer));
      return;
    }
    const sstrKeys = GetObjKeys(objDGRegionFlds);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objDGRegionFlds.GetFldValue(strKey);
   ...
   * @param pobjDGRegionFldsEN:表实体类对象
   * @returns 列表的第一个关键字值
   **/
