/**
 * 类名:CMProjectCRUD(界面:CMProjectCRUD,00050318)
 * 表名:CMProject(00050512)
 * 版本:2025.05.12.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/15 01:34:36
 * 生成者:
 工程名称:AGC(0005)
...
import {
  CombineCMProjectConditionObj4ExportExcel,
  CombineCMProjectConditionObj,
...
import {
  CMProject_GetRecCountByCondCache,
...
/** CMProjectCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class CMProjectCRUD implements clsOperateList {
  ...
  public static objPageCRUD: CMProjectCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    CMProjectCRUD.objPageCRUD = this;
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortCMProjectBy == '')
        viewVarSet.sortCMProjectBy = `${clsCMProjectEN.con_CmPrjId} Asc`;
   ...
   **/
  public CombineData(
    arrCMProjectObjLst: Array<clsCMProjectEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrCMProjectObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsCMProjectEN = arrCMProjectObjLst[i];
    ...
    const strFileName = Format('CM项目({0})导出.xlsx', clsCMProjectEN._CurrTabName);
    const strSheetName = 'CM项目列表';
  ...
  public CombineData4Func(
    ...
    const intRowNum = arrCMProjectExObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsCMProjectENEx = arrCMProjectExObjLst[i];
      const objRow: Record<string, any> = {};
      for (let j = 0; j < intColNum; j++) {
        const fldName = arrDataColumn[j].fldName;
        const colHeader = arrDataColumn[j].colHeader;
   ...
   **/
  public async ExportExcel_CMProject4Func(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_CMProject4Func.name;
    if (viewVarSet.sortCMProjectBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortCMProjectBy)为空,请检查!(In BindGv_CMProjectCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return { arrObjLst: [], sheetName: '', fileName: '' };
    }
    const objCMProjectCond = await CombineCMProjectConditionObj4ExportExcel();
    let arrCMProjectExObjLst: Array<clsCMProjectENEx> = [];
    try {
      this.recCount = await CMProject_GetRecCountByCondCache(objCMProjectCond);
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objCMProjectCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      const arrCMProjectObjLst = await CMProject_GetSubObjLstCache(objCMProjectCond);
      arrCMProjectExObjLst = arrCMProjectObjLst.map(CMProjectEx_CopyToEx);
    } catch (e) {
    ...
    if (arrCMProjectExObjLst.length == 0) {
      const strKey = Format('{0}', clsCMProjectEN._CurrTabName);
    ...
    try {
      const arrDataColumn: Array<clsDataColumn> = [
        {
          fldName: clsCMProjectEN.con_CmPrjId,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: 'CM工程Id',
        ...
        {
          fldName: clsCMProjectEN.con_CmPrjName,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: 'CM工程名',
        ...
        {
          fldName: clsCMProjectENEx.con_PrjName,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '工程名称',
        ...
        {
          fldName: clsCMProjectENEx.con_ApplicationTypeSimName,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '应用类型',
        ...
        {
          fldName: clsCMProjectEN.con_IsFstLcase,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '是否首字母小写',
        ...
        {
          fldName: clsCMProjectEN.con_ProjectFileName,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '工程文件名',
        ...
        {
          fldName: clsCMProjectEN.con_IsRefresh4RelaView,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '是否刷新相关视图',
        ...
        {
          fldName: clsCMProjectENEx.con_UseStateName,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '使用状态',
        ...
        {
          fldName: clsCMProjectEN.con_UpdDate,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '修改日期',
        ...
        {
          fldName: clsCMProjectEN.con_Memo,
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '说明',
      ...
      arrCMProjectExObjLst = arrCMProjectExObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrCMProjectExObjLst, arrDataColumn);
      //console.log("完成BindGv_CMProject!");
    } catch (e) {
   ...
   **/
  public async btnSelectRecordInTab_Click(strCmPrjId: string) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_CMProject4Func();
  }

  /** 显示CMProject对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrCMProjectObjLst:需要绑定的对象列表
   **/
  public async BindTab_CMProject(
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      {
        fldName: clsCMProjectEN.con_CmPrjId,
        sortBy: clsCMProjectEN.con_CmPrjId,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: 'CM工程Id',
        text: '',
      ...
      {
        fldName: clsCMProjectEN.con_CmPrjName,
        sortBy: clsCMProjectEN.con_CmPrjName,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: 'CM工程名',
        text: '',
      ...
      {
        fldName: clsCMProjectEN.con_IsFstLcase,
        sortBy: clsCMProjectEN.con_IsFstLcase,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '首字母小写?',
        text: '',
      ...
      {
        fldName: clsCMProjectEN.con_ProjectFileName,
        sortBy: clsCMProjectEN.con_ProjectFileName,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '工程文件名',
        text: '',
      ...
      {
        fldName: clsCMProjectEN.con_IsRefresh4RelaView,
        sortBy: clsCMProjectEN.con_IsRefresh4RelaView,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '是否刷新相关视图',
        text: '',
      ...
      {
        fldName: clsCMProjectEN.con_UpdDate,
        sortBy: clsCMProjectEN.con_UpdDate,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '修改日期',
        text: '',
      ...
      {
        fldName: clsCMProjectEN.con_Memo,
        sortBy: clsCMProjectEN.con_Memo,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '说明',
        text: '',
    ...
    if (refCMProject_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrCMProjectExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_CMProject函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
        arrCMProjectExObjLst,
        arrDataColumn,
  ...
  public async BindTab_CMProject4Func(
    ...
    if (refCMProject_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_CMProject4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_CMProject4Func(this.listPara.listDiv);
  }
   ...
   **/
  public async BindGv_CMProject4Func(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_CMProject4Func.name;
    if (viewVarSet.sortCMProjectBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortCMProjectBy)为空,请检查!(In BindGv_CMProjectCache)',
      );
    ...
    const objCMProjectCond = await CombineCMProjectConditionObj();
    const strWhereCond = JSON.stringify(objCMProjectCond);
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        lblMsg.innerHTML = Format('根据条件:[{0}]获取的对象列表数为0!', objCMProjectCond.whereCond);
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objCMProjectCond.whereCond,
        );
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrCMProjectExObjLst, true);
        return;
      }
      ...
      if (CMProjectCRUD.sortFunStatic != undefined) {
        strSortFun = CMProjectCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
      }
      const objPagerPara: stuPagerPara = {
        pageIndex: intCurrPageIndex,
        pageSize: this.pageSize,
        whereCond: strWhereCond,
        conditionCollection: objCMProjectCond,
        orderBy: viewVarSet.sortCMProjectBy,
        sortFun: strSortFun,
      };
      arrCMProjectExObjLst = await CMProject_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
    ...
    try {
      await this.BindTab_CMProject4Func(divList, arrCMProjectExObjLst);
      //console.log("完成BindGv_CMProject4Func!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName})`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsCMProjectEN, b: clsCMProjectEN): number {
    if (a.memo == b.memo) {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortCMProjectBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsCMProjectENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortCMProjectBy = sortBy;
    viewVarSet.ascOrDesc4SortFun = ascOrDesc4SortFun;
    CMProjectCRUD.sortFunStatic = sortFun;
    await this.BindGv_CMProject4Func(this.listPara.listDiv);
  }
   ...
   **/
  public async CopyRecord(arrCmPrjId: Array<string>) {
    const strThisFuncName = this.CopyRecord.name;
    try {
      const arrCMProjectObjLst = await CMProject_GetObjLstByCmPrjIdLstAsync(arrCmPrjId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrCMProjectObjLst) {
        const strMaxStrId = await CMProject_GetMaxStrIdAsync();
        //console.log('strMaxStrId=' + strMaxStrId);
        objInFor.cmPrjId = strMaxStrId;
        const returnBool = await CMProject_AddNewRecordAsync(objInFor);
   ...
   **/
  public async SetApplicationTypeId(arrCmPrjId: Array<string>, intApplicationTypeId: number) {
    ...
    try {
      ...
      for (const objInFor of arrCMProjectObjLst) {
        ...
        ObjectAssign(objCMProjectEN, objInFor);
        objCMProjectEN.SetCmPrjId(objInFor.cmPrjId);
        objCMProjectEN.SetApplicationTypeId(intApplicationTypeId);
        let returnBool = false;
        try {
          objCMProjectEN.sfUpdFldSetStr = objCMProjectEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await CMProject_UpdateRecordAsync(objCMProjectEN);
        } catch (e) {
          const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
          console.error(strMsg);
          throw strMsg;
        ...
        if (returnBool == true) {
          CMProject_DeleteKeyIdCache(objInFor.cmPrjId);
          intCount++;
        } else {
   ...
   * @param divContainer:显示容器
   * @param objCMProject:需要显示的对象
   **/
  public ShowCMProjectObj(divContainer: HTMLDivElement, objCMProject: clsCMProjectEN) {
    if (divContainer == null) {
      alert(Format('所给div为空，divContainer为null!', divContainer));
      return;
    }
    const sstrKeys = GetObjKeys(objCMProject);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objCMProject.GetFldValue(strKey);
