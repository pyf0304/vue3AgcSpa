 ...
 /**
 * 类名:CMProjectAppRela_DetailAi(界面:CMProjectAppRelaCRUD,00050340)
 * 表名:CMProjectAppRela(00050600)
 * 版本:2026.05.30(服务器:PYF-AI)
 * 日期:2026/06/02 08:48:22
 * 生成者:
 工程名称:AGC(0005)
...
import { divVarSet,refCMProjectAppRela_DetailAi } from "@/views/CodeMan/CMProjectAppRelaVueShare";
import { CMProjectAppRelaKey } from "@/ts/L0Entity/CodeMan/clsCMProjectAppRelaEN";
import { IShowList } from "@/ts/PubFun/IShowList";
import { enumPageDispMode } from "@/ts/PubFun/enumPageDispMode";
 /* CMProjectAppRela_DetailAi 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_DetailAi_TS4TypeScript:GeneCode)
*/
export abstract class  CMProjectAppRela_DetailAi 
{
...
if (CMProjectAppRela_DetailAi.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
 refCMProjectAppRela_DetailAi.value.hideDialog();
}
}
...
if (CMProjectAppRela_DetailAi.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
if (refCMProjectAppRela_DetailAi.value == null)
{
const strMsg = Format(
...
await refCMProjectAppRela_DetailAi.value.showDialog();
}
divVarSet.refDivDetail = refCMProjectAppRela_DetailAi.value.$refs.refDivDetail;
if (divVarSet.refDivDetail == null)
{
if (CMProjectAppRela_DetailAi.times4TestShowDialog < 2)
{
CMProjectAppRela_DetailAi.times4TestShowDialog++;
setTimeout(() => {
...
CMProjectAppRela_DetailAi.times4TestShowDialog = 0;
}
       if (strOp === "Detail" ) {
...
*/
public async btnDetailRecordInTab_Click(key: CMProjectAppRelaKey) {
...
*/
public async DetailRecord(key: CMProjectAppRelaKey):Promise<boolean> 
{
...
if (refCMProjectAppRela_DetailAi.value == null)
{
const strMsg = Format('当前详细信息区的DetailObj为空，请检查！(in {0}.{1}).\nrefCMProjectAppRela_DetailAi imported from: @/views/CodeMan/CMProjectAppRelaVueShare',
this.constructor.name,
strThisFuncName,
);
console.error(strMsg);
...
await refCMProjectAppRela_DetailAi.value.ShowDataFromCMProjectAppRelaObj(objCMProjectAppRelaEN);
...
*/
public async btnDetailRecord_Click(key: CMProjectAppRelaKey) {
this.opType = "Detail";
const bolIsSuccess = await this.ShowDialog_CMProjectAppRela('Detail');
if (bolIsSuccess == false) return;
if (key.cMProjectAppRelaId == 0)
...
refCMProjectAppRela_DetailAi.value.strCancelButtonText = value;
}
}