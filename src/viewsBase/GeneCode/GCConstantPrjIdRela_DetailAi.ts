/**
 * 类名:GCConstantPrjIdRela_DetailAi(界面:GCConstantPrjIdRelaCRUD,00050344)
 * 表名:GCConstantPrjIdRela(00050641)
 * 版本:2026.04.19(服务器:PYF-AI)
 * 日期:2026/05/29 17:37:57
 * 生成者:
 工程名称:AGC(0005)
...
/* GCConstantPrjIdRela_DetailAi 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_DetailAi_TS4TypeScript:GeneCode)
*/
export abstract class GCConstantPrjIdRela_DetailAi {
  ...
  public HideDialog_GCConstantPrjIdRela() {
    if (GCConstantPrjIdRela_DetailAi.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      refGCConstantPrjIdRela_DetailAi.value.hideDialog();
    }
  }
  ...
  public async ShowDialog_GCConstantPrjIdRela(strOp: string): Promise<boolean> {
    ...
    if (GCConstantPrjIdRela_DetailAi.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (refGCConstantPrjIdRela_DetailAi.value == null) {
        const strMsg = Format(
      ...
      await refGCConstantPrjIdRela_DetailAi.value.showDialog();
    }
    divVarSet.refDivDetail = refGCConstantPrjIdRela_DetailAi.value.$refs.refDivDetail;
    if (divVarSet.refDivDetail == null) {
      if (GCConstantPrjIdRela_DetailAi.times4TestShowDialog < 2) {
        GCConstantPrjIdRela_DetailAi.times4TestShowDialog++;
        setTimeout(() => {
      ...
      GCConstantPrjIdRela_DetailAi.times4TestShowDialog = 0;
    }
    if (strOp === 'Detail') {
...
*/
  public async DetailRecord4Func(key: GCConstantPrjIdRelaKey) {
    ...
    try {
      ...
      if (refGCConstantPrjIdRela_DetailAi.value == null) {
        const strMsg = Format(
          '当前详细信息区的DetailObj为空，请检查！(in {0}.{1}).\nrefGCConstantPrjIdRela_DetailAi imported from: @/views/GeneCode/GCConstantPrjIdRelaVueShare',
          this.constructor.name,
          strThisFuncName,
        );
        console.error(strMsg);
      ...
      await refGCConstantPrjIdRela_DetailAi.value.ShowDataFromGCConstantPrjIdRelaObj(
        objGCConstantPrjIdRelaENEx,
      );
...
*/
  ...
  public set btnCancelGCConstantPrjIdRela(value: string) {
    refGCConstantPrjIdRela_DetailAi.value.strCancelButtonText = value;
  }
}
