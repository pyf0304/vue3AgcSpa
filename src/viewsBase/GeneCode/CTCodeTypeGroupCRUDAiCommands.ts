...
/** Ai 场景下可用的命令标识。 */
export type CTCodeTypeGroupCommandIdAi =
  | 'query'
  | 'create'
  | 'update'
  | 'delete'
...
export type CTCodeTypeGroupCommandSpecAi = TableCommandSpec<CTCodeTypeGroupCommandIdAi> & {
  /** 辅助控件ID（如下拉框ID） */
  auxControlId?: string;
  /** 辅助控件类型 */
...
export const C_T_CODE_TYPE_GROUP_COMMAND_SPECS_AI: Array<CTCodeTypeGroupCommandSpecAi> = [
  {
    id: 'query',
    region: 'query',
    text: '查询',
...
/** 获取查询区命令。 */
export function getQueryCommandsAi(): Array<CTCodeTypeGroupCommandSpecAi> {
  return C_T_CODE_TYPE_GROUP_COMMAND_SPECS_AI.filter(
    (x) => x.region === 'query' && x.visible !== false,
  );
}

/** 获取功能区命令。 */
export function getFeatureCommandsAi(): Array<CTCodeTypeGroupCommandSpecAi> {
  return C_T_CODE_TYPE_GROUP_COMMAND_SPECS_AI.filter(
