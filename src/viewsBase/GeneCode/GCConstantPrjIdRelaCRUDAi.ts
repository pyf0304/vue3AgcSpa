...
import {
  GCConstantPrjIdRelaEx_FuncMapByFldName,
...
import {
  ...
  getListColumnsAi,
} from '@/viewsBase/GeneCode/GCConstantPrjIdRelaCRUDAiColumns';

/**
 * Ai版基类：
 * 在Ai3基础上增加命令schema驱动，不改变原有基类行为。
 */
export abstract class GCConstantPrjIdRelaCRUDAi
  ...
  implements clsOperateList
{
  ...
  public async DelMultiRecord(arrConstId: Array<string>) {
    ...
    try {
      const returnInt = await GCConstantPrjIdRela_DelRecKeyLstsAsync(arrConstId);
      if (returnInt > 0) {
  ...
  /** 按当前查询条件导出原始列表数据。 */
  public async ExportExcel_GCConstantPrjIdRela(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_GCConstantPrjIdRela.name;
    ...
    let arrGCConstantPrjIdRelaObjLst: Array<clsGCConstantPrjIdRelaEN> = [];
    try {
      ...
      arrGCConstantPrjIdRelaObjLst = await GCConstantPrjIdRela_GetObjLstAsync(strWhereCond);
    } catch (e) {
    ...
    if (arrGCConstantPrjIdRelaObjLst.length == 0) {
    ...
    try {
      ...
      arrGCConstantPrjIdRelaObjLst = arrGCConstantPrjIdRelaObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrGCConstantPrjIdRelaObjLst, arrDataColumn);
    } catch (e) {
  ...
  /** 按导出列规格整理导出结果。 */
  public async ExportExcel_GCConstantPrjIdRelaAi(): Promise<ExportExcelData> {
    const raw = await this.ExportExcel_GCConstantPrjIdRela();
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_GCConstantPrjIdRela();
  }
  ...
  public async BindGv_GCConstantPrjIdRela4Func(divList: HTMLDivElement) {
    ...
    try {
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortGCConstantPrjIdRelaBy,
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
  ...
  public async ExtendFldFuncMap(
    ...
    for (const objDataColumn of arrDataColumn) {
      ...
      for (const objInFor of arrGCConstantPrjIdRelaExObjLst) {
        await GCConstantPrjIdRelaEx_FuncMapByFldName(objDataColumn.fldName, objInFor);
      }
    }
  }
  ...
  public async ExtendTdFldFuncMap(
    ...
    const tdFieldNames = this.ResolveTdFieldNames(
      refGCConstantPrjIdRela_List.value?.tdFieldNames,
      dataColumn.value,
      IsNullOrEmpty,
    );
    ...
    for (const normalizedFldName of tdFieldNames) {
      ...
      for (const objInFor of arrGCConstantPrjIdRelaExObjLst) {
        await GCConstantPrjIdRelaEx_FuncMapByFldName(normalizedFldName, objInFor);
      }
    }
  }
  ...
  public async SortBy(objAnchorElement: any) {
    await this.SortByTemplate({
      ...
      currSortBy: viewVarSet.sortGCConstantPrjIdRelaBy,
      onEntitySort: async (sortColumnKey: string, sortDirection: string) => {
        if (
          Object.prototype.hasOwnProperty.call(clsGCConstantPrjIdRelaENEx, sortColumnKey) == false
        ) {
          return false;
        }
      ...
      onApplySortState: (sortBy, ascOrDesc4SortFun, sortFun) => {
        ...
        const currentCtor = this.constructor as typeof GCConstantPrjIdRelaCRUDAi;
