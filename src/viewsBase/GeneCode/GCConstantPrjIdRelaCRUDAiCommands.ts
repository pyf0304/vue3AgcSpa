...
/** Ai 场景下可用的命令标识。 */
export type GCConstantPrjIdRelaCommandIdAi =
  | 'query'
  | 'create'
  | 'update'
  | 'delete'
...
export type GCConstantPrjIdRelaCommandSpecAi = TableCommandSpec<GCConstantPrjIdRelaCommandIdAi> & {
  /** 辅助控件ID（如下拉框ID） */
  auxControlId?: string;
  /** 辅助控件类型 */
...
export const G_C_CONSTANT_PRJ_ID_RELA_COMMAND_SPECS_AI: Array<GCConstantPrjIdRelaCommandSpecAi> = [
  {
    id: 'query',
    region: 'query',
    text: '查询',
...
/** 获取查询区命令。 */
export function getQueryCommandsAi(): Array<GCConstantPrjIdRelaCommandSpecAi> {
  return G_C_CONSTANT_PRJ_ID_RELA_COMMAND_SPECS_AI.filter(
    (x) => x.region === 'query' && x.visible !== false,
  );
}

/** 获取功能区命令。 */
export function getFeatureCommandsAi(): Array<GCConstantPrjIdRelaCommandSpecAi> {
  return G_C_CONSTANT_PRJ_ID_RELA_COMMAND_SPECS_AI.filter(
