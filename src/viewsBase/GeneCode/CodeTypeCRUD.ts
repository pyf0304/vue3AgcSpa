...
/** CodeTypeCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class CodeTypeCRUD implements clsOperateList {
  ...
  public async PageLoad() {
    ...
    try {
      ...
      if (viewVarSet.sortCodeTypeBy == '')
        viewVarSet.sortCodeTypeBy = `${clsCodeTypeEN.con_FrontOrBack} Asc`;
   ...
   **/
  public CombineData(
    arrCodeTypeObjLst: Array<clsCodeTypeEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrCodeTypeObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsCodeTypeEN = arrCodeTypeObjLst[i];
    ...
    const strFileName = Format('代码类型({0})导出.xlsx', clsCodeTypeEN._CurrTabName);
    const strSheetName = '代码类型列表';
   ...
   **/
  public async ExportExcel_CodeType(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_CodeType.name;
    if (viewVarSet.sortCodeTypeBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortCodeTypeBy)为空,请检查!(In BindGv_CodeTypeCache)',
      );
    ...
    const strWhereCond = await CombineCodeTypeCondition();
    let arrCodeTypeObjLst: Array<clsCodeTypeEN> = [];
    try {
      this.recCount = await CodeType_GetRecCountByCondAsync(strWhereCond);
      if (this.recCount == 0) {
      ...
      arrCodeTypeObjLst = await CodeType_GetObjLstAsync(strWhereCond);
    } catch (e) {
    ...
    try {
      const arrDataColumn: Array<clsDataColumn> = [
        ...
        {
          fldName: 'isJavaScript',
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '是否JavaScript语言',
          text: '',
          tdClass: 'text-left',
          columnType: 'Label',
          orderNum: 8,
        ...
        {
          fldName: 'isSilverLight',
          sortBy: '',
          sortFun: SortFun,
          getDataSource: '',
          colHeader: '是否SilverLight语言',
          text: '',
          tdClass: 'text-left',
          columnType: 'Label',
          orderNum: 9,
      ...
      arrCodeTypeObjLst = arrCodeTypeObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrCodeTypeObjLst, arrDataColumn);
      //console.log("完成BindGv_CodeType!");
    } catch (e) {
  ...
  public async SetDdl_SqlDsTypeIdInDivInFeature() {
    await SQLDSType_BindDdl_SqlDsTypeIdInDivCache(divVarSet.refDivFunction, 'ddlSqlDsTypeId'); //
  }

  /**
   * 设置绑定下拉框，针对字段:[InUse]
   * (AGC.BusinessLogicEx.clsASPDropDownListBLEx_Static:GC_SetBindDdl4Bool_TS4QueryEdit)
   **/

  public async SetDdl_InUse() {
    BindDdl_TrueAndFalseInDiv(divVarSet.refDivFunction, 'ddlInUse');
  }

  /**
   * 设置绑定下拉框，针对字段:[GroupName]
   * (AGC.BusinessLogicEx.clsASPDropDownListBLEx_Static:GC_SetBindDdl4TabFeature4QueryEdit_TS)
   **/

  public async SetDdl_GroupNameInDivInFeature() {
    await FieldType_BindDdl_FieldTypeIdInDivCache(divVarSet.refDivFunction, 'ddlGroupName'); //
  }

  /** 设置字段值-SqlDsTypeId
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_btnSetFldValue_Click)
   **/
  public async btnSetSqlDsTypeId_Click() {
    const strThisFuncName = this.btnSetSqlDsTypeId_Click.name;
    try {
      const arrKeyIds = GetCheckedKeyIdsInDivObj(divVarSet.refDivList);
      if (arrKeyIds.length == 0) {
        alert(`请选择需要设置数据源类型Id的${this.thisTabName}记录!`);
        return '';
      }
      const strSqlDsTypeId = GetSelectValueInDivObj(
        divVarSet.refDivFunction,
        'ddlSqlDsTypeId_SetFldValue',
      );
      if (strSqlDsTypeId == '') {
        const strMsg = '请输入数据源类型Id(SqlDsTypeId)!';
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
        return;
      ...
      //console.log(arrKeyIds);
      await this.SetSqlDsTypeId(arrKeyIds, strSqlDsTypeId);
      await this.BindGv_CodeType4Func(divVarSet.refDivList);
    } catch (e) {
   ...
   **/
  public async btnSelectRecordInTab_Click(strCodeTypeId: string) {
    ...
    try {
      ...
      this.SelectRecord(strCodeTypeId);
    } catch (e) {
  ...
  public async DelRecord(strCodeTypeId: string) {
    ...
    try {
      const returnInt = await CodeType_DelRecordAsync(strCodeTypeId);
      if (returnInt > 0) {
   ...
   * @param sender:参数列表
   **/
  public async SelectRecord(strCodeTypeId: string) {
    const strThisFuncName = this.SelectRecord.name;
    try {
      const objCodeTypeEN = await CodeType_GetObjByCodeTypeIdAsync(strCodeTypeId);
      console.log('完成SelectRecord!', objCodeTypeEN);
      Redirect('/Index/Main_CodeType');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_CodeType();
  }

  /** 显示CodeType对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrCodeTypeObjLst:需要绑定的对象列表
   **/
  public async BindTab_CodeType(
    ...
    if (refCodeType_List.value != null) {
      ...
      if (divDataLst == null) {
        alert('在BindTab_CodeType函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
   ...
   * @param arrCodeTypeExObjLst:需要绑定的对象列表
   **/
   ...
   **/
  public async BindGv_CodeType4Func(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_CodeType4Func.name;
    if (divList == null) {
      const strMsg = Format(
    ...
    let arrCodeTypeExObjLst: Array<clsCodeTypeENEx> = [];
    try {
      this.recCount = await CodeType_GetRecCountByCondAsync(strWhereCond);
      if (this.recCount == 0) {
        const lblMsg: HTMLSpanElement = <HTMLSpanElement>document.createElement('span');
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrCodeTypeExObjLst, true);
        return;
      }
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortCodeTypeBy, //如果该字段为空,就使用下面的排序函数
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
      };
      arrCodeTypeExObjLst = await CodeTypeEx_GetObjExLstByPagerAsync(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
    ...
    if (arrCodeTypeExObjLst.length == 0) {
      const strKey = Format('{0}', clsCodeTypeEN._CurrTabName);
    ...
    try {
      await this.BindTab_CodeType4Func(divList, arrCodeTypeExObjLst);
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   **/
  public CopyToEx(objCodeTypeENS: clsCodeTypeEN): clsCodeTypeENEx {
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsCodeTypeEN, b: clsCodeTypeEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortCodeTypeBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsCodeTypeENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortCodeTypeBy = sortBy;
  ...
  public async CopyRecord(arrCodeTypeId: Array<string>) {
    ...
    try {
      ...
      for (const objInFor of arrCodeTypeObjLst) {
        ...
        objInFor.codeTypeId = strMaxStrId;
        const returnBool = await CodeType_AddNewRecordAsync(objInFor);
   ...
   **/
  public async SetSqlDsTypeId(arrCodeTypeId: Array<string>, strSqlDsTypeId: string) {
    const strThisFuncName = this.SetSqlDsTypeId.name;
    ...
    try {
      ...
      for (const objInFor of arrCodeTypeObjLst) {
        ...
        objCodeTypeEN.SetSqlDsTypeId(strSqlDsTypeId);
        ...
        if (returnBool == true) {
          //CodeType_DeleteKeyIdCache(objInFor.codeTypeId);
          intCount++;
        } else {
  ...
  public async DelMultiRecord(arrCodeTypeId: Array<string>) {
    ...
    try {
      const returnInt = await CodeType_DelCodeTypesAsync(arrCodeTypeId);
      if (returnInt > 0) {
        //CodeType_ReFreshCache();
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param divContainer:显示容器
   * @param objCodeType:需要显示的对象
   **/
  public ShowCodeTypeObj(divContainer: HTMLDivElement, objCodeType: clsCodeTypeEN) {
    if (divContainer == null) {
      alert(Format('所给div为空，divContainer为null!', divContainer));
      return;
    }
    const sstrKeys = GetObjKeys(objCodeType);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objCodeType.GetFldValue(strKey);
