...
import {
  CombineCTCodeTypeGroupRelaCondition,
  divVarSet,
  viewVarSet,
  dataColumn,
  BindTabByList,
...
import {
  CTCodeTypeGroupRela_GetRecCountByCondAsync,
...
import {
  CTCodeTypeGroupRelaEx_FuncMapByFldName,
  CTCodeTypeGroupRelaEx_GetObjExLstByPagerAsync,
} from '@/ts/L3ForWApiEx/GeneCode/clsCTCodeTypeGroupRelaExWApi';
...
import {
  ...
  getListColumnsAi,
} from '@/viewsBase/GeneCode/CTCodeTypeGroupRelaCRUDAiColumns';

/**
 * Ai版基类：
 * 在Ai3基础上增加命令schema驱动，不改变原有基类行为。
 */
export abstract class CTCodeTypeGroupRelaCRUDAi
  ...
  implements clsOperateList
{
  ...
  public async PageLoadCache() {
    try {
      ...
      if (viewVarSet.sortCTCodeTypeGroupRelaBy == '') {
        viewVarSet.sortCTCodeTypeGroupRelaBy = `${clsCTCodeTypeGroupRelaEN.con_CodeTypeId} Asc`;
      }
  ...
  public async DelMultiRecord(arrCodeTypeId: Array<string>) {
    ...
    try {
      const returnInt = await CTCodeTypeGroupRela_DelRecKeyLstsAsync(arrCodeTypeId);
      if (returnInt > 0) {
  ...
  /** 按当前查询条件导出原始列表数据。 */
  public async ExportExcel_CTCodeTypeGroupRela(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_CTCodeTypeGroupRela.name;
    if (viewVarSet.sortCTCodeTypeGroupRelaBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortCTCodeTypeGroupRelaBy)为空,请检查!(In BindGv_CTCodeTypeGroupRelaCache)',
      );
      console.error(strMsg);
      alert(strMsg);
    ...
    const strWhereCond = await CombineCTCodeTypeGroupRelaCondition();
    let arrCTCodeTypeGroupRelaObjLst: Array<clsCTCodeTypeGroupRelaEN> = [];
    try {
      this.recCount = await CTCodeTypeGroupRela_GetRecCountByCondAsync(strWhereCond);
      if (this.recCount == 0) {
      ...
      arrCTCodeTypeGroupRelaObjLst = await CTCodeTypeGroupRela_GetObjLstAsync(strWhereCond);
    } catch (e) {
    ...
    if (arrCTCodeTypeGroupRelaObjLst.length == 0) {
    ...
    try {
      ...
      arrCTCodeTypeGroupRelaObjLst = arrCTCodeTypeGroupRelaObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrCTCodeTypeGroupRelaObjLst, arrDataColumn);
    } catch (e) {
  ...
  /** 按导出列规格整理导出结果。 */
  public async ExportExcel_CTCodeTypeGroupRelaAi(): Promise<ExportExcelData> {
    const raw = await this.ExportExcel_CTCodeTypeGroupRela();
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_CTCodeTypeGroupRela();
  }

  /** 把实体列表组装成导出文件数据。 */
  public CombineData(
    arrCTCodeTypeGroupRelaObjLst: Array<clsCTCodeTypeGroupRelaEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const arrData = this.BuildExportRowsByColumns(
      arrCTCodeTypeGroupRelaObjLst,
      arrDataColumn,
      (obj, fld) => obj.GetFldValue(fld),
    );
    const strFileName = Format(
      'CTCodeTypeGroupRela({0})导出.xlsx',
      clsCTCodeTypeGroupRelaEN._CurrTabName,
    );
    const strSheetName = 'CTCodeTypeGroupRela列表';
    return { arrObjLst: arrData, sheetName: strSheetName, fileName: strFileName };
  }

  /** 绑定表格内容并同步分页器。 */
  public async BindTab_CTCodeTypeGroupRela4Func(
    divContainer: HTMLDivElement,
    arrCTCodeTypeGroupRelaExObjLst: Array<clsCTCodeTypeGroupRelaENEx>,
  ) {
    const strThisFuncName = this.BindTab_CTCodeTypeGroupRela4Func.name;
    ...
    if (refCTCodeTypeGroupRela_List.value != null) {
      ...
      try {
        await this.ExtendTdFldFuncMap(arrCTCodeTypeGroupRelaExObjLst);
      } catch (e) {
      ...
      await BindTabByList(arrCTCodeTypeGroupRelaExObjLst, this.dispAllErrMsg_q);
    } else {
      try {
        await this.ExtendFldFuncMap(arrCTCodeTypeGroupRelaExObjLst, arrDataColumn);
      } catch (e) {
      ...
      if (divDataLst == null) {
        alert('在BindTab_CTCodeTypeGroupRela4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
        arrCTCodeTypeGroupRelaExObjLst,
        arrDataColumn,
  ...
  public async BindGv_CTCodeTypeGroupRela4Func(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_CTCodeTypeGroupRela4Func.name;
    ...
    let arrCTCodeTypeGroupRelaExObjLst: Array<clsCTCodeTypeGroupRelaENEx> = [];
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        console.error('Error: ', strMsg);
        alert(strMsg);
        BindTabByList(arrCTCodeTypeGroupRelaExObjLst, true);
        return;
      }
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortCTCodeTypeGroupRelaBy,
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
      ...
      arrCTCodeTypeGroupRelaExObjLst = await CTCodeTypeGroupRelaEx_GetObjExLstByPagerAsync(
        objPagerPara,
      );
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
    ...
    if (arrCTCodeTypeGroupRelaExObjLst.length == 0) {
      const strKey = Format('{0}', clsCTCodeTypeGroupRelaEN._CurrTabName);
    ...
    try {
      await this.BindTab_CTCodeTypeGroupRela4Func(divList, arrCTCodeTypeGroupRelaExObjLst);
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName})`;
      console.error(strMsg);
      alert(strMsg);
  ...
  public async ExtendFldFuncMap(
    ...
    for (const objDataColumn of arrDataColumn) {
      ...
      for (const objInFor of arrCTCodeTypeGroupRelaExObjLst) {
        await CTCodeTypeGroupRelaEx_FuncMapByFldName(objDataColumn.fldName, objInFor);
      }
    }
  }
  ...
  public async ExtendTdFldFuncMap(
    ...
    const tdFieldNames = this.ResolveTdFieldNames(
      refCTCodeTypeGroupRela_List.value?.tdFieldNames,
      dataColumn.value,
      IsNullOrEmpty,
    );
    ...
    for (const normalizedFldName of tdFieldNames) {
      ...
      for (const objInFor of arrCTCodeTypeGroupRelaExObjLst) {
        await CTCodeTypeGroupRelaEx_FuncMapByFldName(normalizedFldName, objInFor);
      }
    }
  }
   ...
   * 日期: 2026-06-06
   */
  public SortFunExportExcel(a: clsCTCodeTypeGroupRelaEN, b: clsCTCodeTypeGroupRelaEN): number {
    // 第一级排序：orderNum
  ...
  public async SortBy(objAnchorElement: any) {
    await this.SortByTemplate({
      ...
      currSortBy: viewVarSet.sortCTCodeTypeGroupRelaBy,
      onEntitySort: async (sortColumnKey: string, sortDirection: string) => {
        if (
          Object.prototype.hasOwnProperty.call(clsCTCodeTypeGroupRelaENEx, sortColumnKey) == false
        ) {
          return false;
        }
      ...
      onApplySortState: (sortBy, ascOrDesc4SortFun, sortFun) => {
        viewVarSet.sortCTCodeTypeGroupRelaBy = sortBy;
        viewVarSet.ascOrDesc4SortFun = ascOrDesc4SortFun;
        const currentCtor = this.constructor as typeof CTCodeTypeGroupRelaCRUDAi;
