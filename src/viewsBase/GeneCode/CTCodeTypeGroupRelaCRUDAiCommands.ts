...
/** Ai 场景下可用的命令标识。 */
export type CTCodeTypeGroupRelaCommandIdAi = 'query' | 'create' | 'update' | 'delete' | 'export';

/** Ai 命令配置：复用通用表命令结构并扩展辅助控件信息。 */
export type CTCodeTypeGroupRelaCommandSpecAi = TableCommandSpec<CTCodeTypeGroupRelaCommandIdAi> & {
  /** 辅助控件ID（如下拉框ID） */
  auxControlId?: string;
  /** 辅助控件类型 */
...
export const C_T_CODE_TYPE_GROUP_RELA_COMMAND_SPECS_AI: Array<CTCodeTypeGroupRelaCommandSpecAi> = [
  {
    id: 'query',
    region: 'query',
    text: '查询',
...
/** 获取查询区命令。 */
export function getQueryCommandsAi(): Array<CTCodeTypeGroupRelaCommandSpecAi> {
  return C_T_CODE_TYPE_GROUP_RELA_COMMAND_SPECS_AI.filter(
    (x) => x.region === 'query' && x.visible !== false,
  );
}

/** 获取功能区命令。 */
export function getFeatureCommandsAi(): Array<CTCodeTypeGroupRelaCommandSpecAi> {
  return C_T_CODE_TYPE_GROUP_RELA_COMMAND_SPECS_AI.filter(
