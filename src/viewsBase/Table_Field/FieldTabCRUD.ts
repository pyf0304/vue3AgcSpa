/**
 * 类名:FieldTabCRUD(界面:FieldTabCRUD,00050316)
 * 表名:FieldTab(00050021)
 * 版本:2025.05.12.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/15 01:34:51
 * 生成者:
 工程名称:AGC(0005)
...
import {
  FieldTab_GetRecCountByCondAsync,
  FieldTab_GetObjLstAsync,
  FieldTab_DelRecordAsync,
  FieldTab_GetObjByFldIdAsync,
  FieldTab_GetObjLstByFldIdLstAsync,
  FieldTab_GetMaxStrIdByPrefixAsync,
  FieldTab_AddNewRecordAsync,
  FieldTab_UpdateRecordAsync,
  FieldTab_DelFieldTabsAsync,
} from '@/ts/L3ForWApi/Table_Field/clsFieldTabWApi';
import { IsNullOrEmpty, Format } from '@/ts/PubFun/clsString';
import { vFieldTab_SimEx_BindDdl_FldIdInDivCache } from '@/ts/L3ForWApiEx/Table_Field/clsvFieldTab_SimExWApi';
import {
...
/** FieldTabCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class FieldTabCRUD implements clsOperateList {
  ...
  public static objPageCRUD: FieldTabCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    FieldTabCRUD.objPageCRUD = this;
  ...
  public async PageLoad() {
    ...
    try {
      ...
      if (viewVarSet.sortFieldTabBy == '')
        viewVarSet.sortFieldTabBy = `${clsFieldTabEN.con_FldId} Asc`;
   ...
   **/
  public CombineData(
    arrFieldTabObjLst: Array<clsFieldTabEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrFieldTabObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsFieldTabEN = arrFieldTabObjLst[i];
    ...
    const strFileName = Format('工程字段({0})导出.xlsx', clsFieldTabEN._CurrTabName);
    const strSheetName = '工程字段列表';
   ...
   **/
  public async ExportExcel_FieldTab(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_FieldTab.name;
    if (viewVarSet.sortFieldTabBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortFieldTabBy)为空,请检查!(In BindGv_FieldTabCache)',
      );
    ...
    const strWhereCond = await CombineFieldTabCondition();
    ...
    try {
      ...
      arrFieldTabObjLst = await FieldTab_GetObjLstAsync(strWhereCond);
    } catch (e) {
    ...
    try {
      ...
      arrFieldTabObjLst = arrFieldTabObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrFieldTabObjLst, arrDataColumn);
      //console.log("完成BindGv_FieldTab!");
    } catch (e) {
  ...
  /**
   * 设置绑定下拉框，针对字段:[HomologousFldId]
   * (AGC.BusinessLogicEx.clsASPDropDownListBLEx_Static:GC_SetBindDdl4TabFeature4QueryEdit_TS)
   **/

  public async SetDdl_HomologousFldIdInDivInFeature(strPrjId: string) {
    if (IsNullOrEmpty(strPrjId) == true) {
      const strMsg = Format('参数:[strPrjId]不能为空!(In .SetDdl_HomologousFldIdInDivInFeature)');
      console.error(strMsg);
      throw strMsg;
    }
    if (strPrjId.length != 4) {
      const strMsg = Format(
        '缓存分类变量:[strPrjId]的长度:[{0}]不正确!(.SetDdl_HomologousFldIdInDivInFeature)',
        strPrjId.length,
      );
      console.error(strMsg);
      throw strMsg;
   ...
   **/
  public async btnSetHomologousFldId_Click() {
    const strThisFuncName = this.btnSetHomologousFldId_Click.name;
    try {
      const arrKeyIds = GetCheckedKeyIdsInDivObj(divVarSet.refDivList);
      if (arrKeyIds.length == 0) {
        alert(`请选择需要设置同源字段Id的${this.thisTabName}记录!`);
        return '';
      }
      ...
      //console.log(arrKeyIds);
      await this.SetHomologousFldId(arrKeyIds, strHomologousFldId);
      await this.BindGv_FieldTab4Func(divVarSet.refDivList);
    } catch (e) {
   ...
   **/
  public async btnSelectRecordInTab_Click(strFldId: string) {
   ...
   **/
  public async DelRecord(strFldId: string) {
    const strThisFuncName = this.DelRecord.name;
    try {
      const returnInt = await FieldTab_DelRecordAsync(strFldId);
      if (returnInt > 0) {
   ...
   * @param sender:参数列表
   **/
  public async SelectRecord(strFldId: string) {
    const strThisFuncName = this.SelectRecord.name;
    try {
      const objFieldTabEN = await FieldTab_GetObjByFldIdAsync(strFldId);
      console.log('完成SelectRecord!', objFieldTabEN);
      Redirect('/Index/Main_FieldTab');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_FieldTab();
  }

  /** 显示FieldTab对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrFieldTabObjLst:需要绑定的对象列表
   **/
  public async BindTab_FieldTab(
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      {
        fldName: clsFieldTabEN.con_FldName,
      ...
      {
        fldName: clsFieldTabENEx.con_TabNum,
        sortBy: 'tabNum',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '表数',
    ...
    if (refFieldTab_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrFieldTabExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_FieldTab函数中，divDataLst不存在!');
        return;
      }
  ...
  public async BindTab_FieldTab4Func(
    ...
    if (refFieldTab_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_FieldTab4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(divDataLst, arrFieldTabExObjLst, arrDataColumn, clsFieldTabEN.con_FldId, this);
    }
   ...
   * @param arrFieldTabExObjLst:需要映射的对象列表
  ...
  public async ExtendFldFuncMap(
    ...
    for (const objDataColumn of arrDataColumn) {
      ...
      for (const objInFor of arrFieldTabExObjLst) {
        await FieldTabEx_FuncMapByFldName(objDataColumn.fldName, objInFor);
      }
    }
  }
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_FieldTab4Func(this.listPara.listDiv);
  }
   ...
   **/
  public async BindGv_FieldTab4Func(divList: HTMLDivElement) {
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrFieldTabExObjLst, true);
        return;
      }
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortFieldTabBy, //如果该字段为空,就使用下面的排序函数
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
    ...
    if (arrFieldTabExObjLst.length == 0) {
      const strKey = Format('{0}', clsFieldTabEN._CurrTabName);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsFieldTabEN, b: clsFieldTabEN): number {
    if (a.prjId == b.prjId) {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    if (Object.prototype.hasOwnProperty.call(clsFieldTabENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
   ...
   **/
  public async CopyRecord(arrFldId: Array<string>) {
    const strThisFuncName = this.CopyRecord.name;
    try {
      const arrFieldTabObjLst = await FieldTab_GetObjLstByFldIdLstAsync(arrFldId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrFieldTabObjLst) {
        const strMaxStrId = await FieldTab_GetMaxStrIdByPrefixAsync(objInFor.prjId);
        //console.log('strMaxStrId=' + strMaxStrId);
        objInFor.fldId = strMaxStrId;
        const returnBool = await FieldTab_AddNewRecordAsync(objInFor);
   ...
   **/
  public async SetHomologousFldId(arrFldId: Array<string>, strHomologousFldId: string) {
    const strThisFuncName = this.SetHomologousFldId.name;
    if (strHomologousFldId == null || strHomologousFldId == '') {
      const strMsg = '请输入同源字段Id(HomologousFldId)!';
      console.error('Error: ', strMsg);
      //console.trace();
      alert(strMsg);
    ...
    try {
      ...
      let intCount = 0;
      for (const objInFor of arrFieldTabObjLst) {
        const objFieldTabEN = new clsFieldTabEN();
        ObjectAssign(objFieldTabEN, objInFor);
        objFieldTabEN.SetFldId(objInFor.fldId);
        objFieldTabEN.SetHomologousFldId(strHomologousFldId);
        let returnBool = false;
        try {
          objFieldTabEN.sfUpdFldSetStr = objFieldTabEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await FieldTab_UpdateRecordAsync(objFieldTabEN);
        } catch (e) {
          const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
          console.error(strMsg);
          throw strMsg;
        ...
        if (returnBool == true) {
          //FieldTab_DeleteKeyIdCache(objInFor.fldId);
          intCount++;
        } else {
   ...
   **/
  public async DelMultiRecord(arrFldId: Array<string>) {
    const strThisFuncName = this.DelMultiRecord.name;
    try {
      const returnInt = await FieldTab_DelFieldTabsAsync(arrFldId);
      if (returnInt > 0) {
        //FieldTab_ReFreshCache();
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param divContainer:显示容器
   * @param objFieldTab:需要显示的对象
   **/
  public ShowFieldTabObj(divContainer: HTMLDivElement, objFieldTab: clsFieldTabEN) {
    if (divContainer == null) {
      alert(Format('所给div为空，divContainer为null!', divContainer));
      return;
    }
    const sstrKeys = GetObjKeys(objFieldTab);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objFieldTab.GetFldValue(strKey);
