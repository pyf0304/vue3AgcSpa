/**
 * 类名:FieldTab4CodeConvCRUD(界面:FieldTab4CodeConvCRUD,00050326)
 * 表名:FieldTab4CodeConv(00050593)
 * 版本:2025.05.12.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/15 01:34:50
 * 生成者:
 工程名称:AGC(0005)
...
import {
  PrjId_Session,
  CombineFieldTab4CodeConvConditionObj4ExportExcel,
  CombineFieldTab4CodeConvConditionObj,
  divVarSet,
  viewVarSet,
  dataColumn,
  BindTabByList,
...
import {
  ...
  FieldTab4CodeConv_GetSubObjLstCache,
  FieldTab4CodeConv_DelRecordAsync,
  FieldTab4CodeConv_ReFreshCache,
  FieldTab4CodeConv_GetObjByFldIdAsync,
  FieldTab4CodeConv_GetObjExLstByPagerCache,
  FieldTab4CodeConv_GetObjLstByFldIdLstAsync,
  FieldTab4CodeConv_AddNewRecordAsync,
  FieldTab4CodeConv_DelFieldTab4CodeConvsAsync,
} from '@/ts/L3ForWApi/Table_Field/clsFieldTab4CodeConvWApi';
import {
  FieldTab4CodeConvEx_CopyToEx,
  FieldTab4CodeConvEx_FuncMapByFldName,
} from '@/ts/L3ForWApiEx/Table_Field/clsFieldTab4CodeConvExWApi';
import {
...
/** FieldTab4CodeConvCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class FieldTab4CodeConvCRUD implements clsOperateList {
  ...
  public static objPageCRUD: FieldTab4CodeConvCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    FieldTab4CodeConvCRUD.objPageCRUD = this;
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortFieldTab4CodeConvBy == '')
        viewVarSet.sortFieldTab4CodeConvBy = `${clsFieldTab4CodeConvEN.con_FldId} Asc`;
   ...
   **/
  public CombineData(
    arrFieldTab4CodeConvObjLst: Array<clsFieldTab4CodeConvEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrFieldTab4CodeConvObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsFieldTab4CodeConvEN = arrFieldTab4CodeConvObjLst[i];
    ...
    const strFileName = Format('字段4代码转换({0})导出.xlsx', clsFieldTab4CodeConvEN._CurrTabName);
    const strSheetName = '字段4代码转换列表';
  ...
  public CombineData4Func(
    ...
    const intRowNum = arrFieldTab4CodeConvExObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsFieldTab4CodeConvENEx = arrFieldTab4CodeConvExObjLst[i];
      const objRow: Record<string, any> = {};
      for (let j = 0; j < intColNum; j++) {
        const fldName = arrDataColumn[j].fldName;
        const colHeader = arrDataColumn[j].colHeader;
   ...
   **/
  public async ExportExcel_FieldTab4CodeConv4Func(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_FieldTab4CodeConv4Func.name;
    ...
    const objFieldTab4CodeConvCond = await CombineFieldTab4CodeConvConditionObj4ExportExcel();
    ...
    try {
      ...
      const arrFieldTab4CodeConvObjLst = await FieldTab4CodeConv_GetSubObjLstCache(
        objFieldTab4CodeConvCond,
        PrjId_Session.value,
      );
      arrFieldTab4CodeConvExObjLst = arrFieldTab4CodeConvObjLst.map(FieldTab4CodeConvEx_CopyToEx);
    } catch (e) {
    ...
    try {
      ...
      arrFieldTab4CodeConvExObjLst = arrFieldTab4CodeConvExObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrFieldTab4CodeConvExObjLst, arrDataColumn);
      //console.log("完成BindGv_FieldTab4CodeConv!");
    } catch (e) {
  ...
  public async DelRecord(strFldId: string) {
    ...
    try {
      const returnInt = await FieldTab4CodeConv_DelRecordAsync(strFldId);
      if (returnInt > 0) {
        FieldTab4CodeConv_ReFreshCache(PrjId_Session.value);
  ...
  public async SelectRecord(strFldId: string) {
    ...
    try {
      ...
      console.log('完成SelectRecord!', objFieldTab4CodeConvEN);
      Redirect('/Index/Main_FieldTab4CodeConv');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_FieldTab4CodeConv4Func();
  }

  /** 显示FieldTab4CodeConv对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrFieldTab4CodeConvObjLst:需要绑定的对象列表
   **/
  public async BindTab_FieldTab4CodeConv(
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      {
        fldName: clsFieldTab4CodeConvENEx.con_TabName,
        sortBy: 'tabName',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '表名',
    ...
    if (refFieldTab4CodeConv_List.value != null) {
      ...
      if (divDataLst == null) {
        alert('在BindTab_FieldTab4CodeConv函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
   ...
   * @param arrFieldTab4CodeConvExObjLst:需要映射的对象列表
  ...
  public async ExtendFldFuncMap(
    ...
    for (const objDataColumn of arrDataColumn) {
      ...
      for (const objInFor of arrFieldTab4CodeConvExObjLst) {
        await FieldTab4CodeConvEx_FuncMapByFldName(objDataColumn.fldName, objInFor);
      }
    }
  }
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_FieldTab4CodeConv4Func(this.listPara.listDiv);
  }
  ...
  public async BindGv_FieldTab4CodeConv4Func(divList: HTMLDivElement) {
    ...
    const objFieldTab4CodeConvCond = await CombineFieldTab4CodeConvConditionObj();
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrFieldTab4CodeConvExObjLst, true);
        return;
      }
   ...
   **/
  public CopyToEx(objFieldTab4CodeConvENS: clsFieldTab4CodeConvEN): clsFieldTab4CodeConvENEx {
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsFieldTab4CodeConvEN, b: clsFieldTab4CodeConvEN): number {
    if (a.fldId == b.fldId) {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortFieldTab4CodeConvBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsFieldTab4CodeConvENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortFieldTab4CodeConvBy = sortBy;
    viewVarSet.ascOrDesc4SortFun = ascOrDesc4SortFun;
    FieldTab4CodeConvCRUD.sortFunStatic = sortFun;
    await this.BindGv_FieldTab4CodeConv4Func(this.listPara.listDiv);
  }
  ...
  public async CopyRecord(arrFldId: Array<string>) {
    ...
    try {
      const arrFieldTab4CodeConvObjLst = await FieldTab4CodeConv_GetObjLstByFldIdLstAsync(arrFldId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrFieldTab4CodeConvObjLst) {
        const returnBool = await FieldTab4CodeConv_AddNewRecordAsync(objInFor);
  ...
  public async DelMultiRecord(arrFldId: Array<string>) {
    ...
    try {
      const returnInt = await FieldTab4CodeConv_DelFieldTab4CodeConvsAsync(arrFldId);
      if (returnInt > 0) {
        FieldTab4CodeConv_ReFreshCache(PrjId_Session.value);
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param divContainer:显示容器
   * @param objFieldTab4CodeConv:需要显示的对象
   **/
  public ShowFieldTab4CodeConvObj(
    divContainer: HTMLDivElement,
    ...
    const sstrKeys = GetObjKeys(objFieldTab4CodeConv);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objFieldTab4CodeConv.GetFldValue(strKey);
   ...
   * @param pobjFieldTab4CodeConvEN:表实体类对象
   * @returns 列表的第一个关键字值
   **/
