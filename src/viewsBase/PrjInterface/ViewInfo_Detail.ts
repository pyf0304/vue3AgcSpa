/**
 * 类名:ViewInfo_Detail(界面:ViewInfo_U,00050280)
 * 表名:ViewInfo(00050006)
 * 版本:2025.05.19.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/19 11:40:51
 * 生成者:
 工程名称:AGC(0005)
 ...
 * 编程语言:TypeScript
 **/
import { ViewInfo_GetObjByViewIdAsync } from '@/ts/L3ForWApi/PrjInterface/clsViewInfoWApi';
...
import { divVarSet, refViewInfo_Detail } from '@/views/PrjInterface/ViewInfo_UVueShare';
import { clsViewInfoENEx } from '@/ts/L0Entity/PrjInterface/clsViewInfoENEx';
import { IShowList } from '@/ts/PubFun/IShowList';
import { enumPageDispMode } from '@/ts/PubFun/enumPageDispMode';
/* ViewInfo_Detail 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_DetailCS_TS4TypeScript:GeneCode)
*/
export abstract class ViewInfo_Detail {
  ...
  public HideDialog_ViewInfo() {
    if (ViewInfo_Detail.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      refViewInfo_Detail.value.hideDialog();
    }
  }
  ...
  public async ShowDialog_ViewInfo(strOp: string): Promise<boolean> {
    ...
    if (ViewInfo_Detail.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      ...
      await refViewInfo_Detail.value.showDialog();
    }
    divVarSet.refDivDetail = refViewInfo_Detail.value.$refs.refDivDetail;
    if (divVarSet.refDivDetail == null) {
      if (ViewInfo_Detail.times4TestShowDialog < 2) {
        ViewInfo_Detail.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_ViewInfo(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      ViewInfo_Detail.times4TestShowDialog = 0;
    }
    if (strOp === 'Detail') {
      this.btnCancelViewInfo = '关闭';
    }
    return true;
  }
...
*/
  public async DetailRecord4Func(strViewId: string) {
    ...
    try {
      ...
      if (refViewInfo_Detail.value == null) {
        const strMsg = Format(
          '当前详细信息区的DetailObj为空，请检查！(in {0}.{1}).\nrefViewInfo_Detail imported from: @/views/PrjInterface/ViewInfo_UVueShare',
          this.constructor.name,
          strThisFuncName,
        );
        console.error(strMsg);
      ...
      await refViewInfo_Detail.value.ShowDataFromViewInfoObj(objViewInfoENEx);
    } catch (e) {
      const strMsg = Format(
...
*/
  ...
  public set btnCancelViewInfo(value: string) {
    refViewInfo_Detail.value.strCancelButtonText = value;
  }
}
