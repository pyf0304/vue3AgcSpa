 ...
 **/
export abstract class ViewRelaTabCRUD implements clsOperateList {
   ...
   **/
  public CombineData(
    arrViewRelaTabObjLst: Array<clsViewRelaTabEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrViewRelaTabObjLst.length;
   ...
   **/
  public async ExportExcel_ViewRelaTab4Func(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_ViewRelaTab4Func.name;
    ...
    try {
      ...
      const arrViewRelaTabObjLst = await ViewRelaTab_GetObjLstAsync(strWhereCond);
      arrViewRelaTabExObjLst = arrViewRelaTabObjLst.map(ViewRelaTabEx_CopyToEx);
    } catch (e) {
    ...
    try {
      ...
      arrViewRelaTabExObjLst = arrViewRelaTabExObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrViewRelaTabExObjLst, arrDataColumn);
  ...
  public async DelRecord(lngmId: number) {
    ...
    try {
      const returnInt = await ViewRelaTab_DelRecordAsync(lngmId);
      if (returnInt > 0) {
  ...
  public async SelectRecord(lngmId: number) {
    ...
    try {
      ...
      console.log('完成SelectRecord!', objViewRelaTabEN);
      Redirect('/Index/Main_ViewRelaTab');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_ViewRelaTab4Func();
  }

  /** 显示ViewRelaTab对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrViewRelaTabObjLst:需要绑定的对象列表
   **/
  public async BindTab_ViewRelaTab(
  ...
  public async BindGv_ViewRelaTab(divList: HTMLDivElement) {
    ...
    try {
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortViewRelaTabBy, //如果该字段为空,就使用下面的排序函数
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
   ...
   **/
  public CopyToEx(objViewRelaTabENS: clsViewRelaTabEN): clsViewRelaTabENEx {
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsViewRelaTabEN, b: clsViewRelaTabEN): number {
    if (a.mId == b.mId) {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortViewRelaTabBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsViewRelaTabENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
  ...
  public async CopyRecord(arrmId: Array<string>) {
    ...
    try {
      const arrViewRelaTabObjLst = await ViewRelaTab_GetObjLstBymIdLstAsync(arrmId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrViewRelaTabObjLst) {
        const returnBool = await ViewRelaTab_AddNewRecordAsync(objInFor);
  ...
  public async DelMultiRecord(arrmId: Array<string>) {
    ...
    try {
      const returnInt = await ViewRelaTab_DelViewRelaTabsAsync(arrmId);
      if (returnInt > 0) {
        //ViewRelaTab_ReFreshCache();
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param objViewRelaTab:需要显示的对象
   **/
  public ShowViewRelaTabObj(divContainer: HTMLDivElement, objViewRelaTab: clsViewRelaTabEN) {
    if (divContainer == null) {
      alert(Format('所给div为空，divContainer为null!', divContainer));
      return;
    }
    const sstrKeys = GetObjKeys(objViewRelaTab);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objViewRelaTab.GetFldValue(strKey);
   ...
   * @param pobjViewRelaTabEN:表实体类对象
   * @returns 列表的第一个关键字值
   **/
