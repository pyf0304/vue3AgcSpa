 ...
 /**
 * 类名:PrjDataBase_DetailAi(界面:PrjDataBaseCRUD,00050346)
 * 表名:PrjDataBase(00050176)
 * 版本:2026.05.30(服务器:WIN-SRV103-116)
 * 日期:2026/06/10 23:57:22
 * 生成者:
 工程名称:AGC(0005)
...
import { divVarSet,refPrjDataBase_DetailAi } from "@/views/PrjManage/PrjDataBaseVueShare";
import { clsPrjDataBaseENEx } from "@/ts/L0Entity/PrjManage/clsPrjDataBaseENEx";
import { IShowList } from "@/ts/PubFun/IShowList";
import { enumPageDispMode } from "@/ts/PubFun/enumPageDispMode";
 /* PrjDataBase_DetailAi 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_DetailAi_TS4TypeScript:GeneCode)
*/
export abstract class  PrjDataBase_DetailAi 
{
...
if (PrjDataBase_DetailAi.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
 refPrjDataBase_DetailAi.value.hideDialog();
}
}
...
if (PrjDataBase_DetailAi.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
if (refPrjDataBase_DetailAi.value == null)
{
const strMsg = Format(
...
await refPrjDataBase_DetailAi.value.showDialog();
}
divVarSet.refDivDetail = refPrjDataBase_DetailAi.value.$refs.refDivDetail;
if (divVarSet.refDivDetail == null)
{
if (PrjDataBase_DetailAi.times4TestShowDialog < 2)
{
PrjDataBase_DetailAi.times4TestShowDialog++;
setTimeout(() => {
...
PrjDataBase_DetailAi.times4TestShowDialog = 0;
}
       if (strOp === "Detail" ) {
...
*/
public async DetailRecord4Func(key: PrjDataBaseKey) 
{
...
await PrjDataBaseEx_FuncMapByFldName(clsPrjDataBaseENEx.con_DataBaseTypeName, objPrjDataBaseENEx);
await PrjDataBaseEx_FuncMapByFldName(clsPrjDataBaseENEx.con_UseStateName, objPrjDataBaseENEx);
if (refPrjDataBase_DetailAi.value == null)
{
const strMsg = Format('当前详细信息区的DetailObj为空，请检查！(in {0}.{1}).\nrefPrjDataBase_DetailAi imported from: @/views/PrjManage/PrjDataBaseVueShare',
this.constructor.name,
strThisFuncName,
);
console.error(strMsg);
...
await refPrjDataBase_DetailAi.value.ShowDataFromPrjDataBaseObj(objPrjDataBaseENEx);
...
*/
public async btnDetailRecord_Click(key: PrjDataBaseKey) {
this.opType = "Detail";
const bolIsSuccess = await this.ShowDialog_PrjDataBase('Detail');
if (bolIsSuccess == false) return;
if (IsNullOrEmpty(key.prjDataBaseId) == true)
...
{
const strMsg = Format("(errid:WiTsCs0030)Copy表对象数据出错,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
console.error(strMsg);
alert(strMsg);
 return objPrjDataBaseENT;
}
...
refPrjDataBase_DetailAi.value.strCancelButtonText = value;
}
}