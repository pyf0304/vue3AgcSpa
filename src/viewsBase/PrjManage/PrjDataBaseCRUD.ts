...
/** PrjDataBaseCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class PrjDataBaseCRUD implements clsOperateList {
  ...
  public CombineData(
    ...
    const intRowNum = arrPrjDataBaseObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsPrjDataBaseEN = arrPrjDataBaseObjLst[i];
  ...
  public async ExportExcel_PrjDataBaseCache(): Promise<ExportExcelData> {
    ...
    try {
      ...
      //console.log("完成BindGv_PrjDataBase!");
    } catch (e) {
   ...
   **/
  public async btnSelectRecordInTab_Click(strPrjDataBaseId: string) {
    ...
    try {
      ...
      this.SelectRecord(strPrjDataBaseId);
    } catch (e) {
  ...
  /** 显示PrjDataBase对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrPrjDataBaseObjLst:需要绑定的对象列表
   **/
  public async BindTab_PrjDataBase(
    ...
    if (refPrjDataBase_List.value != null) {
      ...
      if (divDataLst == null) {
        alert('在BindTab_PrjDataBase函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
   ...
   * @param arrPrjDataBaseExObjLst:需要绑定的对象列表
   **/
   ...
   * @param arrPrjDataBaseExObjLst:需要映射的对象列表
  ...
  public async BindGv_PrjDataBase4Func(divList: HTMLDivElement) {
    ...
    try {
      ...
      if (PrjDataBaseCRUD.sortFunStatic != undefined) {
        strSortFun = PrjDataBaseCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
      }
    ...
    try {
      ...
      //console.log("完成BindGv_PrjDataBase4Func!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName})`;
      console.error(strMsg);
      alert(strMsg);
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortPrjDataBaseBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsPrjDataBaseENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
    ...
    PrjDataBaseCRUD.sortFunStatic = sortFun;
    await this.BindGv_PrjDataBase4Func(this.listPara.listDiv);
  }
  ...
  public async ExtendTdFldFuncMap(arrPrjDataBaseExObjLst: Array<clsPrjDataBaseENEx>) {
    const arrFldName = clsPrjDataBaseEN._AttributeName;
    const rawTdFieldNames = refPrjDataBase_List.value?.tdFieldNames;
    const tdFieldNames = Array.isArray(rawTdFieldNames)
      ? rawTdFieldNames
      : Array.isArray(rawTdFieldNames?.value)
      ? rawTdFieldNames.value
      : Array.isArray(dataColumn.value)
      ? dataColumn.value.map((col) => col.fldName).filter((name) => IsNullOrEmpty(name) == false)
      : [];

    if (tdFieldNames == null || tdFieldNames.length === 0) return;

    for (const strFldName of tdFieldNames) {
      if (IsNullOrEmpty(strFldName) == true) continue;
      if (arrFldName.indexOf(strFldName) > -1) continue;
      for (const objInFor of arrPrjDataBaseExObjLst) {
        await PrjDataBase_FuncMapByFldName(strFldName, objInFor);
      }
    }
  }
   ...
   **/
  public async CopyRecord(arrPrjDataBaseId: Array<string>) {
    ...
    try {
      ...
      for (const objInFor of arrPrjDataBaseObjLst) {
        const strMaxStrId = await PrjDataBase_GetMaxStrIdAsync();
        //console.log('strMaxStrId=' + strMaxStrId);
        objInFor.prjDataBaseId = strMaxStrId;
        const returnBool = await PrjDataBase_AddNewRecordAsync(objInFor);
   ...
   * @param divContainer:显示容器
   * @param objPrjDataBase:需要显示的对象
   **/
  public ShowPrjDataBaseObj(divContainer: HTMLDivElement, objPrjDataBase: clsPrjDataBaseEN) {
    if (divContainer == null) {
      alert(Format('所给div为空，divContainer为null!', divContainer));
      return;
    }
    const sstrKeys = GetObjKeys(objPrjDataBase);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objPrjDataBase.GetFldValue(strKey);
   ...
   * @param pobjPrjDataBaseEN:表实体类对象
   * @returns 列表的第一个关键字值
   **/
