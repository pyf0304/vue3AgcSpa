...
export const PRJDATABASE_COLUMN_SPECS_AI2: Array<PrjDataBaseColumnSpecAi2> = [
  {
...
export function getListColumnsAi2(): Array<clsDataColumn> {
  ...
  return [
    checkboxColumn,
    ...PRJDATABASE_COLUMN_SPECS_AI2.filter((x) => x.includeInList).map(toDataColumn),
  ];
}

/** 获取导出列规格。 */
export function getExportColumnSpecsAi2(): Array<PrjDataBaseColumnSpecAi2> {
  return PRJDATABASE_COLUMN_SPECS_AI2.filter((x) => x.includeInExport);
}
