...
/** Ai 列规格：复用通用表列规格。 */
export type PrjDataBaseColumnSpecAi = TableColumnSpec;

/** 列定义主表：通过 includeInList/includeInExport 控制最终用途。 */
export const PRJDATABASE_COLUMN_SPECS_AI: Array<PrjDataBaseColumnSpecAi> = [
  {
...
/** 把规格对象转换为前端表格列对象。 */
function toDataColumn(spec: PrjDataBaseColumnSpecAi): clsDataColumn {
  return {
...
export function getListColumnsAi(): Array<clsDataColumn> {
  ...
  return [
    checkboxColumn,
    ...PRJDATABASE_COLUMN_SPECS_AI.filter((x) => x.includeInList).map(toDataColumn),
  ];
}

/** 获取导出列规格。 */
export function getExportColumnSpecsAi(): Array<PrjDataBaseColumnSpecAi> {
  return PRJDATABASE_COLUMN_SPECS_AI.filter((x) => x.includeInExport);
}
