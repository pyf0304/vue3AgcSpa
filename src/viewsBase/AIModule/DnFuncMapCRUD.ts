/**
 * 类名:DnFuncMapCRUD(界面:DnFuncMapCRUD,00050283)
 * 表名:DnFuncMap(00050549)
 * 版本:2025.05.12.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/15 01:34:45
 * 生成者:
 工程名称:AGC(0005)
...
/** DnFuncMapCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class DnFuncMapCRUD implements clsOperateList {
  ...
  public static objPageCRUD: DnFuncMapCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    DnFuncMapCRUD.objPageCRUD = this;
  ...
  public async PageLoad() {
    ...
    try {
      ...
      if (viewVarSet.sortDnFuncMapBy == '')
        viewVarSet.sortDnFuncMapBy = `${clsDnFuncMapEN.con_UpdDate} Asc`;
   ...
   **/
  public CombineData(
    arrDnFuncMapObjLst: Array<clsDnFuncMapEN>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
    const intRowNum = arrDnFuncMapObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsDnFuncMapEN = arrDnFuncMapObjLst[i];
    ...
    const strFileName = Format('结点函数映射({0})导出.xlsx', clsDnFuncMapEN._CurrTabName);
    const strSheetName = '结点函数映射列表';
  ...
  public CombineData4Func(
    ...
    const intRowNum = arrDnFuncMapExObjLst.length;
    const intColNum = arrDataColumn.length;
    const arrData: Array<Record<string, any>> = [];
    for (let i = 0; i < intRowNum; i++) {
      const objEN: clsDnFuncMapENEx = arrDnFuncMapExObjLst[i];
      const objRow: Record<string, any> = {};
      for (let j = 0; j < intColNum; j++) {
        const fldName = arrDataColumn[j].fldName;
        const colHeader = arrDataColumn[j].colHeader;
   ...
   **/
  public async ExportExcel_DnFuncMap4Func(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_DnFuncMap4Func.name;
    if (viewVarSet.sortDnFuncMapBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortDnFuncMapBy)为空,请检查!(In BindGv_DnFuncMapCache)',
      );
    ...
    const strWhereCond = await CombineDnFuncMapCondition();
    ...
    try {
      ...
      const arrDnFuncMapObjLst = await DnFuncMap_GetObjLstAsync(strWhereCond);
      arrDnFuncMapExObjLst = arrDnFuncMapObjLst.map(DnFuncMapEx_CopyToEx);
    } catch (e) {
    ...
    if (arrDnFuncMapExObjLst.length == 0) {
      const strKey = Format('{0}', clsDnFuncMapEN._CurrTabName);
    ...
    try {
      ...
      arrDnFuncMapExObjLst = arrDnFuncMapExObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrDnFuncMapExObjLst, arrDataColumn);
      //console.log("完成BindGv_DnFuncMap!");
    } catch (e) {
   ...
   **/
  public async btnSelectRecordInTab_Click(strDnFuncMapId: string) {
    ...
    try {
      ...
      this.SelectRecord(strDnFuncMapId);
    } catch (e) {
   ...
   **/
  public async DelRecord(strDnFuncMapId: string) {
    const strThisFuncName = this.DelRecord.name;
    try {
      const returnInt = await DnFuncMap_DelRecordAsync(strDnFuncMapId);
      if (returnInt > 0) {
   ...
   * @param sender:参数列表
   **/
  public async SelectRecord(strDnFuncMapId: string) {
    const strThisFuncName = this.SelectRecord.name;
    try {
      const objDnFuncMapEN = await DnFuncMap_GetObjByDnFuncMapIdAsync(strDnFuncMapId);
      console.log('完成SelectRecord!', objDnFuncMapEN);
      Redirect('/Index/Main_DnFuncMap');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_DnFuncMap4Func();
  }

  /** 显示DnFuncMap对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrDnFuncMapObjLst:需要绑定的对象列表
   **/
  public async BindTab_DnFuncMap(
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      {
        fldName: clsDnFuncMapENEx.con_InDataNodeName,
      ...
      {
        fldName: clsDnFuncMapENEx.con_OutDataNodeName,
      ...
      {
        fldName: clsDnFuncMapENEx.con_AssociationMappingName,
      ...
      {
        fldName: clsDnFuncMapENEx.con_FuncMapModeName,
      ...
      {
        fldName: clsDnFuncMapENEx.con_DnFunctionName,
      ...
      {
        fldName: clsDnFuncMapEN.con_UpdDate,
        sortBy: clsDnFuncMapEN.con_UpdDate,
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '修改日期',
        text: '',
      ...
      {
        fldName: clsDnFuncMapEN.con_TabId,
        sortBy: 'tabName',
        sortFun: SortFun,
        getDataSource: '',
        colHeader: '表名',
    ...
    if (refDnFuncMap_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrDnFuncMapExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_DnFuncMap函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async BindTab_DnFuncMap4Func(
    ...
    if (refDnFuncMap_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_DnFuncMap4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
   ...
   * @param arrDnFuncMapExObjLst:需要映射的对象列表
  ...
  public async BindGv_DnFuncMap4Func(divList: HTMLDivElement) {
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrDnFuncMapExObjLst, true);
        return;
      }
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortDnFuncMapBy, //如果该字段为空,就使用下面的排序函数
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsDnFuncMapEN, b: clsDnFuncMapEN): number {
    if (a.memo == b.memo) {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortDnFuncMapBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsDnFuncMapENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortDnFuncMapBy = sortBy;
    viewVarSet.ascOrDesc4SortFun = ascOrDesc4SortFun;
    DnFuncMapCRUD.sortFunStatic = sortFun;
    await this.BindGv_DnFuncMap4Func(this.listPara.listDiv);
  }
   ...
   **/
  public async CopyRecord(arrDnFuncMapId: Array<string>) {
    const strThisFuncName = this.CopyRecord.name;
    try {
      const arrDnFuncMapObjLst = await DnFuncMap_GetObjLstByDnFuncMapIdLstAsync(arrDnFuncMapId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrDnFuncMapObjLst) {
        const strMaxStrId = await DnFuncMap_GetMaxStrIdAsync();
        //console.log('strMaxStrId=' + strMaxStrId);
        objInFor.dnFuncMapId = strMaxStrId;
        const returnBool = await DnFuncMap_AddNewRecordAsync(objInFor);
  ...
  public async DelMultiRecord(arrDnFuncMapId: Array<string>) {
    ...
    try {
      const returnInt = await DnFuncMap_DelDnFuncMapsAsync(arrDnFuncMapId);
      if (returnInt > 0) {
        //DnFuncMap_ReFreshCache();
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param divContainer:显示容器
   * @param objDnFuncMap:需要显示的对象
   **/
  public ShowDnFuncMapObj(divContainer: HTMLDivElement, objDnFuncMap: clsDnFuncMapEN) {
    if (divContainer == null) {
      alert(Format('所给div为空，divContainer为null!', divContainer));
      return;
    }
    const sstrKeys = GetObjKeys(objDnFuncMap);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objDnFuncMap.GetFldValue(strKey);
