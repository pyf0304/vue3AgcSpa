/**
 * 类名:CacheUseState_Edit(界面:CacheUseStateCRUD,00050292)
 * 表名:CacheUseState(00050566)
 * 版本:2025.05.03.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/08 11:50:40
 * 生成者:
 工程名称:AGC(0005)
...
import {
  ...
  CacheUseState_AddNewRecordWithReturnKeyAsync,
  ...
  CacheUseState_EditRecordExAsync,
} from '@/ts/L3ForWApi/SystemSet/clsCacheUseStateWApi';
import {
  UserId_Local,
  CacheUseState_DeleteKeyIdCache,
  divVarSet,
...
export abstract class CacheUseState_Edit {
  ...
  public HideDialog_CacheUseState() {
    if (CacheUseState_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      refCacheUseState_Edit.value.hideDialog();
    }
  }
  ...
  public async ShowDialog_CacheUseState(strOp: string): Promise<boolean> {
    ...
    if (CacheUseState_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (refCacheUseState_Edit.value == null) {
        const strMsg = Format(
      ...
      await refCacheUseState_Edit.value.showDialog(this);
    }
        ...
        case '确认添加':
          ...
          if (['02', '03', '06'].indexOf(clsCacheUseStateEN.PrimaryTypeId) > -1) {
            ...
            if (returnBool == true) {
              ...
              if (this.iShowList != null)
                this.iShowList.BindGvCache(clsCacheUseStateEN._CurrTabName, this.keyId.toString());
            }
          }
          break;
        case '确认修改':
  ...
  public async AddNewRecord() {
    ...
    refCacheUseState_Edit.value.Clear();
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    let objCacheUseStateEN;
    try {
      objCacheUseStateEN = await refCacheUseState_Edit.value.GetEditDataCacheUseStateObj();
    } catch (e) {
      const strMsg = Format(
        '从界面获取数据不成功,{0}.(in {1}.{2})',
        e,
    ...
    try {
      ...
      if (returnBool == true) {
        ...
        const strInfo = `添加[缓存使用状态(CacheUseState)]记录成功!`;
        //显示信息框
  ...
  public async AddNewRecordWithReturnKeySave(): Promise<number> {
    ...
    try {
      ...
      const responseKeyId = await CacheUseState_AddNewRecordWithReturnKeyAsync(objCacheUseStateEN);
      const returnKeyId: string = responseKeyId;
      if (IsNullOrEmpty(returnKeyId) == false) {
        objCacheUseStateEN.mId = Number(returnKeyId);
        CacheUseState_ReFreshCache(UserId_Local.value);
        const strInfo = `添加[缓存使用状态(CacheUseState)]记录成功!`;
        //显示信息框
        if (this.isShowMsg == true) alert(strInfo);
  ...
  public async ShowData(lngmId: number) {
    ...
    //3、用提供的关键字初始化一个类对象；
    refCacheUseState_Edit.value.ShowDataFromCacheUseStateObj(objCacheUseStateEN);
  }
  ...
  public async UpdateRecord(lngmId: number): Promise<boolean> {
    ...
    try {
      ...
      await refCacheUseState_Edit.value.ShowDataFromCacheUseStateObj(objCacheUseStateEN);
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    const objCacheUseStateEN = await refCacheUseState_Edit.value.GetEditDataCacheUseStateObj();
    ...
    try {
      ...
      if (returnBool == true) {
        CacheUseState_ReFreshCache(UserId_Local.value);
        CacheUseState_DeleteKeyIdCache(UserId_Local.value, this.keyId);
      }
      return returnBool;
    } catch (e) {
      const strMsg = Format(
  ...
  public async EditRecordExSave(): Promise<boolean> {
    ...
    try {
      ...
      const returnBool = await CacheUseState_EditRecordExAsync(objCacheUseStateEN);
      if (returnBool == true) {
        CacheUseState_ReFreshCache(UserId_Local.value);
      }
      return returnBool;
  ...
  public set btnCancelCacheUseState(value: string) {
    refCacheUseState_Edit.value.strCancelButtonText = value;
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitCacheUseState(): string {
    const strValue = refCacheUseState_Edit.value.strSubmitButtonText;
    return strValue;
  }
  /**
  ...
  public set btnSubmitCacheUseState(value: string) {
    refCacheUseState_Edit.value.strSubmitButtonText = value;
  }
}
