/**
 * 类名:CacheUseStateCRUD(界面:CacheUseStateCRUD,00050292)
 * 表名:CacheUseState(00050566)
 * 版本:2025.05.12.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/15 01:34:33
 * 生成者:
 工程名称:AGC(0005)
...
import {
  ...
  CombineCacheUseStateConditionObj,
  divVarSet,
  viewVarSet,
  dataColumn,
  BindTabByList,
...
import {
  ...
  CacheUseState_GetObjExLstByPagerCache,
...
export abstract class CacheUseStateCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortCacheUseStateBy == '')
        viewVarSet.sortCacheUseStateBy = `${clsCacheUseStateEN.con_mId} Asc`;
   ...
   **/
  public async ExportExcel_CacheUseState4Func(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_CacheUseState4Func.name;
    if (viewVarSet.sortCacheUseStateBy == null) {
      const strMsg = Format(
    ...
    const objCacheUseStateCond = await CombineCacheUseStateConditionObj4ExportExcel();
    ...
    try {
      ...
      const arrCacheUseStateObjLst = await CacheUseState_GetSubObjLstCache(
        objCacheUseStateCond,
        UserId_Local.value,
      );
    ...
    if (arrCacheUseStateExObjLst.length == 0) {
      const strKey = Format('{0}_{1}', clsCacheUseStateEN._CurrTabName, UserId_Local.value);
    ...
    try {
      ...
      arrCacheUseStateExObjLst = arrCacheUseStateExObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrCacheUseStateExObjLst, arrDataColumn);
  ...
  public async DelRecord(lngmId: number) {
    ...
    try {
      ...
      if (returnInt > 0) {
        CacheUseState_ReFreshCache(UserId_Local.value);
  ...
  public async BindTab_CacheUseState(
    ...
    if (refCacheUseState_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrCacheUseStateExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_CacheUseState函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async BindTab_CacheUseState4Func(
    ...
    if (refCacheUseState_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_CacheUseState4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_CacheUseState4Func(this.listPara.listDiv);
  }
  ...
  public async BindGv_CacheUseState4Func(divList: HTMLDivElement) {
    ...
    const objCacheUseStateCond = await CombineCacheUseStateConditionObj();
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrCacheUseStateExObjLst, true);
        return;
      }
      ...
      if (CacheUseStateCRUD.sortFunStatic != undefined) {
        strSortFun = CacheUseStateCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
      }
      const objPagerPara: stuPagerPara = {
        pageIndex: intCurrPageIndex,
        pageSize: this.pageSize,
        whereCond: strWhereCond,
        conditionCollection: objCacheUseStateCond,
        orderBy: viewVarSet.sortCacheUseStateBy,
        sortFun: strSortFun,
      };
      arrCacheUseStateExObjLst = await CacheUseState_GetObjExLstByPagerCache(
        objPagerPara,
        UserId_Local.value,
      );
    } catch (e) {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortCacheUseStateBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsCacheUseStateENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortCacheUseStateBy = sortBy;
