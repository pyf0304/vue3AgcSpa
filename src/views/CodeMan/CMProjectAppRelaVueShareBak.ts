...
export const CmPrjId_Session = ref(''); //2、界面操作类型变量,例如AddNew,Update等
...
const refCMProjectAppRela_DetailAi = ref();
...
export function CMProjectAppRela_DeleteKeyIdCache(
  ...
  if (key.cMProjectAppRelaId != 0) {
    // 使用 delete 删除特定的键
    const cacheKey = `${key.cMProjectAppRelaId}_${strPrjId}`;
