/*import $ from "jquery";*/
import { AdjustOrderNum_EdtEx } from './AdjustOrderNum_EdtEx';
...
/** TabFeatureCRUD_EditEx_AdjustOrderNum 的摘要说明。其中Q代表查询,U代表修改
 (AutoGCLib.WA_ViewScriptCSEx_TS4TypeScript:GeneCode)
*/
export class TabFeatureCRUD_EditEx_AdjustOrderNum extends TabFeatureCRUD implements IShowList {
  ...
  async BindGvCache(strType: string, strPara: string) {
    ...
    switch (strType) {
      case 'TabFeature':
        divList = GetDivObjInDivObj(divVarSet.refDivLayout, 'divTabFeatureLst0173');
        ...
        break;
      case 'AdjustOrderNum_Edt':
        divList = GetDivObjInDivObj(divVarSet.refDivLayout, 'divTabFeatureLst0167');
        if (divList == null) return;
        await this.BindGv_TabFeature4Func(divVarSet.refDivList);
        await this.BindLi_TabFeatureLst(TabId_Static.value, divList);
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      let strTabId = '';
      if (IsNullOrEmpty(this.GetTabId) == false) {
        strTabId = this.GetTabId;
      } else if (IsNullOrEmpty(clsPrivateSessionStorage.tabId_Main) == false) {
  ...
  public async BindLi_TabFeatureLst(strTabId: string, divList: HTMLDivElement) {
    // strDivName = strDivName + this.featureId;
    ...
    try {
      if (IsNullOrEmpty(strTabId) == true) {
        ...
        arrTabFeatureLst = await TabFeatureEx_GetObjLstByTabIdCache(
    ...
    //const strCmPrjId = TabFeatureCRUD_EditEx_AdjustOrderNum.CmPrjIdCache;
    //const intFldId = 0;
  ...
  public GetUl_CreateFunc(strFeatureId: string, strTabId: string): HTMLUListElement {
    ...
    (function (strFeatureId1, strTabId1) {
      btnCreateFunc.onclick = function () {
        TabFeatureCRUD_EditEx_AdjustOrderNum.btnCreateBindDdlFunc(strFeatureId1, strTabId1);
      };
  ...
  public static btnEditAdjustOrderNum(strTabFeatureId: string) {
    const objPage: TabFeatureCRUD_EditEx_AdjustOrderNum =
      new TabFeatureCRUD_EditEx_AdjustOrderNum();
    const objPageEdit: AdjustOrderNum_EdtEx = new AdjustOrderNum_EdtEx(
  ...
  public GetButton_CreateTabFeature(strFldId: string, strTabId: string): HTMLButtonElement {
    ...
    (function (strFldId, strTabId1) {
      btnCreateFunc.onclick = function () {
        TabFeatureCRUD_EditEx_AdjustOrderNum.btnCreateBindDdlFunc(strFldId, strTabId1);
      };
    })(strFldId, strTabId);
  ...
  public GetButton_UpdateTabFeature(strTabFeatureId: string): HTMLButtonElement {
    ...
    (function (strTabFeatureId1) {
      btnUpdateteTabFeature.onclick = function () {
        TabFeatureCRUD_EditEx_AdjustOrderNum.btnUpdateTabFeature(strTabFeatureId1);
      };
    })(strTabFeatureId);
  ...
  public GetButton_EditAdjustOrderNum(strTabFeatureId: string): HTMLButtonElement {
    const btnUpdateteTabFeature: HTMLButtonElement = document.createElement('button');

    btnUpdateteTabFeature.id = Format('btnEditAdjustOrderNum{0}', strTabFeatureId);
    btnUpdateteTabFeature.className = 'btn btn-outline-info btn-sm text-nowrap ml-1';
    btnUpdateteTabFeature.title = '编辑调整次序的字段';

    (function (strTabFeatureId1) {
      btnUpdateteTabFeature.onclick = function () {
        TabFeatureCRUD_EditEx_AdjustOrderNum.btnEditAdjustOrderNum(strTabFeatureId1);
      };
    })(strTabFeatureId);
    btnUpdateteTabFeature.innerText = '编辑字段';
    return btnUpdateteTabFeature;
  }
  public GetButton_GeneCode(strTabFeatureId: string): HTMLButtonElement {
    ...
    btnGeneCode.title = '生成调整次序的相关代码';

    (function (strTabFeatureId1, divLayout) {
      btnGeneCode.onclick = function () {
        TabFeatureCRUD_EditEx_AdjustOrderNum.btnGeneCode(strTabFeatureId1, divLayout);
      };
  ...
  public static async btnCopyNode_Click(strTabFeatureId: string, divLayout: HTMLDivElement) {
    try {
      ...
      const divList = GetDivObjInDivObj(divLayout, 'divTabFeatureLst0167');
      if (divList == null) return;
      const objTabFeatureCRUD_EditEx = new TabFeatureCRUD_EditEx_AdjustOrderNum();
  ...
  public GetButton_CopyNode(strTabFeatureId: string): HTMLButtonElement {
    ...
    (function (strTabFeatureId1, divLayout) {
      btnCopyNode.onclick = function () {
        TabFeatureCRUD_EditEx_AdjustOrderNum.btnCopyNode_Click(strTabFeatureId1, divLayout);
      };
    })(strTabFeatureId, divVarSet.refDivLayout);
  ...
  public GetButton_DelNode(strTabFeatureId: string): HTMLButtonElement {
    ...
    (function (strTabFeatureId1, divLayout) {
      btnDelNode.onclick = function () {
        TabFeatureCRUD_EditEx_AdjustOrderNum.btnDelRecordEx_Click(strTabFeatureId1, divLayout);
      };
    })(strTabFeatureId, divVarSet.refDivLayout);
  ...
  public async GetDiv_TabFeature(objTabFeature: clsvTabFeature_SimENEx): Promise<HTMLDivElement> {
    ...
    const btnEditAdjustOrderNum: HTMLButtonElement = this.GetButton_EditAdjustOrderNum(
      objTabFeature.tabFeatureId,
    );
  ...
  public async btnTab_Click() {
    const divList = GetDivObjInDivObj(divVarSet.refDivLayout, 'divTabFeatureLst0167');
    if (divList == null) return;
    await this.BindLi_TabFeatureLst(TabId_Static.value, divList);
  }
  ...
  public static async GeneCode4Feature(
    ...
    const vsTabId: string = strTabId; // this.getQueryString("tabId");//WebApp,,,wfmExamType_QUDI
    const vsTypeParas = '';
    ...
    try {
      const objGCResult = await AutoGeneCode_GeneCode4FeatureAsync(objGCPara);

      //CheckControlExistInDivObj(GeneTabCodeEx.divCode, "textarea", "txtCodeText");
      //CheckControlExistInDivObj(GeneTabCodeEx.divCode, "input", "lblClassName");
      //CheckControlExistInDivObj(GeneTabCodeEx.divCode, "input", "lblFileName");
      //CheckControlExistInDivObj(GeneTabCodeEx.divCode, "span", "lblPrjName");
      //CheckControlExistInDivObj(GeneTabCodeEx.divCode, "span", "lblSQLDSTypeName");
      //CheckControlExistInDivObj(GeneTabCodeEx.divCode, "span", "lblCodeTypeName");
      //CheckControlExistInDivObj(GeneTabCodeEx.divCode, "span", "lblLangType");
      //CheckControlExistInDivObj(GeneTabCodeEx.divCode, "span", "lblGCUserId");
      //CheckControlExistInDivObj(GeneTabCodeEx.divCode, "span", "lblTabName");
  ...
  public get featureId(): string {
    return TabFeatureCRUD_EditEx_AdjustOrderNum.GetPropValue('featureId');
  }
