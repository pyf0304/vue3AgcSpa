      ...
      <table>
        <tbody>
          <tr class="text-primary">
            ...
            <th @click="sortColumn('tabRelationTypeName|Ex')">
              关系类型
              <span>
                <i
                  :class="
                    sortColumnKey === 'tabRelationTypeName|Ex'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
            ...
            <th @click="sortColumn('relaTabName|Ex')">
              相关表名
              <span>
                <i
                  :class="
                    sortColumnKey === 'relaTabName|Ex'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            ...
            <td v-html="item.prjRelationId"></td>

            <td v-html="item.relationName"></td>

            <td v-html="item.tabName"></td>

            <td v-html="item.tabRelationTypeName"></td>

            <td v-html="item.relaTabName"></td>

            <td v-html="item.isUpdMainTabDate"></td>

            <td v-html="item.isDelRelateRec"></td>

            <td v-html="item.memo"></td>

            <td v-html="item.isCheckCurrData"></td>

            <td v-html="item.isUpdRelateFld"></td>

            <td v-html="item.isCopyForceRela"></td>
    ...
    setup(props, { emit }) {
      ...
      const btnClickInRow = (item: any) => {
        emit('on-edit-tab-relainfo', {
          prjRelationId: item.prjRelationId,
          content: '这是当前表的关键字',
        });
      };
