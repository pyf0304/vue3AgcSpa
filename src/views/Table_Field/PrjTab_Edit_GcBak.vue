      ...
      >
        <tbody>
          ...
          <tr id="trRelaTabId4View">
            <td class="text-right">
              <label
                id="lblRelaTabId4View"
                name="lblRelaTabId4View"
                class="col-form-label text-right"
                style="width: 90px"
                >视图相关表
              </label>
            ...
            <td class="text-left" ColSpan="2">
              <span class="form-control form-control-sm" style="width: 200px">
                <input
                  ...
                  type="checkbox"
                  Text="是否与SQL表相关"
                /><label for="chkIsReleToSqlTab">是否与SQL表相关</label></span
              >
            </td>
          </tr>
          <tr id="trMemo">
          ...
          <tr id="trParaVar1TS">
            <td class="text-right">
              <label
                id="lblCacheClassifyFieldTS"
                name="lblCacheClassifyFieldTS"
                class="col-form-label text-right"
                style="width: 90px"
                >缓存分类字段_TS
              </label>
          ...
          <tr id="trParaVar2TS">
            <td class="text-right">
              <label
                id="lblCacheClassifyField2TS"
                name="lblCacheClassifyField2TS"
                class="col-form-label text-right"
                style="width: 90px"
                >缓存分类字段2_TS
              </label>
          ...
          <tr id="trWhereFormatBack">
            <td class="text-right">
              <label
                id="lblWhereFormatBack"
                name="lblWhereFormatBack"
                class="col-form-label text-right"
                style="width: 90px"
                >后台缓存条件格式
              </label>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      async function BindDdl4EditRegionInDiv() {
        ...
        arrvFieldTab_Sim.value = await vFieldTab_SimEx_GetArrvFieldTab_SimByTabIdCache(strCmPrjId); //编辑区域
      ...
      async function GetEditDataPrjTabObj() {
        ...
        pobjPrjTabEN.SetRelaTabId4View(relaTabId4View.value); // 视图相关表
        ...
        pobjPrjTabEN.SetCacheModeId(cacheModeId.value); // 缓存方式
        pobjPrjTabEN.SetCacheClassifyField(cacheClassifyField.value); // 缓存分类字段
        pobjPrjTabEN.SetCacheClassifyField2(cacheClassifyField2.value); // 缓存分类字段2
        ...
        pobjPrjTabEN.SetCacheClassifyFieldTS(cacheClassifyFieldTS.value); // 缓存分类字段_TS
        pobjPrjTabEN.SetParaVar1TS(paraVar1TS.value); // 参数变量_TS
        pobjPrjTabEN.SetCacheClassifyField2TS(cacheClassifyField2TS.value); // 缓存分类字段2_TS
        pobjPrjTabEN.SetParaVar2TS(paraVar2TS.value); // 参数变量2_TS
      ...
      async function ShowDataFromPrjTabObj(pobjPrjTabEN: clsPrjTabEN) {
        ...
        cacheClassifyFieldTS.value = pobjPrjTabEN.cacheClassifyFieldTS; // 缓存分类字段_TS
        paraVar1TS.value = pobjPrjTabEN.paraVar1TS; // 参数变量_TS
        cacheClassifyField2TS.value = pobjPrjTabEN.cacheClassifyField2TS; // 缓存分类字段2_TS
        paraVar2TS.value = pobjPrjTabEN.paraVar2TS; // 参数变量2_TS
        isMultiKeyClassify.value = pobjPrjTabEN.isMultiKeyClassify; // 是否可以多个关键字分类共存
        whereFormat.value = pobjPrjTabEN.whereFormat; // 缓存条件格式
        isShare.value = pobjPrjTabEN.isShare; // 是否共享表
        isUseDelSign.value = pobjPrjTabEN.isUseDelSign; // 是否使用删除标记
        whereFormatBack.value = pobjPrjTabEN.whereFormatBack; // 后台缓存条件格式
      }
