  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const btnCreate_Click = async () => {
        ...
        objPage_Edit.value = new PrjDataBase_EditEx(objPage.value);
        await objPage_Edit.value.btnAddNewRecord_Click();
      };
      /**
       * 修改记录
      ...
      const btnUpdate_Click = async () => {
        ...
        if (IsNullOrEmpty(strKeyId) || strKeyId === 'undefined') {
          alert('请选择需要修改的记录!');
          return;
        }
        objPage_Edit.value = new PrjDataBase_EditEx(objPage.value);
        await objPage_Edit.value.btnUpdateRecord_Click(strKeyId);
      };
