    ...
    <div id="divQuery" ref="refDivQuery" class="div_query">
      ...
      >
        <tbody>
          <tr>
            <td class="text-right"
              ><label class="col-form-label text-right" style="width: 90px">数据库拥有者</label></td
            >
            <td class="text-left"
              ><input
            ...
            /></td>
            <td class="text-right"
              ><label class="col-form-label text-right" style="width: 90px">数据库类型ID</label></td
            >
            <td class="text-left">
              <select
            ...
            /></td>
            <td class="text-right"
              ><label class="col-form-label text-right" style="width: 90px">服务器</label></td
            >
            <td class="text-left"
              ><input
                id="txtIpAddress_q"
          ...
          <tr>
            <td class="text-right"
              ><label class="col-form-label text-right" style="width: 90px">使用状态Id</label></td
            >
            <td class="text-left">
              <select
                id="ddlUseStateId_q"
...
<script lang="ts">
  ...
  import PrjDataBaseCRUDExAi2 from '@/views/PrjManage/PrjDataBaseCRUDExAi2';

  export default defineComponent({
    name: 'PrjDataBaseCRUDAi2',
    components: {
      PrjDataBase_EditCom,
    ...
    setup() {
      const strTitle = ref('数据库对象维护(Ai2版)');
      const objPage = ref<PrjDataBaseCRUDExAi2>();
      const arrDataBaseType = ref<clsDataBaseTypeEN[] | null>([]);
      const arrUseState = ref<clsUseStateEN[] | null>([]);

      const ensurePage = (): PrjDataBaseCRUDExAi2 | null => {
      ...
      const btnExportExcel_Click = async () => {
        const page = ensurePage();
        if (page == null) return;
        const data: ExportExcelData = await page.onExportAi2();
        if (data.sheetName === '') {
          alert('获取导出数据出错,请检查!');
          return;
        }
      ...
      function btn_Click(strCommandName: string, strKeyId: string) {
        PrjDataBaseCRUDExAi2.btn_Click(strCommandName, strKeyId);
      }

      onMounted(async () => {
        await BindDdl4QryRegion();
        await BindDdl4FeatureRegion();
        PrjDataBaseCRUDExAi2.vuebtn_Click = btn_Click;
        PrjDataBaseCRUDExAi2.GetPropValue = GetPropValue;
        objPage.value = new PrjDataBaseCRUDExAi2();
