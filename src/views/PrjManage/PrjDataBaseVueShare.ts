...
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjDataBaseEN.con_DatabaseOwner, databaseOwner_q.value);
}
if ( dataBaseTypeId_q.value != "" && dataBaseTypeId_q.value != "0")
{
strWhereCond += Format(" And PrjDataBase.{0} = '{1}'", clsPrjDataBaseEN.con_DataBaseTypeId, dataBaseTypeId_q.value);
}
if ( dataBaseUserId_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjDataBaseEN.con_DataBaseUserId, dataBaseUserId_q.value);
}
if ( ipAddress_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjDataBaseEN.con_IpAddress, ipAddress_q.value);
}
if ( useStateId_q.value != "" && useStateId_q.value != "0")
{
strWhereCond += Format(" And PrjDataBase.{0} = '{1}'", clsPrjDataBaseEN.con_UseStateId, useStateId_q.value);
}

}
catch(objException)
{
const strMsg:string = Format("在组合查询条件(CombinePrjDataBaseCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
};
...
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjDataBaseEN.con_DatabaseOwner, databaseOwner_q.value);
objPrjDataBaseCond.SetCondFldValue(clsPrjDataBaseEN.con_DatabaseOwner, databaseOwner_q.value, "like");
}
if ( dataBaseTypeId_q.value != "" && dataBaseTypeId_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsPrjDataBaseEN.con_DataBaseTypeId, dataBaseTypeId_q.value);
objPrjDataBaseCond.SetCondFldValue(clsPrjDataBaseEN.con_DataBaseTypeId, dataBaseTypeId_q.value, "=");
}
if ( dataBaseUserId_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjDataBaseEN.con_DataBaseUserId, dataBaseUserId_q.value);
objPrjDataBaseCond.SetCondFldValue(clsPrjDataBaseEN.con_DataBaseUserId, dataBaseUserId_q.value, "like");
}
if ( ipAddress_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjDataBaseEN.con_IpAddress, ipAddress_q.value);
...
if ( useStateId_q.value != "" && useStateId_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsPrjDataBaseEN.con_UseStateId, useStateId_q.value);
...
{
const strMsg:string = Format("在组合查询条件对象(CombinePrjDataBaseConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
export const CombinePrjDataBaseConditionObj4ExportExcel = async ():Promise<ConditionCollection> => 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
const objPrjDataBaseCond = new ConditionCollection();
...
{
const strMsg:string = Format("在组合导出Excel条件对象(CombinePrjDataBaseConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objPrjDataBaseCond.whereCond = strWhereCond;
return objPrjDataBaseCond;
...
if (
IsNullOrEmpty(key.prjDataBaseId) == false )
{
// 使用 delete 删除特定的键
