...
import { PrjDataBaseCRUDAi3 } from '@/viewsBase/PrjManage/PrjDataBaseCRUDAi3';
import { PRJ_DATABASE_QUERY_SPECS_AI3 } from '@/viewsBase/PrjManage/PrjDataBaseCRUDAi3Query';

export default class PrjDataBaseCRUDExAi3 extends PrjDataBaseCRUDAi3 implements IShowList {
  public static vuebtn_Click: (strCommandName: string, strKeyId: string) => void;
  public static GetPropValue: (strPropName: string) => string;
  private static isQueryFieldLabelsInitialized = false;
   ...
   *
   * 设计目的：
   * 1) 不改 Base 的 schema 文件（避免重新生成时被覆盖）。
   * 2) 把项目差异化文案集中在 Ex 层，开发者只维护一份映射。
   * 3) 使用一次性初始化，避免重复执行带来的理解成本。
   */
  public static initQueryFieldLabels() {
    if (this.isQueryFieldLabelsInitialized) return;

    const labelMap: Record<string, string> = {
      dataBaseTypeId_q: '数据库类型',
      dataBaseUserId_q: '数据库用户',
    };

    PRJ_DATABASE_QUERY_SPECS_AI3.forEach((field) => {
      const label = labelMap[field.key];
      if (label != null) {
        field.label = label;
      }
    });

    this.isQueryFieldLabelsInitialized = true;
  }
  ...
  public static async btn_Click(strCommandName: string, strKeyId: string) {
    ...
    switch (strCommandName) {
      ...
      case 'Delete':
        ...
        break;
      default:
        strMsg = `命令:${strCommandName}在函数(PrjDataBaseCRUDExAi3.btn_Click)中没有被处理!`;
        console.error(strMsg);
        alert(strMsg);
        break;
    }
