      ...
      <table>
        <tbody>
          <tr class="text-primary">
            ...
            <th @click="sortColumn('ctlTypeName|Ex')">
              控件类型名
              <span>
                <i
                  :class="
                    sortColumnKey === 'ctlTypeName|Ex'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
            ...
            <th @click="sortColumn('ctlTypeId')">
              控件类型Id
              <span>
                <i
                  :class="
                    sortColumnKey === 'ctlTypeId'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            <td>
              <input
                :id="'chk' + item.fldDispUnitStyleId"
            ...
            <td v-html="item.fldDispUnitStyleId"></td>

            <td v-html="item.ctlTypeName"></td>

            <td v-html="item.fldDispUnitStyleName"></td>

            <td v-html="item.fldDispUnitStyleDesc"></td>

            <td v-html="item.fldDispUnitStyleContent"></td>

            <td v-html="item.fldDispUnitFormat"></td>

            <td v-html="item.styleNameContent"></td>

            <td v-html="item.styleNameTitle"></td>
    ...
    setup(props, { emit }) {
      ...
      const btnClickInRow = (item: any) => {
        emit('on-edit-tab-relainfo', {
          fldDispUnitStyleId: item.fldDispUnitStyleId,
          content: '这是当前表的关键字',
        });
      };
