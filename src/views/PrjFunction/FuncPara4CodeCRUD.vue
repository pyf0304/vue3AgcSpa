<template>
  <div id="divLayout" ref="refDivLayout" class="div_layout">
          ...
          <tr>
            ...
            <td class="text-left">
              <select
                ...
                v-model="funcPurposeId_q"
                class="form-control form-control-sm"
                style="width: 120px"
              >
                <option
                  v-for="(item, index) in arrFunctionPurpose"
                  :key="index"
                  :value="item.funcPurposeId"
                >
                  {{ item.funcPurposeName }}
                </option></select
              >
            </td>
            <td class="text-left">
    ...
    <div id="divList" ref="refDivList" class="div_List">
      <FuncPara4Code_ListCom
        ref="refFuncPara4Code_List"
        :items="dataListFuncPara4Code"
...
<script lang="ts">
  ...
  import FuncPara4Code_ListCom from '@/views/PrjFunction/FuncPara4Code_List.vue';
  ...
  import { FunctionPurpose_GetArrFunctionPurpose } from '@/ts/L3ForWApi/PrjFunction/clsFunctionPurposeWApi';
  export default defineComponent({
    ...
    setup() {
      ...
      const objPage = ref<FuncPara4CodeCRUDEx>();
      const objPage_Edit = ref<FuncPara4Code_EditEx>();
      const opType = ref('');
      const thisConstructorName = 'FuncPara4CodeCRUD';

      const arrDataTypeAbbr = ref<clsDataTypeAbbrEN[] | null>([]);
      const arrFunctionPurpose = ref<clsFunctionPurposeEN[] | null>([]);
      ...
      const btnQuery_Click = async () => {
        ...
        await objPage.value.BindGv_FuncPara4Code4Func(refDivList.value);
      };
      ...
      const btnCreate_Click = async () => {
        ...
        objPage_Edit.value = new FuncPara4Code_EditEx('FuncPara4Code_EditEx', objPage.value);
        ...
        try {
          opType.value = 'Add';
          const bolIsSuccess = await objPage_Edit.value.ShowDialog_FuncPara4Code(opType.value);
      ...
      const btnClone_Click = async () => {
        ...
        try {
          ...
          await objPage.value.BindGv_FuncPara4Code4Func(divVarSet.refDivList);
        } catch (e) {
      ...
      const btnUpdate_Click = async () => {
        ...
        const strFuncParaId4Code = GetFirstCheckedKeyIdInDivObj(divVarSet.refDivList);
        ...
        try {
          ...
          const bolIsSuccess = await objPage_Edit.value.ShowDialog_FuncPara4Code(opType.value);
          if (bolIsSuccess == false) return;
          const update = await objPage_Edit.value.UpdateRecord(strFuncParaId4Code);
          if (update == false) {
      ...
      async function BindDdl4QryRegion() {
        ...
        arrFunctionPurpose.value = await FunctionPurpose_GetArrFunctionPurpose(); //查询区域
        funcPurposeId_q.value = '0';
      }
      ...
      onMounted(async () => {
        ...
        FuncPara4CodeCRUDEx.vuebtn_Click = btn_Click;
        FuncPara4CodeCRUDEx.GetPropValue = GetPropValue;
        objPage.value = new FuncPara4CodeCRUDEx();
    ...
    methods: {
      ...
      async EditTabRelaInfo(data: any) {
        ...
        router.push({ name: 'editFuncPara4Code', params: { courseId: data.courseId } });
      },
