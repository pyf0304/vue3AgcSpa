...
<script lang="ts">
  ...
  import Function4GeneCode_EditAiEx from '@/views/PrjFunction/Function4GeneCode_EditAiEx';
  ...
  import { Function4GeneCode_EditAi } from '@/viewsBase/PrjFunction/Function4GeneCode_EditAi';
  import { enumPageDispMode } from '@/ts/PubFun/enumPageDispMode';
  export default defineComponent({
    name: 'Function4GeneCodeEditAi',
    ...
    setup() {
      ...
      const keyId = ref('');
      const objPage_Edit = ref<Function4GeneCode_EditAiEx>();
      ...
      async function BindDdl4EditRegionInDiv() {
        ...
        arrProgLangType.value = await ProgLangType_GetArrProgLangTypeByIsVisible(
          IsVisible_Giving.value,
        ); //编辑区域
       ...
       **/
      const showDialog = async (pobjPage_Edit: Function4GeneCode_EditAiEx) => {
    ...
    methods: {
      ...
      btnFunction4GeneCode_Edit_Click(strCommandName: string, strKeyId: string) {
        const objPageEdit = this.objPage_Edit as Function4GeneCode_EditAiEx | undefined;
        if (objPageEdit == null) {
          const strMsg = '编辑页面初始化不成功,请联系管理员!(in btnFunction4GeneCode_Edit_Click)';
          console.error(strMsg);
          alert(strMsg);
          return;
        }
        if (IsNullOrEmpty(strKeyId) == false) {
          objPageEdit.btnEdit_Click(strCommandName, { funcId4GC: strKeyId });
        } else {
          objPageEdit.btnEdit_Click(strCommandName, { funcId4GC: '' });
        }
      },
    },
  });
