/**
 * 类名:FunctionTemplate_EditEx(界面:FunctionTemplateCRUD,00050347)
 * 表名:FunctionTemplate(0005)
 * 版本:2026.05.02(服务器:PYF-AI)
 * 日期:2026/05/02 10:17:18
 * 生成者:
 * 工程名称:AGC(0005)
...
import { FunctionTemplate_EditAi } from '@/viewsBase/PrjFunction/FunctionTemplate_EditAi';

/**
 * FunctionTemplate_EditEx 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScript_EditCSExAi_TS4TypeScript:GeneCode)
 *
 * 扩展说明:
 * 1、继承自 FunctionTemplate_EditAi 基类
 * 2、可在此类中添加自定义业务逻辑
 * 3、不会被代码生成器覆盖
 */
export default class FunctionTemplate_EditExAi extends FunctionTemplate_EditAi {
  /**
   * 构造函数
   * (AutoGCLib.Vue_ViewScript_EditCSExAi_TS4TypeScript:Gen_Constructor)
   */
  constructor(objShowList: any) {
    super('FunctionTemplate_EditExAi', objShowList);
  }
