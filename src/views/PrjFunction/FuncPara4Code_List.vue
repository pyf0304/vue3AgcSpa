      ...
      <table>
        <tbody>
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            ...
            <td v-html="item.funcParaId4Code"></td>

            <td v-html="item.paraName"></td>

            <td v-html="item.paraCnName"></td>

            <td v-html="item.dataTypeName"></td>

            <td v-html="item.parameterType"></td>

            <td v-html="item.parameterTypeFullName"></td>

            <td v-html="item.isByRef"></td>
    ...
    setup(props, { emit }) {
      ...
      const btnClickInRow = (item: any) => {
        emit('on-edit-tab-relainfo', {
          funcParaId4Code: item.funcParaId4Code,
          content: '这是当前表的关键字',
        });
      };
