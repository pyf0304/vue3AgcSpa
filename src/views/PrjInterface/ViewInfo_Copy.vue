      ...
      <div class="card-body">
        ...
        <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
          <button
            ...
            :disabled="executeLoading || isTaskCompleted"
            @click="btnExecuteCopy_Click"
          >
            {{ executeBtnText }}
          </button>
          <span v-if="showContinueHint" class="text-danger">请单击后，复制界面</span>
          <button
          ...
          <span class="text-muted">{{ taskRuntimeSummary }}</span>
        </div>
...
<script lang="ts" setup>
  ...
  import {
    ...
    ViewInfoEx_GetCopyTaskStatusByView,
  ...
  const taskIdMapByView = ref<Record<string, number>>({});
  ...
  const executeBtnText = computed(() => {
    ...
    if (isTaskCompleted.value) return '复制完成';
    return '继续复制';
  });

  const showContinueHint = computed(() => executeBtnText.value === '继续复制');

  const executeBtnClass = computed(() => {
    if (isTaskCompleted.value) return 'btn-secondary';
    return 'btn-success';
  });

  const isTaskCompleted = computed(() => {
    const status = normalizeString(currentTaskStatus.value).toLowerCase();
    return (
  ...
  const taskRuntimeSummary = computed(() => {
    if (activeTaskId.value <= 0) {
      return '任务未开始';
    }
    return `任务Id: ${activeTaskId.value} | 状态: ${currentTaskStatus.value || '未知'} | 步骤: ${
      currentStep.value || '未知'
    }`;
  });
  ...
  function buildTaskCacheKey(sourceViewId: string): string {
    return `${targetPrjId}__${sourceViewId}`;
  }

  function getCachedTaskId(sourceViewId: string): number {
    return Number(taskIdMapByView.value[buildTaskCacheKey(sourceViewId)] ?? 0);
  }

  function cacheTaskId(sourceViewId: string, taskId: number) {
    if (taskId <= 0) return;
    taskIdMapByView.value[buildTaskCacheKey(sourceViewId)] = taskId;
  }

  function clearCachedTaskId(sourceViewId: string) {
    delete taskIdMapByView.value[buildTaskCacheKey(sourceViewId)];
  }

  async function checkTargetViewExists(targetViewIdToCheck: string): Promise<boolean> {
    if (IsNullOrEmpty(targetViewIdToCheck)) return false;
    const strWhere =
      `${clsViewInfoEN.con_PrjId}='${escapeSqlLike(targetPrjId)}' and ` +
      `${clsViewInfoEN.con_ViewId}='${escapeSqlLike(targetViewIdToCheck)}'`;
    const arrView = await ViewInfo_GetObjLstAsync(strWhere);
    return arrView.length > 0;
  }

  function isFinishedTaskStatus(statusText: string): boolean {
    const status = normalizeString(statusText).toLowerCase();
    return status.includes('completed') || status.includes('success');
  }

  function shouldReprepareTask(statusText: string): boolean {
    const status = normalizeString(statusText).toLowerCase();
    return (
      status.includes('completed') ||
      status.includes('success') ||
  ...
  function markTaskFailed(error: unknown, stepText: string) {
    if (activeTaskId.value <= 0) return;
    currentTaskStatus.value = currentTaskStatus.value || 'Failed';
    currentStep.value = stepText;
    taskMessage.value = `任务Id=${activeTaskId.value}，${getErrorMessage(error)}`;
  }
  ...
  async function loadSourceRegionsAsPending(sourceViewId: string) {
    const arrRegionId = await ViewRegionRelaEx_GetRegionIdLstByViewId(sourceViewId);
    const arrSourceRegions: Array<clsViewRegionEN> = await viewRegionStore.getObjENLst(arrRegionId);
  ...
  async function btnLoadRegions_Click(row: ViewInfoCopyRow) {
    ...
    try {
      ...
      let hasTaskByView = false;
      try {
        const objTaskByView = await ViewInfoEx_GetCopyTaskStatusByView(targetPrjId, row.viewId);
        if (objTaskByView.taskId > 0) {
          applyTaskStatus(objTaskByView);
          if (isFinishedTaskStatus(currentTaskStatus.value)) {
            const targetExists = await checkTargetViewExists(targetViewId.value);
            if (!targetExists) {
              clearCachedTaskId(row.viewId);
              resetTaskRuntimeInfo();
              taskMessage.value =
                '检测到历史任务已完成，但目标界面已不存在（可能被删除），已重置为未复制状态，请点击“开始复制”。';
            } else {
              hasTaskByView = true;
              taskMessage.value = `检测到历史已完成任务(TaskId=${objTaskByView.taskId})。如需再次复制，请点击“开始复制”，系统会创建新任务。`;
            }
          } else {
            hasTaskByView = true;
            startPolling();
            if (!taskMessage.value) {
              taskMessage.value = `已加载界面区域，并带出当前复制状态。TaskId=${objTaskByView.taskId}`;
            }
          }
        }
      } catch (eByView) {
        console.warn('按界面查询任务状态失败，回退本地缓存任务Id。', eByView);
      }

      if (!hasTaskByView) {
        const cachedTaskId = getCachedTaskId(row.viewId);
        if (cachedTaskId > 0) {
          activeTaskId.value = cachedTaskId;
          ...
          if (isFinishedTaskStatus(currentTaskStatus.value)) {
            ...
            if (!targetExists) {
              ...
              resetTaskRuntimeInfo();
              taskMessage.value =
                '检测到缓存任务已完成，但目标界面已不存在（可能被删除），已重置为未复制状态，请点击“开始复制”。';
            } else if (!taskMessage.value) {
              taskMessage.value = `已加载界面区域，并带出当前复制状态。TaskId=${cachedTaskId}`;
            }
          } else {
            startPolling();
            if (!taskMessage.value) {
          ...
          resetTaskRuntimeInfo();
          taskMessage.value = `仅查看源界面区域: ${row.viewName}(${row.viewId})，当前未带出历史复制任务状态。`;
        }
      }
  ...
  async function btnCopy_Click(row: ViewInfoCopyRow) {
    ...
    try {
      if (row.prjId === targetPrjId) {
        message.warning('不能复制到同一个工程，请选择其他源工程界面。');
        return;
      }
      stopPolling();
      ...
      cacheTaskId(row.viewId, taskResult.taskId);
      ...
      if (!IsNullOrEmpty(taskResult.message)) {
        message.info(taskResult.message);
      }
    } catch (error) {
      const errMsg = getErrorMessage(error);
      if (activeTaskId.value > 0) {
        markTaskFailed(error, '准备复制失败');
        // 任务已创建，仅提示加载失败
        message.warning(
  ...
  async function btnExecuteCopy_Click() {
    ...
    try {
      if (activeTaskId.value <= 0 || shouldReprepareTask(currentTaskStatus.value)) {
  ...
  function applyTaskStatus(taskStatus: ViewInfoExCopyTaskStatusResult) {
    ...
    const strTaskMessage = normalizeString(taskStatus.message);
    if (!IsNullOrEmpty(strTaskMessage)) {
      taskMessage.value = strTaskMessage;
    }
    ...
    if (selectedSourceRow.value != null) {
      cacheTaskId(selectedSourceRow.value.viewId, taskStatus.taskId);
    }
  }
    ...
    try {
      ...
      if (isFinish) {
        ...
        if (status.includes('completed') || status.includes('success')) {
          if (!IsNullOrEmpty(targetViewId.value)) {
            viewInfoStore.delObj(targetViewId.value);
          }
          ViewInfo_ReFreshCache(targetPrjId);
