      ...
      <table>
        <tbody>
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            ...
            <td v-html="item.viewMasterId"></td>

            <td v-html="item.viewMasterName"></td>

            <td v-html="item.viewMasterPath"></td>
    ...
    setup(props, { emit }) {
      ...
      const btnClickInRow = (item: any) => {
        emit('on-edit-tab-relainfo', {
          viewMasterId: item.viewMasterId,
          content: '这是当前表的关键字',
        });
      };
