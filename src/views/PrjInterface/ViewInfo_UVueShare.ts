/**
 * 类名:ViewInfo_UVueShare(界面:ViewInfo_U,00050280)
 * 表名:ViewInfo(00050006)
 * 版本:2025.05.03.1(服务器:WIN-SRV103-116)
 * 日期:2025/05/08 15:09:34
 * 生成者:
 工程名称:AGC(0005)
...
export function ViewInfo_DeleteKeyIdCache(strPrjId: string, strViewId: string): void {
  if (IsNullOrEmpty(strPrjId) == true) {
    const strMsg = Format('参数:[strPrjId]不能为空!(In ViewInfo_UVueShare.ViewInfo.ReFreshCache)');
    console.error(strMsg);
    throw strMsg;
  }
  if (strPrjId.length != 4) {
    const strMsg = Format(
      '缓存分类变量:[strPrjId]的长度:[{0}]不正确!(ViewInfo_UVueShare.ViewInfo.ReFreshCache)',
      strPrjId.length,
    );
    console.error(strMsg);
    throw strMsg;
