      ...
      <table>
        <tbody>
          <tr class="text-primary">
            ...
            <th @click="sortColumn('charEncodingName|Ex')">
              字符编码名称
              <span>
                <i
                  :class="
                    sortColumnKey === 'charEncodingName|Ex'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            ...
            <td v-html="item.progLangTypeId"></td>

            <td v-html="item.progLangTypeName"></td>

            <td v-html="item.progLangTypeENName"></td>

            <td v-html="item.charEncodingId"></td>

            <td v-html="item.charEncodingName"></td>
    ...
    setup(props, { emit }) {
      ...
      const btnClickInRow = (item: any) => {
        emit('on-edit-tab-relainfo', {
          progLangTypeId: item.progLangTypeId,
          content: '这是当前表的关键字',
        });
      };
