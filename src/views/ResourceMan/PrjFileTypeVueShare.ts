...
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjFileTypeEN.con_PrjFileTypeId, prjFileTypeId_q.value);
}
if ( prjFileTypeName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjFileTypeEN.con_PrjFileTypeName, prjFileTypeName_q.value);
}

}
catch(objException)
{
const strMsg:string = Format("在组合查询条件(CombinePrjFileTypeCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
};
...
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjFileTypeEN.con_PrjFileTypeId, prjFileTypeId_q.value);
objPrjFileTypeCond.SetCondFldValue(clsPrjFileTypeEN.con_PrjFileTypeId, prjFileTypeId_q.value, "like");
}
if ( prjFileTypeName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsPrjFileTypeEN.con_PrjFileTypeName, prjFileTypeName_q.value);
objPrjFileTypeCond.SetCondFldValue(clsPrjFileTypeEN.con_PrjFileTypeName, prjFileTypeName_q.value, "like");
}

}
catch(objException)
{
const strMsg:string = Format("在组合查询条件对象(CombinePrjFileTypeConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
export const CombinePrjFileTypeConditionObj4ExportExcel = async ():Promise<ConditionCollection> => 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
const objPrjFileTypeCond = new ConditionCollection();
...
{
const strMsg:string = Format("在组合导出Excel条件对象(CombinePrjFileTypeConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objPrjFileTypeCond.whereCond = strWhereCond;
return objPrjFileTypeCond;
...
if (
IsNullOrEmpty(key.prjFileTypeId) == false )
{
// 使用 delete 删除特定的键
