...
{
strWhereCond += Format(" And FileResource.{0} = '{1}'", clsFileResourceEN.con_CodeTypeId, codeTypeId_q.value);
}
if ( prjFileTypeId_q.value != "" && prjFileTypeId_q.value != "0")
{
strWhereCond += Format(" And FileResource.{0} = '{1}'", clsFileResourceEN.con_PrjFileTypeId, prjFileTypeId_q.value);
}
if ( fileDirName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsFileResourceEN.con_FileDirName, fileDirName_q.value);
}
if ( fileName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsFileResourceEN.con_FileName, fileName_q.value);
}
if ( extension_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsFileResourceEN.con_Extension, extension_q.value);
}
if ( tabId_q.value != "" && tabId_q.value != "0")
{
strWhereCond += Format(" And FileResource.{0} = '{1}'", clsFileResourceEN.con_TabId, tabId_q.value);
}
...
{
const strMsg:string = Format("在组合查询条件(CombineFileResourceCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
};
...
if ( codeTypeId_q.value != "" && codeTypeId_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsFileResourceEN.con_CodeTypeId, codeTypeId_q.value);
objFileResourceCond.SetCondFldValue(clsFileResourceEN.con_CodeTypeId, codeTypeId_q.value, "=");
}
if ( prjFileTypeId_q.value != "" && prjFileTypeId_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsFileResourceEN.con_PrjFileTypeId, prjFileTypeId_q.value);
objFileResourceCond.SetCondFldValue(clsFileResourceEN.con_PrjFileTypeId, prjFileTypeId_q.value, "=");
}
if ( fileDirName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsFileResourceEN.con_FileDirName, fileDirName_q.value);
objFileResourceCond.SetCondFldValue(clsFileResourceEN.con_FileDirName, fileDirName_q.value, "like");
}
if ( fileName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsFileResourceEN.con_FileName, fileName_q.value);
...
{
strWhereCond += Format(" And {0} like '%{1}%'", clsFileResourceEN.con_Extension, extension_q.value);
objFileResourceCond.SetCondFldValue(clsFileResourceEN.con_Extension, extension_q.value, "like");
}
if ( tabId_q.value != "" && tabId_q.value != "0")
{
...
{
const strMsg:string = Format("在组合查询条件对象(CombineFileResourceConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
export const CombineFileResourceConditionObj4ExportExcel = async ():Promise<ConditionCollection> => 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
const objFileResourceCond = new ConditionCollection();
...
{
const strMsg:string = Format("在组合导出Excel条件对象(CombineFileResourceConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objFileResourceCond.whereCond = strWhereCond;
return objFileResourceCond;
