<template>
  <div id="divLayout" ref="refDivLayout" class="div_layout">
          ...
          <tr>
            ...
            <td class="text-left">
              <select
                ...
                style="width: 120px"
              >
                <option v-for="(item, index) in arrViewInfo" :key="index" :value="item.viewId">
                  {{ item.viewName }}
                </option></select
              >
            </td>
    ...
    <div id="divList" ref="refDivList" class="div_List">
      <ViewRegionRela_ListCom
        ref="refViewRegionRela_List"
        :items="dataListViewRegionRela"
...
<script lang="ts">
  ...
  import ViewRegionRela_DetailEx from '@/views/RegionManage/ViewRegionRela_DetailEx';
  ...
  import ViewRegionRela_ListCom from '@/views/RegionManage/ViewRegionRela_List.vue';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const opType = ref('');
      const thisConstructorName = 'ViewRegionRelaCRUD';

      const arrViewInfo = ref<clsViewInfoEN[] | null>([]);
      ...
      const btnQuery_Click = async () => {
        ...
        await objPage.value.BindGv_ViewRegionRela4Func(refDivList.value);
      };
      ...
      const btnCreate_Click = async () => {
        ...
        objPage_Edit.value = new ViewRegionRela_EditEx('ViewRegionRela_EditEx', objPage.value);
        ...
        try {
          opType.value = 'Add';
          const bolIsSuccess = await objPage_Edit.value.ShowDialog_ViewRegionRela(opType.value);
      ...
      const btnDelete_Click = async () => {
        ...
        try {
          ...
          await objPage.value.BindGv_ViewRegionRela4Func(divVarSet.refDivList);
        } catch (e) {
      ...
      const btnExportExcel_Click = async () => {
        ...
        const objExportExcelData: ExportExcelData =
          await objPage.value.ExportExcel_ViewRegionRela4Func();
...
*/
      const btnDetail_Click = async () => {
        ...
        objPage_Detail.value = new ViewRegionRela_DetailEx(objPage.value);
        ...
        const lngmId = Number(strFstKeyId);
        opType.value = 'Detail';
        const bolIsSuccess = await objPage_Detail.value.ShowDialog_ViewRegionRela('Detail');
        if (bolIsSuccess == false) return;
        // 为编辑区绑定下拉框
      ...
      async function BindDdl4QryRegion() {
        ...
        arrViewInfo.value = await ViewInfo_GetArrViewInfoByPrjId(strPrjId); //查询区域
        viewId_q.value = '0';

        arrViewRegion.value = await ViewRegion_GetArrViewRegionByPrjId(strPrjId); //查询区域
        regionId_q.value = '0';
      }
      ...
      onMounted(async () => {
        ...
        ViewRegionRelaCRUDEx.vuebtn_Click = btn_Click;
        ViewRegionRelaCRUDEx.GetPropValue = GetPropValue;
        objPage.value = new ViewRegionRelaCRUDEx();
    ...
    methods: {
      ...
      async EditTabRelaInfo(data: any) {
        ...
        router.push({ name: 'editViewRegionRela', params: { courseId: data.courseId } });
      },
