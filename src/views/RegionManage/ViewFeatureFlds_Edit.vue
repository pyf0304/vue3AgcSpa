  ...
  <div>
    <template v-if="isDialog">
      <a-modal v-model:visible="dialogVisible" :width="dialogWidth" :title="strTitle">
        <div id="divEditLayout" ref="refDivEdit" class="tab_layout">
          ...
          >
            <tr id="trLabelCaption">
              <td class="text-right">
                <label id="lblLabelCaption" class="col-form-label text-right" style="width: 90px"
                  >标签标题
                </label>
              </td>
              <td class="text-left">
          ...
          <table>
            <tr id="trCtrlId">
              <td class="text-right">
                <label id="lblCtrlId" class="col-form-label text-right" style="width: 90px"
                  >控件Id
                </label>
              </td>
              <td class="text-left">
            ...
            <tr id="trVarId">
              <td class="text-right">
                <label id="lblVarId" class="col-form-label text-right" style="width: 90px"
                  >变量Id
                </label>
              </td>
              <td class="text-left">
        ...
        <template #footer>
          <a-button id="btnCancelViewFeatureFlds" @click="dialogVisible = false">{{
            strCancelButtonText
          }}</a-button>
          <a-button
            id="btnSubmitViewFeatureFlds"
            type="primary"
            @click="btnViewFeatureFlds_Edit_Click('Submit', '')"
        ...
        <table id="tabEdit" style="width: 333px" class="border border-info table-bordered">
          ...
          <tr id="trReleFldId">
            ...
            <td class="text-left" colSpan="3">
              ...
              >
                <!-- <option
                  v-for="option in arrBindData.vFieldTab_SimLst"
                  :key="option.fldId"
                  :value="option.fldId"
                >
          ...
          <tr v-if="isShowCtrl === true" id="trCtlTypeId">
            <td class="text-right">
            ...
            <td class="text-left">
              ...
              >
                <!-- <option
                  v-for="option in arrBindData.ctlTypeLst"
                  :key="option.ctlTypeId"
                  :value="option.ctlTypeId"
                >
...
<script lang="ts">
  ...
  */
      ...
      onMounted(async () => {
        // await ShowViewFeatureFldsData(props.objData);
        // //this.myonload();
      });
      ...
      // // Define the handleDropdownChange method
      // const ddlDsTabId_Change = async (strTabId: string) => {
      //   // Do something with the new value, such as updating the state or making an API call
      //   console.log('Selected tabId:', strTabId);
      //   const strCmPrjId = clsPrivateSessionStorage.cmPrjId;

      //   const arrvTabFeature_Sim0: Array<clsvTabFeature_SimEN> =
      //     await vTabFeature_SimEx_GetObjLstCache(strCmPrjId);
      //   arrvTabFeature_Sim.value = arrvTabFeature_Sim0.filter(
      //     (x) => x.tabId == strTabId && x.featureId == enumPrjFeature.Tab_BindDdl_0173,
      //   );
      //   selectedTabFeatureId.value = props.objData.tabFeatureId4Ddl;
      //   if (selectedTabFeatureId.value == null || selectedTabFeatureId.value == '') {
      //     selectedTabFeatureId.value = arrvTabFeature_Sim0[0].tabFeatureId;
      //   }
      //   isShowTabFeature.value = true;
      // };
      // const ddlTabFeatureId4Ddl_Change = async (strTabFeatureId: string) => {
      //   // Do something with the new value, such as updating the state or making an API call
      //   console.log('Selected TabFeatureId:', strTabFeatureId);
      ...
      //     strFldNameCond1.value = await vFieldTab_Sim_GetNameByFldIdCache(fldIdCond1.value);
      //     strFldNameCond2.value = await vFieldTab_Sim_GetNameByFldIdCache(fldIdCond2.value);
      //     const arrGCVariableType = await GCVariableType_GetObjLstCache();
      //     if (arrGCVariableType == null) return;
      //     arrGCVariableType1.value = arrGCVariableType;
      //     arrGCVariableType2.value = arrGCVariableType;

      //     // await GCVariableType_BindDdl_VarTypeIdInDivCache(divVarSet.refDivEdit, 'ddlVarTypeIdCond1');
      //     // await GCVariableType_BindDdl_VarTypeIdInDivCache(divVarSet.refDivEdit, 'ddlVarTypeIdCond2');
      //   } else if (arrTabFeatureFlds_Sel.length == 1) {
      //     isShowCond1.value = true;
      //     isShowCond2.value = false;

      //     fldIdCond1.value = arrTabFeatureFlds_Sel[0].fldId;
      //     fldIdCond2.value = '';
      //     strFldNameCond1.value = await vFieldTab_Sim_GetNameByFldIdCache(fldIdCond1.value);
      //     strFldNameCond2.value = '条件2-字段';
