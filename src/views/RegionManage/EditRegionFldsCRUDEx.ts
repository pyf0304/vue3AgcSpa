...
*/
  ...
  public async ShowCodeTypeButton(strViewId: string) {
    ...
    const objLblAppType = document.getElementById('lblCurrAppType_EditRegion');
    ...
    const objLblFrontend = document.getElementById('lblCurrFrontend_EditRegion');
    if (objLblFrontend != null) {
      const strFrontend =
        EditRegionFldsCRUDEx.objCMProjects?.cmPrjName || clsPrivateSessionStorage.cmPrjId;
      objLblFrontend.textContent = strFrontend;
    }
    const objLblRegionType = document.getElementById('lblCurrRegionType_EditRegion');
    if (objLblRegionType != null) {
      objLblRegionType.textContent = '编辑区(0003)';
    }
    const objLblMachine = document.getElementById('lblCurrMachine_EditRegion');
    ...
    const arrObjLst_V2: Array<clsAppCodeTypeRelaENEx> = arrAppCodeTypeRelaExObjLst.filter(
      (x) =>
        x.dependsOn == 'View' &&
        mapCodeTypeRegionType.get(x.codeTypeId) == enumRegionType.EditRegion_0003,
    );
      ...
      for (const objInFor of arrObjLst_V2) {
        ...
        if (bolIsExistRegion == false) {
          ...
          objButton.onclick = async function (this: HTMLButtonElement, evt: Event) {
            await EditRegionFldsCRUDEx.btnCodeType_Click(this, evt);
          };
        ...
        objHtmlInputButton.onclick = function (this) {
          EditRegionFldsCRUDEx.btnCodeType_Click(this, event);
        };
  ...
  public static async btnCodeType_Click(thisControl: any, event: any) {
    ...
    const objProgress = document.getElementById('lblGeneProgress_EditRegion');
    if (objProgress != null) {
      objProgress.textContent = `开始生成(${strCodeTypeId})...`;
    }
    const bolSuccess = await EditRegionFldsCRUDEx.GeneCode4View(
      strViewId,
      strCodeTypeId,
