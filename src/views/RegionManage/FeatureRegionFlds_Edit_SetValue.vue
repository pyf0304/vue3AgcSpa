<template>
  ...
  <a-modal v-model:visible="dialogVisible" :width="dialogWidth" :title="strTitle">
    <div id="divEditLayout_SetValue" ref="refDivEdit" class="tab_layout">
      <table
        id="tabEdit"
        style="width: 600px"
      ...
      >
        ...
        <tr v-show="isShowDefaultValue" id="trDefaultValue">
          <td class="text-right">
            <label
              id="lblDefaultValue"
...
<script lang="ts">
  ...
  import { refDivEdit } from '@/views/RegionManage/FeatureRegionFlds_SimVueShare';

  import { TabId_Static, RegionId_Static } from '@/views/RegionManage/FeatureRegionFldsVueShare';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const updUser = ref('');
      const updDate = ref('');
      const isShowDefaultValue = ref(false); // 是否显示缺省值输入框
      ...
      async function BindDdl4EditRegionInDiv() {
        ...
        const strTabId = TabId_Static.value; //静态变量;//Session存储、local存储
    ...
    methods: {
      ...
      btnClick(strCommandName: string, strKeyId: string) {
        ...
        //}
        //FeatureRegionFlds_Edit_SetValueEx.btnClick(strCommandName, strKeyId);
      },
      ...
      Submit_FeatureRegionFlds(strOp: string) {
        alert(`提交${strOp}`);
        const objPage = new FeatureRegionFlds_Edit_SetValueEx(
          'FeatureRegionFlds_Edit_SetValueEx',
          new FeatureRegionFldsCRUDEx(),
        );
...
*/
      async ddlValueGivingModeId_SelectedIndexChanged(e: Event) {
        ...
        this.isShowDefaultValue = false; // 隐藏缺省值输入框
        switch (this.valueGivingModeId) {
          case enumValueGivingMode.DefaultValue_01:
            this.isShowDefaultValue = true; // 显示缺省值输入框
            break;
          case enumValueGivingMode.GivenValue_02:
            break;
        }
        //        alert('请在扩展层:FeatureRegionFlds_Edit_SetValueEx中重写该函数！');
      },
    },
  });
</script>
