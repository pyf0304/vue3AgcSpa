    ...
    <template v-else>
      <table>
        <tr class="text-primary">
          ...
          <th style="width: 150px" @click="sortColumn('')">
            区域类型名s
            <span v-if="sortColumnKey === ''">
              <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
            </span>
        ...
        <tr v-for="(item, index) in items" :key="index" class="text-secondary">
          ...
          <td v-html="item.regionTypeNames"></td>

          <td v-html="item.isUseInRegion"></td>
