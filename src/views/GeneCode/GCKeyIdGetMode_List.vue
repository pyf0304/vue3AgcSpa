      ...
      <table>
        <tbody>
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            ...
            <td v-html="item.keyIdGetModeId"></td>

            <td v-html="item.keyIdGetModeName"></td>

            <td v-html="item.keyIdGetModeENName"></td>
    ...
    setup(props, { emit }) {
      ...
      const btnClickInRow = (item: any) => {
        emit('on-edit-tab-relainfo', {
          keyIdGetModeId: item.keyIdGetModeId,
          content: '这是当前表的关键字',
        });
      };
